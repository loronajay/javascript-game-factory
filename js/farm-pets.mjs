...
export function createPetSim(options) {
    ...
    function canSee(pet, target) {
        ...
        for (let index = 1; index <= steps; index += 1) {
            ...
            if (solids.some((obstacle) => obstacleBlocks(point, obstacle, pet.species.radius * 0.6)))
                return false;
        }
        return true;
    }
    ...
    function pickTarget(pet) {
        ...
        for (let attempt = 0; attempt < TARGET_TRIES; attempt += 1) {
            ...
            if (pet.species.habitat === "water") {
                if (!standable(x, z, pet, pet.species))
                    continue;
            }
            else {
                if (!inField(x, z, pet.species.radius) || blockedBySolid(x, z, pet.species.radius) || blockedByPet(x, z, pet, pet.species.radius))
                    continue;
                if (!inside && blockedByKeepOut(x, z, pet.species.radius))
                    continue;
                if (!canSee(pet, { x, z }))
                    continue;
            }
    ...
    function stepAlong(pet, heading, distance, player) {
        for (const feeler of FEELERS) {
            ...
            if (pet.species.habitat === "water") {
                if (!inWater(x, z, pet.species.radius))
                    continue;
            }
            else {
                if (!inField(x, z, pet.species.radius))
                    continue;
                if (blockedBySolid(x, z, pet.species.radius))
                    continue;
            }
            if (blockedByPet(x, z, pet, pet.species.radius))
                continue;
            if (blockedByPlayer(x, z, player, pet.species.radius))
                continue;
            pet.x = x;
            pet.z = z;
            return true;
    ...
    return Object.freeze({
        ...
        sync(layout) {
            ...
            for (const pet of pets) {
                if (pet.species.habitat !== "water" || inWater(pet.x, pet.z, pet.species.radius))
                    continue;
        ...
        pickUp(instanceId) {
            ...
            if (!pet || pet.species.habitat === "water" || pet.state === "carried")
                return false;
            pet.state = "carried";
            pet.moving = false;
            pet.timer = 0;
        ...
        putDown(instanceId, spot) {
            ...
            if (!pet || pet.state !== "carried")
                return false;
            if (!inField(spot.x, spot.z, pet.species.radius) || blockedBySolid(spot.x, spot.z, pet.species.radius) || blockedByPet(spot.x, spot.z, pet, pet.species.radius))
                return false;
            pet.x = spot.x;
            pet.z = spot.z;
            pet.yaw = spot.yaw;
