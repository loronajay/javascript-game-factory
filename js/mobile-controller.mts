const CONTROL_KEY_SPECS: Record<string, { key: string; code: string }> = {
...
function normalizeKeySpec(spec: any): { key: string; code: string } | null {
  if (!spec) return null;
...
function getDirectionKeys(direction: any, dpad: any): any[] {
  const map = dpad || {};
  switch (direction) {
...
function resolveMobileControllerProfile(profileOrId: any): any {
...
function createKeyboardDispatcher({ target, KeyboardEventCtor }: { target?: any; KeyboardEventCtor?: any } = {}) {
  const eventTarget = target || globalThis.window;
  const EventCtor = KeyboardEventCtor || globalThis.KeyboardEvent;
  const held = new Set<string>();

  function dispatch(type: string, spec: any): boolean {
  ...
  function press(spec: any): boolean {
    const keySpec = normalizeKeySpec(spec);
  ...
  function release(spec: any): boolean {
    const keySpec = normalizeKeySpec(spec);
    if (!keySpec) return false;
    const id = `${keySpec.key}:${keySpec.code}`;
    if (!held.has(id)) return false;
...
function isMobileLike(win: any = globalThis.window): boolean {
  if (!win) return false;
...
function directionCenter(direction: any): number | null {
  switch (direction) {
    case 'right': return 0;
...
function angleToDirection(angle: number, mode = 'eight-way'): any {
  const zones = getPadZones(mode).map((zone) => [zone.direction, zone.center, zone.half] as [any, number, number]);

  let bestDirection: any = null;
...
function directionFromPoint(x: number, y: number, rect: any, options: any = {}): any {
...
function ringSegmentPath(cx: number, cy: number, innerRadius: number, outerRadius: number, startDeg: number, endDeg: number): string {
...
function ensureStyle(doc: any, profile: any): void {
...
function positionControl(el: any, placement: any): void {
...
function getPadZones(mode = 'eight-way'): Array<{ direction: string; center: number; half: number; rotation: number }> {
  if (mode === 'horizontal') {
    return [
...
function createPad(doc: any, dispatcher: any, padConfig: any, placement: any): any {
  ...
  const segmentByDirection = new Map<string, any>();
  ...
  let activeDirection: any = null;
  let activeKeys: any[] = [];
  let pointerId: any = null;
  let thumbVisible = false;
  let thumbX = 0;
  let thumbY = 0;

  function setThumbFromEvent(event: any): void {
  ...
  function applyDirection(direction: any): void {
  ...
  function updateFromEvent(event: any): void {
  ...
  pad.addEventListener('pointerdown', (event: any) => {
    pointerId = event.pointerId;
    pad.setPointerCapture?.(event.pointerId);
    updateFromEvent(event);
  });
  pad.addEventListener('pointermove', (event: any) => {
...
function createFaceButton(doc: any, dispatcher: any, buttonConfig: any, placement: any): any {
  ...
  function release(event: any): void {
    if (event.pointerId !== pointerId) return;
    pointerId = null;
...
function mountMobileController(options: any = {}): any {
