...
function formatBulletinDate(value: any): string {
...
export function buildBulletinsPageViewModel(bulletins: any = buildPublicBulletinFeed()) {
...
function renderBulletinCard(item: any): string {
  ...
  return `
    <article class="${cardClass}">
      ...
      ${(item.bodyParagraphs || [])
        .map((paragraph: string) => `<p class="bulletin-card__body">${escapeHtml(paragraph)}</p>`)
        .join("")}
...
export function renderBulletinsPage(doc: Document = globalThis.document, bulletins: any = buildPublicBulletinFeed()) {
