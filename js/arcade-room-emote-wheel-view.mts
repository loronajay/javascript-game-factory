...
import { EMOTE_CATALOG, EMOTE_DISPLAY_MS, type EmoteId, type EmoteWheel } from "./arcade-room-emotes.mjs";

export type EmoteWheelViewOptions = Readonly<{
  wheel: EmoteWheel;
  root: HTMLElement;
  /** Where the sender's own emote pops. */
  selfRoot: HTMLElement;
  now?: () => number;
}>;

export type EmoteWheelView = Readonly<{
  /** Every frame or so: hides the finished self burst. Cheap when nothing changed. */
  tick: () => void;
}>;

export function createEmoteWheelView(options: EmoteWheelViewOptions): EmoteWheelView {
  const { wheel, root, selfRoot } = options;
  const now = options.now ?? (() => performance.now());
  const slots = new Map<EmoteId, HTMLElement>();
  let selfShownAt = -Infinity;
  let selfShownId: EmoteId | null = null;
  ...
  function render(): void {
    ...
    if (open) {
      ...
      for (const [id, slot] of slots) slot.setAttribute("aria-selected", lit?.id === id ? "true" : "false");
