...
function buildLinkedEntryPartnerOptions(viewerPlayerId: any = "", viewerRelationshipsRecord: any = {}): any[] {
...
function buildLinkedEntryAction(event: any, options: any = {}) {
...
export function buildEventDetailViewModel(event: any, options: any = {}) {
  ...
  return {
    ...
    relatedItems: event.relatedGames.length > 0
      ? event.relatedGames.map((slug: any) => ({
...
function renderHeroCard(container: HTMLElement | null, model: any): void {
  ...
  const metaHtml = model.metaItems.map((item: any) => `
        ...
        <div class="event-hero-card__entry-row">
          ...
          >
            ${model.linkedEntryAction.partnerOptions.length > 0
              ? model.linkedEntryAction.partnerOptions.map((option: any) => `
...
function renderPanel(container: HTMLElement | null, title: any, subtitle: any, items: any): void {
  if (!container) return;
...
export function renderEventDetailPage(doc: Document = globalThis.document, options: any = {}) {
...
export function recordLinkedEntryBetweenPlayers(leftPlayerId: any, rightPlayerId: any, options: any = {}) {
