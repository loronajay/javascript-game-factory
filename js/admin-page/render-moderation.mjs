import { badge, button, emptyState, escapeHtml, field, formatDate, panel, select, statusTone, textInput, } from "./render-shared.mjs";
...
const REPORT_FILTERS = ["open", "resolved", "dismissed", "all"];
const TARGET_LABELS = {
    thought: "Thought",
...
function adminRow(admin, isOnlyAdmin) {
    return `
    <li class="admin-row">
      <div class="admin-row__main">
...
export function renderAccounts(state) {
    ...
    <form class="admin-form" data-form="suspend">
      ${field("Player ID", textInput("playerId", "", { placeholder: "player-…" }), "Find this on the player's profile page URL.")}
  ...
  `;
    ...
    <form class="admin-form" data-form="grant-admin">
      ${field("Player ID", textInput("playerId", "", { placeholder: "player-…" }), "That player's account gains full console access immediately.")}
      <div class="admin-form__actions">
...
export function renderAudit(state) {
    if (!state.audit.length)
        return panel("Audit Log", emptyState("No admin actions recorded yet."));
