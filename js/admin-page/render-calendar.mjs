import { badge, button, emptyState, escapeHtml, formatDate, panel, select, textInput, } from "./render-shared.mjs";
...
const PAYMENT_FILTERS = ["paid", "pending", "refunded", "cancelled"];
...
function renderMetrics(state) {
    ...
    if (!metrics)
        return emptyState("Preorder totals are unavailable.");
    const tiles = [
...
function renderOrder(order) {
    ...
    <header class="admin-order__head">
      ...
      <div class="admin-order__flags">
        ...
        ${badge(order.voucherBonusState === "granted" ? "10 vouchers paid"
        : order.voucherBonusState === "already_held" ? "bonus already held"
            : order.voucherBonusState === "revoked" ? "bonus revoked" : "bonus pending", order.voucherBonusState === "granted" ? "good" : "muted")}
      </div>
    </header>
