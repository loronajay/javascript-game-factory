...
function bulletinAttachmentField(editing, state) {
    ...
    <div class="admin-field">
      ...
      >
      <span class="admin-field__hint">${escapeHtml(state.uploadingImage
        ? "Uploading…"
        : "PNG, JPEG, GIF, or WEBP. Flyers keep their shape — portrait posters are letterboxed, never cropped.")}</span>
    </div>
  `;
}
...
function cabinetCard(game, override) {
    ...
    <form class="admin-cabinet${override?.hidden ? " admin-cabinet--hidden" : ""}" data-form="cabinet" data-slug="${escapeHtml(game.slug)}">
      ...
      ${field("Description", textArea("description", override?.description || "", 4, game.description), "Used by catalog search and future cabinet detail surfaces.")}
      <div class="admin-cabinet__grid">
        ${field("Categories / tags", textInput("categories", (override?.categories || []).join(", "), { placeholder: game.categories.join(", ") }), "Comma-separated. The first category is the card label.")}
        ${field("Visual formats", textInput("dimensions", (override?.dimensions || []).join(", "), { placeholder: game.dimensions.join(", ") }), "Use 2d, 3d, or both.")}
      </div>
      <div class="admin-cabinet__grid">
        ${field("Play modes", textInput("playModes", (override?.playModes || []).join(", "), { placeholder: game.playModes.join(", ") }), "Use solo, local, and/or online.")}
        ${field("Hover preview video", textInput("previewVideo", override?.previewVideo || "", { placeholder: "grid-previews/game-slug.webm" }), "Optional local WEBM/MP4. Desktop hover only; screenshots remain the fallback.")}
      </div>
