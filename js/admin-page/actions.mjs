...
function setFlash(state, result, successMessage) {
    if (result?.ok) {
...
export async function runAdminAction(action, context) {
    ...
    if (action === "bulletin:save") {
        ...
        if (!setFlash(state, result, state.editingBulletinId ? "Bulletin saved." : "Bulletin created."))
            return false;
    ...
    if (action === "bulletin:delete") {
        if (!confirmFn("Delete this bulletin? This cannot be undone."))
            return false;
        const result = await api.deleteBulletin(value);
        if (!setFlash(state, result, "Bulletin deleted."))
            return false;
        state.editingBulletinId = "";
        state.pendingImageUrl = "";
        state.bulletinDraft = null;
    ...
    if (action === "event:save") {
        ...
        if (!setFlash(state, result, state.editingEventId ? "Event saved." : "Event created."))
            return false;
        state.editingEventId = result.data?.event?.id || state.editingEventId;
        return true;
    }
    if (action === "event:delete") {
        if (!confirmFn("Delete this event? This cannot be undone."))
            return false;
        const result = await api.deleteEvent(value);
        if (!setFlash(state, result, "Event deleted."))
            return false;
        state.editingEventId = "";
        return true;
    }
    ...
    if (action === "cabinet:reset") {
        if (!confirmFn("Reset this cabinet to its game.json settings?"))
            return false;
    ...
    if (action === "report:remove") {
        ...
        if (!confirmFn("Permanently delete this content? This cannot be undone."))
            return false;
    ...
    if (action === "report:suspend") {
        if (!confirmFn("Suspend this account for 7 days?"))
            return false;
    ...
    if (action === "account:suspend") {
        ...
        if (!confirmFn(`Suspend ${values.playerId} for ${values.days || 7} days?`))
            return false;
...
export async function refreshCurrentTab(state) {
