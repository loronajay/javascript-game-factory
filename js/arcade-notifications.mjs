...
export function renderNotificationItem(notif, onAccept, onReject, onChallengeAccept, onChallengeDecline) {
    ...
    ` : ""}
    ${isEvent ? `
      <a class="notif-item__game-link" href="${escapeHtml(eventSlug
        ? buildHref(`event/index.html?slug=${encodeURIComponent(eventSlug)}`)
        : buildHref("events/index.html"))}">View Event →</a>
    ` : ""}
  ...
  `;
    if (isFriendRequest) {
        li.querySelector("[data-fr-accept]")?.addEventListener("click", async (e) => {
            ...
            btn.disabled = true;
            const other = btn.closest(".notif-item__actions")?.querySelector("[data-fr-reject]");
            if (other)
                other.disabled = true;
        ...
        li.querySelector("[data-fr-reject]")?.addEventListener("click", async (e) => {
            const btn = e.currentTarget;
            btn.disabled = true;
            const other = btn.closest(".notif-item__actions")?.querySelector("[data-fr-accept]");
            if (other)
                other.disabled = true;
    ...
    if (isChallenge) {
        li.querySelector("[data-ch-accept]")?.addEventListener("click", async (e) => {
            ...
            btn.disabled = true;
            const other = btn.closest(".notif-item__actions")?.querySelector("[data-ch-decline]");
            if (other)
                other.disabled = true;
        ...
        li.querySelector("[data-ch-decline]")?.addEventListener("click", async (e) => {
            const btn = e.currentTarget;
            btn.disabled = true;
            const other = btn.closest(".notif-item__actions")?.querySelector("[data-ch-accept]");
            if (other)
                other.disabled = true;
  ...
  `;
    ...
    async function fetchAndRender() {
        ...
        for (const notif of notifications) {
            const item = renderNotificationItem(notif, async (requestId, itemEl) => {
                ...
                if (ok) {
                    ...
                    if (p)
                        p.innerHTML = p.innerHTML.replace("sent you a friend request", "friend request accepted ✓");
                }
                ...
                if (ok) {
                    ...
                    const p = itemEl.querySelector(".notif-item__text");
                    if (p)
                        p.innerHTML = p.innerHTML.replace("sent you a friend request", "friend request declined");
                }
                ...
                if (ok) {
                    ...
                    const p = itemEl.querySelector(".notif-item__text");
                    if (p)
                        p.innerHTML = p.innerHTML.replace("challenged you to", "challenge from").replace("! 🎮", " declined");
                }
            });
