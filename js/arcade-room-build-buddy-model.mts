...
function material(THREE: ThreeNamespace, color: string, roughness = 0.58, metalness = 0.14): any {
  return new THREE.MeshStandardMaterial({ color, roughness, metalness });
}

function imageMaterial(THREE: ThreeNamespace, src: string): any {
...
function addHardHatTopper(THREE: ThreeNamespace, root: any, warning: any, dark: any): void {
...
export function createBuildBuddyCabinet(THREE: ThreeNamespace, definition: CabinetDefinition): any {
  ...
  const marquee = new THREE.Mesh(
    ...
    imageMaterial(THREE, BUILD_BUDDY_CABINET_ART.crewArt),
  );
  ...
  const screen = new THREE.Mesh(
    ...
    imageMaterial(THREE, BUILD_BUDDY_CABINET_ART.keyArt),
  );
  screen.name = "screen";
