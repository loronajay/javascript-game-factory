...
function formatRelativeTime(isoString: any): string {
  if (!isoString) return "";
...
const GESTURE_NOTIFICATION_LABELS: Record<string, string> = Object.freeze({
  poke: "poked",
  hug: "hugged",
  kick: "kicked",
...
const GESTURE_NOTIFICATION_EMOJIS: Record<string, string> = Object.freeze({
  poke: "👈",
  hug: "🤗",
  kick: "👟",
  blowkiss: "💋",
...
function formatNotificationText(notif: any): string {
  const actor = notif.actorDisplayName || "Someone";
  switch (notif.type) {
    case "thought_reaction": {
      const reactionId = notif.payload?.reactionId || "";
      const glyph = (THOUGHT_REACTION_GLYPHS as Record<string, string>)[reactionId] || "reacted to";
      const verb = reactionId && (THOUGHT_REACTION_GLYPHS as Record<string, string>)[reactionId] ? `${glyph}'d` : "reacted to";
...
function buildRootUrl(moduleUrl: string = import.meta.url): URL | null {
  try {
...
export function buildGameHref(slug: any, options: any = {}): string {
...
export function buildHref(path: unknown, options: any = {}): string {
  const rootUrl = buildRootUrl(options?.moduleUrl || import.meta.url);
...
export function buildNotificationsPageHref(options: any = {}): string {
  return buildHref("notifications/index.html", options);
}

export function shouldHighlightNotificationBell(locationRef: { pathname?: string } | null | undefined = globalThis.location): boolean {
  const pathname = String(locationRef?.pathname || "");
  return /\/notifications(?:\/index\.html)?\/?$/.test(pathname);
}

export function buildNotificationDropdownMarkup({ pageHref = "" }: { pageHref?: string } = {}): string {
...
export function renderNotificationItem(notif: any, onAccept: any, onReject: any, onChallengeAccept: any, onChallengeDecline: any): HTMLLIElement {
  ...
  if (isFriendRequest) {
    li.querySelector("[data-fr-accept]")?.addEventListener("click", async (e) => {
      ...
      btn.disabled = true;
      const other = btn.closest(".notif-item__actions")?.querySelector<HTMLButtonElement>("[data-fr-reject]");
    ...
    li.querySelector("[data-fr-reject]")?.addEventListener("click", async (e) => {
      const btn = e.currentTarget as HTMLButtonElement;
      btn.disabled = true;
      const other = btn.closest(".notif-item__actions")?.querySelector<HTMLButtonElement>("[data-fr-accept]");
      if (other) other.disabled = true;
  ...
  if (isChallenge) {
    li.querySelector("[data-ch-accept]")?.addEventListener("click", async (e) => {
      ...
      btn.disabled = true;
      const other = btn.closest(".notif-item__actions")?.querySelector<HTMLButtonElement>("[data-ch-decline]");
      if (other) other.disabled = true;
    ...
    li.querySelector("[data-ch-decline]")?.addEventListener("click", async (e) => {
      const btn = e.currentTarget as HTMLButtonElement;
      btn.disabled = true;
      const other = btn.closest(".notif-item__actions")?.querySelector<HTMLButtonElement>("[data-ch-accept]");
      if (other) other.disabled = true;
...
export async function initNotificationBell(containerEl: HTMLElement | null, playerId: any): Promise<void> {
  ...
  const badge = bell.querySelector(".notif-bell__badge") as HTMLElement;
  const list = dropdown.querySelector(".notif-dropdown__list") as HTMLElement;
  ...
  async function fetchAndRender(): Promise<void> {
    ...
    for (const notif of notifications) {
      const item = renderNotificationItem(
        notif,
        async (requestId: any, itemEl: any) => {
        ...
        async (challengeId: any, gameSlug: any, itemEl: any) => {
        ...
        async (challengeId: any, itemEl: any) => {
  ...
  document.addEventListener("click", (e) => {
    if (!wrap.contains(e.target as Node | null)) {
      dropdown.hidden = true;
    }
  });
