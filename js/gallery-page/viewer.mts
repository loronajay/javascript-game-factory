...
import type { GalleryPhoto, PhotoSocialState, SetPhotosOptions } from "./viewer-state.mjs";
import type { PlatformApiClient } from "../platform/api/platform-api.mjs";

export interface PhotoViewer {
  setPhotos(newPhotos: GalleryPhoto[], opts?: SetPhotosOptions): void;
  setSocialState(photoId: string, state: PhotoSocialState): void;
...
const PHOTO_REACTION_GLYPHS: Record<string, string> = {
  like: "👍", love: "❤️", laugh: "😂", wow: "😮",
  fire: "🔥", sad: "😢", angry: "😡", poop: "💩",
};

function formatViewerDate(ts: unknown): string {
  if (!ts) return "";
  try {
    return new Date(ts as string | number).toLocaleDateString(undefined, {
...
function buildReactionPickerHtml(): string {
...
export function createPhotoViewer({ doc = globalThis.document, lightweight = false }: { doc?: Document; lightweight?: boolean } = {}): PhotoViewer {
  ...
  const imgEl = overlay.querySelector<HTMLImageElement>(".photo-viewer__img")!;
  const captionEl = overlay.querySelector<HTMLElement>(".photo-viewer__caption")!;
  const timestampEl = overlay.querySelector<HTMLElement>(".photo-viewer__timestamp")!;
  const deleteBtn = overlay.querySelector<HTMLButtonElement>(".photo-viewer__delete");
  const galleryLinkEl = overlay.querySelector<HTMLAnchorElement>(".photo-viewer__gallery-link");
  const prevBtn = overlay.querySelector<HTMLButtonElement>(".photo-viewer__nav--prev");
  const nextBtn = overlay.querySelector<HTMLButtonElement>(".photo-viewer__nav--next");
  const reactionChipsEl = !lightweight ? overlay.querySelector<HTMLElement>(".photo-viewer__reaction-chips") : null;
  const reactionPickerEl = !lightweight ? overlay.querySelector<HTMLElement>(".photo-viewer__reaction-picker") : null;
  const reactBtnEl = !lightweight ? overlay.querySelector<HTMLElement>(".photo-viewer__react-btn") : null;
  const commentListEl = !lightweight ? overlay.querySelector<HTMLElement>(".photo-viewer__comment-list") : null;
  const commentFormEl = !lightweight ? overlay.querySelector<HTMLFormElement>(".photo-viewer__comment-form") : null;
  const commentInputEl = !lightweight ? overlay.querySelector<HTMLTextAreaElement>(".photo-viewer__comment-input") : null;

  void reactBtnEl;

  function updateUrl(photoId: string | undefined) {
  ...
  function setReactionPickerOpen(open: boolean) {
  ...
  overlay.addEventListener("click", (e) => {
    ...
    if (e.target === overlay || target?.closest(".photo-viewer__backdrop")) {
      api.close();
      return;
    }
    if (target?.closest(".photo-viewer__close")) {
      api.close();
      return;
    }
    if (lightweight) {
      if (target?.closest(".photo-viewer__gallery-link")) {
        api.close();
      }
      return;
    }
    if (target?.closest(".photo-viewer__nav--prev")) {
    ...
    if (target?.closest(".photo-viewer__nav--next")) {
    ...
    const deleteBtnClicked = target?.closest<HTMLElement>(".photo-viewer__delete");
    if (deleteBtnClicked) {
      const photoId = deleteBtnClicked.dataset.photoId;
    ...
    const reactionOptionBtn = target?.closest<HTMLElement>(".photo-viewer__reaction-option");
    if (reactionOptionBtn) {
      const reactionId = reactionOptionBtn.dataset.reactionId;
      const photo = viewerState.getCurrentPhoto();
      if (reactionId && photo?.id && onReactFn) {
        setReactionPickerOpen(false);
        onReactFn(photo.id, reactionId);
      }
      return;
    }
    const reactionChipBtn = target?.closest<HTMLElement>(".photo-viewer__reaction-chip");
    if (reactionChipBtn) {
  ...
  overlay.addEventListener("submit", (e) => {
    const form = (e.target as Element | null)?.closest(".photo-viewer__comment-form");
    if (!form) return;
    e.preventDefault();
    const text = commentInputEl?.value?.trim() || "";
    const photo = viewerState.getCurrentPhoto();
    if (text && photo?.id && onCommentFn) {
  ...
  const api: PhotoViewer = {
    setPhotos(newPhotos: GalleryPhoto[], opts: SetPhotosOptions = {}) {
...
export function initPageGalleryViewer({ doc = globalThis.document, galleryPageHref = "../gallery/index.html", apiClient = null }: { doc?: Document; galleryPageHref?: string; apiClient?: PlatformApiClient | null } = {}): PhotoViewer {
  ...
  const viewerActions = hasApi
    ? createPageGalleryViewerActions({ apiClient: apiClient!, fullViewer })
    : null;
