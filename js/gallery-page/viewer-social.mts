import type { PhotoSocialState, ViewerComment } from "./viewer-state.mjs";
...
export function escapeViewerHtml(str: unknown): string {
  return String(str || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
export function formatViewerDate(ts: unknown): string {
  if (!ts) return "";
  try {
...
export function buildReactionPickerHtml(): string {
...
export function buildReactionChipsHtml(state: PhotoSocialState | null | undefined = {}): string {
  const totals = state?.reactionTotals || {};
  const viewerReaction = state?.viewerReaction || "";
  const activeIds = PHOTO_REACTION_IDS.filter((id) => ((totals[id] || 0) > 0) || id === viewerReaction);
