import { buildCommentListHtml, buildReactionChipsHtml, } from "./viewer-social.mjs";
...
function buildReactionPickerHtml() {
    return PHOTO_REACTION_IDS.map(id => `<button class="photo-viewer__reaction-option" type="button" data-reaction-id="${id}" title="${id}">${PHOTO_REACTION_GLYPHS[id]}</button>`).join("");
}
export function createPhotoViewer({ doc = globalThis.document, lightweight = false } = {}) {
    let onDeleteFn = null;
    let onReactFn = null;
    let onCommentFn = null;
    let onDeleteCommentFn = null;
  ...
  `;
    ...
    function updateUrl(photoId) {
        if (lightweight || !globalThis.history?.replaceState)
            return;
    ...
    function paint() {
        ...
        if (lightweight) {
            if (galleryLinkEl)
                galleryLinkEl.href = galleryLinkHref || "#";
        }
        else {
            if (deleteBtn) {
    ...
    overlay.addEventListener("click", (e) => {
        ...
        const deleteBtnClicked = target?.closest(".photo-viewer__delete");
        if (deleteBtnClicked) {
            const photoId = deleteBtnClicked.dataset.photoId;
            if (photoId && onDeleteFn)
                onDeleteFn(photoId);
            return;
        }
        const deleteCommentBtn = target?.closest(".photo-viewer__comment-delete");
        if (deleteCommentBtn) {
            const commentId = deleteCommentBtn.dataset.deleteCommentId;
            const photo = viewerState.getCurrentPhoto();
            if (commentId && photo?.id && onDeleteCommentFn)
                onDeleteCommentFn(photo.id, commentId);
            return;
        }
        ...
        const reactionOptionBtn = target?.closest(".photo-viewer__reaction-option");
        ...
        const reactionChipBtn = target?.closest(".photo-viewer__reaction-chip");
        if (reactionChipBtn) {
    ...
    doc.addEventListener("keydown", (e) => {
        if (overlay.classList.contains("photo-viewer--hidden"))
            return;
        if (e.key === "Escape") {
