...
import { type Size, type ThreeNamespace, box, canvasPlane, cylinder, glow, lighten, sphere, standard, withAlpha } from "./arcade-room-decor-primitives.mjs";

type PropBuild = (THREE: ThreeNamespace, group: any, size: Size, color: string) => void;

/** A few cheerful colours for the small things on shelves: game boxes, prizes, gumballs. */
const PARTY = ["#ff5caf", "#ffd33d", "#7dff4d", "#22e5ff", "#a35bff", "#ff7a1a", "#f4f8ff", "#ff3b3b"] as const;

function chrome(THREE: ThreeNamespace): any {
  return standard(THREE, "#d8dde4", 0.2, 0.9);
}

function dark(THREE: ThreeNamespace): any {
  return standard(THREE, "#1a1d24", 0.5, 0.35);
}

function wood(THREE: ThreeNamespace, color = "#5a3a22"): any {
  return standard(THREE, color, 0.55, 0.08);
}

function glass(THREE: ThreeNamespace, tint = "#bfe6ff"): any {
...
function legs(THREE: ThreeNamespace, group: any, w: number, d: number, height: number, bottom: number, material: any, thickness = 0.05, inset = 0.08): void {
...
function fitText(context: CanvasRenderingContext2D, text: string, cw: number, ch: number, fill: string, weight = "900", family = "'Arial Black', Impact, sans-serif"): void {
...
const FURNITURE: Record<string, PropBuild> = {
  ...
  "sofa": (THREE, group, size, color) => {
    ...
    for (const sign of [-1, 1]) box(THREE, group, [0.15, h * 0.7, d], [sign * (w / 2 - 0.075), bottom + h * 0.4, 0], fabric);
  ...
  "bookcase": (THREE, group, size, color) => {
    ...
    for (const sign of [-1, 1]) box(THREE, group, [0.03, h, d], [sign * (w / 2 - 0.015), 0, 0], timber);
    const shelves = 5;
    let seed = 3;
  ...
  "lockers": (THREE, group, size, color) => {
    ...
    for (let index = 0; index < doors; index += 1) {
      ...
      for (let slat = 0; slat < 5; slat += 1) box(THREE, group, [doorW * 0.5, 0.012, 0.01], [x, bottom + h * 0.82 - slat * 0.035, d / 2 + 0.012], dark(THREE), false);
...
const PROPS: Record<string, PropBuild> = {
  ...
  "change-machine": (THREE, group, size, color) => {
    ...
    for (let index = 0; index < 3; index += 1) box(THREE, group, [w * 0.6, 0.012, 0.01], [0, bottom + h * 0.36 - index * 0.03, d / 2 + 0.008], dark(THREE), false);
  },
  ...
  "photo-booth": (THREE, group, size, color) => {
    ...
    const curtain = canvasPlane(THREE, group, w * 0.84, h * 0.76, [256, 512], (context, cw, ch) => {
      ...
      for (let x = 0; x < cw; x += 32) context.fillRect(x, 0, 12, ch);
  ...
  "dance-pad": (THREE, group, size, color) => {
    ...
    const pad = canvasPlane(THREE, group, w, d, [512, 512], (context, cw, ch) => {
      ...
      context.fillStyle = color;
      const arrow = (cx: number, cy: number, angle: number): void => {
        context.save();
  ...
  "crates": (THREE, group, size, color) => {
    ...
    const bottom = -h / 2;
    const timber = wood(THREE, color);
    const crate = (cw: number, ch: number, position: readonly [number, number, number], turn: number): void => {
  ...
  "barrel": (THREE, group, size, color) => {
    ...
    for (const y of [0.2, 0.5, 0.8]) cylinder(THREE, group, w * 0.5, w * 0.5, 0.04, [0, bottom + h * y, 0], chrome(THREE), 20).castShadow = false;
  ...
  "lava-lamp": (THREE, group, size, color) => {
    ...
    for (const [y, r] of [[0.3, 0.12], [0.55, 0.09], [0.72, 0.07]] as const) {
  ...
  "trophy-case": (THREE, group, size, color) => {
    ...
    for (const sign of [-1, 1]) box(THREE, group, [0.03, h * 0.82, d], [sign * (w / 2 - 0.015), bottom + h * 0.53, 0], frame);
  ...
  "balloons": (THREE, group, size, color) => {
    ...
    const shades = [color, lighten(color, 0.35), PARTY[1], PARTY[3], color, PARTY[4]] as const;
    const radius = w * 0.2;
    for (let index = 0; index < 6; index += 1) {
...
const WALL: Record<string, PropBuild> = {
  "dartboard": (THREE, group, size, color) => {
    ...
    canvasPlane(THREE, group, w * 0.92, w * 0.92, [512, 512], (context, cw, ch) => {
      ...
      for (const [inner, outer] of [[0.94, 1], [0.58, 0.64]] as const) {
        for (let index = 0; index < 20; index += 1) {
    ...
    for (const [x, y] of [[0.02, 0.1], [-0.06, 0.03], [0.05, -0.05]] as const) {
  ...
  "cork-board": (THREE, group, size, color) => {
    ...
    canvasPlane(THREE, group, w - 0.06, h - 0.06, [512, 360], (context, cw, ch) => {
      ...
      const notes = [["#fff7a8", 40, 40, -0.08], ["#bfe6ff", 210, 60, 0.05], ["#ffd6ec", 360, 150, -0.03], ["#dfffd0", 120, 200, 0.07]] as const;
      for (const [paper, x, y, tilt] of notes) {
        ...
        for (let line = 0; line < 4; line += 1) context.fillRect(-44, -28 + line * 18, 70 + (line % 2) * 16, 4);
        context.fillStyle = "#ff3b3b";
        context.beginPath();
  ...
  "ac-unit": (THREE, group, size, color) => {
    ...
    for (let index = 0; index < 4; index += 1) box(THREE, group, [w * 0.9, 0.012, 0.01], [0, -h * 0.3 + index * 0.03, d / 2 + 0.005], dark(THREE), false);
      ...
      while (x < cw) {
        ...
        for (let wy = ch - bh + 8; wy < ch - 6; wy += 12) {
          for (let wx = x + 4; wx < x + bw - 6; wx += 10) {
            seed = (seed * 1664525 + 1013904223) >>> 0;
            if (seed % 3 === 0) context.fillRect(wx, wy, 5, 6);
          }
        }
        x += bw + 3;
      }
  ...
  "trophy-shelf": (THREE, group, size, color) => {
    ...
    for (const x of [-w / 2 + 0.06, w / 2 - 0.06]) box(THREE, group, [0.03, h * 0.35, d * 0.7], [x, -h * 0.47, -d * 0.15], dark(THREE));
    const gold = standard(THREE, color, 0.25, 0.9);
...
const CEILING: Record<string, PropBuild> = {
  ...
  "projector": (THREE, group, size, color) => {
    ...
    for (let index = 0; index < 4; index += 1) box(THREE, group, [0.01, h * 0.28, d * 0.6], [-w * 0.45 + index * 0.03, top - h * 0.6, 0], dark(THREE), false);
...
const CEILING_LIGHTS: Record<string, PropBuild> = {
  ...
  "string": (THREE, group, size, color) => {
    ...
    for (const sign of [-1, 1]) cylinder(THREE, group, 0.012, 0.012, 0.03, [sign * (w / 2 - 0.05), top - 0.015, 0], chrome(THREE), 8).castShadow = false;
  },
 ...
 */
export function buildMarquee(THREE: ThreeNamespace, group: any, size: Size, color: string, text: string): void {
  const { width: w, height: h, depth: d } = size;
  ...
  for (let index = 0; index < bulbs; index += 1) {
    ...
    for (const sign of [-1, 1]) sphere(THREE, group, h * 0.035, [x, sign * (h / 2 - h * 0.06), d / 2], glow(THREE, color, 2.2)).castShadow = false;
  }
  const boardW = w - d * 2;
  const boardH = h * 0.76;
 ...
 */
export function buildHangingSign(THREE: ThreeNamespace, group: any, size: Size, color: string, text: string): void {
  const { width: w, height: h, depth: d } = size;
  const top = h / 2;
  const boardH = h * 0.55;
...
export function extraPropBuilder(prop: string): PropBuild | undefined {
  return FURNITURE[prop] ?? PROPS[prop];
}

export function extraWallBuilder(prop: string): PropBuild | undefined {
  return WALL[prop];
}

export function ceilingPropBuilder(prop: string): PropBuild | undefined {
  return CEILING[prop];
}

export function ceilingLightBuilder(style: string): PropBuild | undefined {
  return CEILING_LIGHTS[style];
}

/** Every prop name these tables draw, so a test can check the catalog never names one they do not. */
export const EXTRA_PROP_NAMES: readonly string[] = Object.freeze([...Object.keys(FURNITURE), ...Object.keys(PROPS)]);
export const EXTRA_WALL_NAMES: readonly string[] = Object.freeze(Object.keys(WALL));
export const CEILING_PROP_NAMES: readonly string[] = Object.freeze(Object.keys(CEILING));
export const CEILING_LIGHT_STYLES: readonly string[] = Object.freeze(Object.keys(CEILING_LIGHTS));
