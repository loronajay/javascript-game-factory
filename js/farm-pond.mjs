...
export function pondProfile(r, depth) {
    if (r >= 1)
        return 0;
    if (r >= SHORE_RADIUS)
        return -SHORE_DEPTH * smooth((1 - r) / (1 - SHORE_RADIUS));
    if (r >= BED_RADIUS)
        return -SHORE_DEPTH - (depth - SHORE_DEPTH) * smooth((SHORE_RADIUS - r) / (SHORE_RADIUS - BED_RADIUS));
...
export function waterlineRadius() {
    ...
    for (let step = 0; step < 40; step += 1) {
        ...
        if (pondProfile(middle, SHORE_DEPTH) < WATER_LEVEL)
            low = middle;
        else
            high = middle;
    }
...
export function pondLocal(region, point) {
...
export function pondRadius(region, point) {
...
export function pondFloorAt(region, point) {
...
export function insidePondWater(region, point, margin = 0) {
    const local = pondLocal(region, point);
...
export function pondRegions(layout) {
...
function footingPoints(box) {
    ...
    for (const lx of [-halfW, 0, halfW]) {
        for (const lz of [-halfD, 0, halfD])
            points.push({ x: box.x + lx * cosine + lz * sine, z: box.z - lx * sine + lz * cosine });
    }
    return points;
}
...
export function aquaticFits(box, region) {
...
export function aquaticBase(box, region) {
...
export function homePond(regions, item) {
