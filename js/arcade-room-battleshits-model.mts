...
function canvasTexture(THREE: ThreeNamespace, width: number, height: number, draw: (context: CanvasRenderingContext2D) => void): any {
  ...
  if (!context) throw new Error("Canvas 2D is required to build Battleshits cabinet artwork");
  draw(context);
...
function addPoopTopper(THREE: ThreeNamespace, parent: any, palette: CabinetDefinition["palette"]): void {
  ...
  const plinth = new THREE.Mesh(
    new THREE.CylinderGeometry(0.2, 0.22, 0.045, 24),
    new THREE.MeshStandardMaterial({ color: palette.trim, roughness: 0.28, metalness: 0.68 }),
  );
...
export function createBattleshitsCabinet(THREE: ThreeNamespace, definition: CabinetDefinition): any {
  ...
  for (const side of [-1, 1]) {
    const sideArt = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ map: createSideArtTexture(THREE, side < 0 ? "left" : "right") }),
    );
