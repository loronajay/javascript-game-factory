...
export type NeonShape = "heart" | "star" | "bolt" | "joystick" | "ghost" | "circle";

/** Trace the shape's path into the context, filling the `w × h` box. Stroke it afterwards. */
export const NEON_SHAPES: Readonly<Record<NeonShape, (context: CanvasRenderingContext2D, w: number, h: number) => void>> = Object.freeze({
  heart: (context, w, h) => {
    context.beginPath();
...
export function neonStroke(context: CanvasRenderingContext2D, color: string, lineWidth: number): void {
...
export function drawNeonShape(context: CanvasRenderingContext2D, w: number, h: number, shape: string, color: string, tube = Math.max(4, Math.min(w, h) * 0.035)): void {
  const trace = NEON_SHAPES[shape as NeonShape] ?? NEON_SHAPES.circle;
...
export type NeonFont = "block" | "script";

function neonFontFamily(font: NeonFont): { style: string; family: string } {
  return font === "script"
...
export function drawNeonText(context: CanvasRenderingContext2D, w: number, h: number, text: string, font: NeonFont, color: string): void {
