...
import type { ChatEntry, RoomChat } from "./arcade-room-chat.mjs";

export type RoomChatViewOptions = Readonly<{
  chat: RoomChat;
  root: HTMLElement;
  log: HTMLElement;
  input: HTMLInputElement;
  /** Where keyboard focus goes back to when the box closes. */
  returnFocusTo: HTMLElement;
}>;

export type RoomChatView = Readonly<{
  /** True when the key belonged to the chat; the caller then leaves it alone. */
  handleKey: (event: KeyboardEvent) => boolean;
  /** Every frame or so: drops faded lines. Cheap when nothing changed. */
  tick: () => void;
}>;

export function createRoomChatView(options: RoomChatViewOptions): RoomChatView {
  const { chat, root, log, input, returnFocusTo } = options;
  const rows = new Map<number, HTMLElement>();
  let lastOpen: boolean | null = null;
  let lastVisibleKey = "";

  function rowFor(entry: ChatEntry): HTMLElement {
  ...
  function render(): void {
    ...
    if (appended || open !== lastOpen) log.scrollTop = log.scrollHeight;
    if (open !== lastOpen) {
      lastOpen = open;
      if (open) {
        input.value = "";
  ...
  input.addEventListener("blur", () => { if (chat.isOpen()) chat.close(); });
  render();
  ...
  function handleKey(event: KeyboardEvent): boolean {
    ...
    if (result !== "typing") event.preventDefault();
    return true;
  }
