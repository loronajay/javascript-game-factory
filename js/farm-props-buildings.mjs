    ...
    return Object.freeze({
        ...
        update(dt) {
            ...
            for (const hinge of hinges)
                hinge.node.rotation.y = hinge.node.userData.restY + hinge.angle * eased;
            return swing !== target;
        },
    });
}
...
function stone(THREE) {
    return farmMaterial(THREE, "fieldstone");
}
function glassMaterial(THREE, opacity = 0.42) {
...
export function tagFixture(mesh, entry) {
...
export function drawFixtureBox(THREE, group, entry, material, options = {}) {
...
export function drawPlatform(THREE, group, entry, material) {
 ...
 */
export function drawFixtureDoor(THREE, group, entry, material, draw) {
 ...
 */
export function drawLadder(THREE, group, entry, material) {
    ...
    for (const x of [-railX, railX])
        tbox(THREE, ladder, [0.06, visualTop - entry.bottom, 0.06], [x, (entry.bottom + visualTop) / 2, 0], material, false);
    for (let y = entry.bottom + 0.32; y < entry.top + 0.2; y += 0.32)
        cylinder(THREE, ladder, 0.022, 0.022, entry.width - 0.06, [0, y, 0], material, 8).rotation.z = Math.PI / 2;
    group.add(ladder);
...
export function drawSeat(THREE, group, entry, material) {
    ...
    for (let index = 0; index < slats; index += 1)
        tbox(THREE, seat, [entry.width, 0.05, slatDepth - 0.015], [0, entry.top - 0.025, -entry.depth / 2 + slatDepth * (index + 0.5)], material);
    ...
    for (const x of [-legX, legX])
        for (const z of [-legZ, legZ])
            tbox(THREE, seat, [0.06, entry.top - 0.05, 0.06], [x, (entry.top - 0.05) / 2, z], material, false);
...
function drawStool(THREE, group, entry, material) {
...
function drawWorkbench(THREE, group, entry, material) {
    ...
    for (const z of [-entry.depth / 2 + 0.06, entry.depth / 2 - 0.06])
        tbox(THREE, bench, [entry.width - 0.1, legHeight, 0.08], [0, legHeight / 2, z], material, false);
...
function drawRailing(THREE, group, entry, material) {
 ...
 */
function drawShell(THREE, group, definition, materials, options = {}) {
    const shell = definition.shell;
    const meshes = [];
    ...
    if (shell.door && options.lintel !== false) {
        ...
        if (lintelHeight > 0.02)
            tbox(THREE, group, [door.width, lintelHeight, shell.wallThickness], [at.x, door.height + lintelHeight / 2, at.z], materials.wall);
    ...
    if (options.cornerBoards && shell.kind === "walls" && materials.trim) {
        ...
        for (const x of [-width / 2, width / 2])
            for (const z of [-depth / 2, depth / 2])
                tbox(THREE, group, [0.16, shell.wallHeight, 0.16], [x, shell.wallHeight / 2, z], materials.trim, false);
    }
    return meshes;
}
/**
 ...
 */
function foundation(THREE, group, definition, height = 0.35, proud = 0.08) {
    const shell = definition.shell;
 ...
 */
function wallBands(THREE, group, definition, material, band) {
    const shell = definition.shell;
    ...
    for (const wall of walls) {
        ...
        if (wall.x * normal.x + wall.z * normal.z < 0)
...
function ironmongery(THREE, leaf, side, leafWidth, height, z) {
    ...
    for (const y of [height * 0.18, height * 0.82]) {
        ...
        for (let index = 0; index < 3; index += 1)
            sphere(THREE, leaf, 0.012, [-side * (0.08 + index * leafWidth * 0.18), y, z + 0.01], metal);
    }
...
function drawLeaf(THREE, leaf, style, side, leafWidth, height, materials) {
    ...
    if (style === "barn") {
        ...
        for (const [y, h] of [[0.72, 0.36], [0.28, 0.3]]) {
 ...
 */
function createDoorLeaves(THREE, group, definition, style, materials) {
    const shell = definition.shell;
    const door = shell.door;
    const at = doorLocal(definition);
    const sides = door.leaves === 2 ? [-1, 1] : [-1];
 ...
 */
function roofSlope(THREE, group, along, run, rise, eaveY, side, materials, thickness) {
 ...
 */
function gableRoof(THREE, group, definition, rise, overhang, materials, options = {}) {
    const { width, depth } = definition.footprint;
    const shell = definition.shell;
    ...
    for (const side of [-1, 1])
        roofSlope(THREE, frame, alongSpan + endOverhang * 2, run, rise, shell.wallHeight, side, materials, thickness);
    // Ridge cap.
 ...
 */
function gambrelRoof(THREE, group, definition, rise, overhang, materials, ridge = "z") {
    const { width, depth } = definition.footprint;
    const shell = definition.shell;
...
function hipRoof(THREE, group, definition, rise, overhang, materials) {
    const { width, depth } = definition.footprint;
    ...
    const c = [[-hw, y0, hd], [hw, y0, hd], [hw, y0, -hd], [-hw, y0, -hd]];
    const r0 = [-ridge, y1, 0];
    const r1 = [ridge, y1, 0];
    const triangles = [
    ...
    for (const [length, x, z, rot] of [[width + overhang * 2, 0, hd, 0], [width + overhang * 2, 0, -hd, 0], [depth + overhang * 2, hw, 0, Math.PI / 2], [depth + overhang * 2, -hw, 0, Math.PI / 2]]) {
        const board = tbox(THREE, group, [length, 0.16, 0.05], [x, wallHeight - 0.08, z], materials.fascia, false);
        board.rotation.y = rot;
    }
    for (let x = -width / 2 + 0.3; x < width / 2; x += 0.6)
        for (const z of [hd - 0.3, -hd + 0.3])
            tbox(THREE, group, [0.08, 0.1, 0.6], [x, wallHeight - 0.06, z], materials.fascia, false);
    for (let z = -depth / 2 + 0.3; z < depth / 2; z += 0.6)
        for (const x of [hw - 0.3, -hw + 0.3])
            tbox(THREE, group, [0.6, 0.1, 0.08], [x, wallHeight - 0.06, z], materials.fascia, false);
    // Ridge tiles.
    if (ridge > 0.1)
        tbox(THREE, group, [ridge * 2 + 0.3, 0.12, 0.3], [0, y1 + 0.04, 0], materials.roof, false);
}
/** A hanging lantern: a warm point light in a little glass-and-iron body on a chain, so no interior is a cave in daylight. */
function roomLight(THREE, group, position, intensity, distance, anchorY) {
...
function wallLantern(THREE, group, position, rotationY) {
 ...
 * turned by `rotationY`.
 */
function paneWindow(THREE, group, size, position, rotationY, frame, options = {}) {
    ...
    for (let index = 1; index < columns; index += 1)
        box(THREE, win, [0.035, h, 0.04], [-w / 2 + (w / columns) * index, 0, 0.0], frame, false);
    for (let index = 1; index < rows; index += 1)
        box(THREE, win, [w, 0.035, 0.04], [0, -h / 2 + (h / rows) * index, 0.0], frame, false);
    if (options.sill !== false)
        tbox(THREE, win, [w + f * 2 + 0.1, 0.06, 0.16], [0, -h / 2 - f - 0.03, 0.06], frame, false);
    if (options.shutters) {
        for (const side of [-1, 1]) {
...
function cupola(THREE, group, position, size, materials) {
    ...
    for (const x of [-size / 2, size / 2])
        for (const z of [-size / 2, size / 2])
            box(THREE, tower, [0.06, h, 0.06], [x, h / 2, z], materials.trim, false);
...
function weathervane(THREE, group, position) {
    const metal = iron(THREE);
...
function chimney(THREE, group, position, height, width = 0.6) {
...
function interiorStuds(THREE, group, definition, material, spacing = 0.6) {
    const { width, depth } = definition.footprint;
    const shell = definition.shell;
    ...
    for (let z = -halfD + t + spacing / 2; z < halfD - t; z += spacing) {
        for (const x of [-halfW + t + 0.03, halfW - t - 0.03])
            tbox(THREE, group, [0.06, shell.wallHeight - 0.1, 0.08], [x, shell.wallHeight / 2, z], material, false);
    }
    // A top plate all round.
    tbox(THREE, group, [width - t * 2, 0.1, 0.1], [0, shell.wallHeight - 0.05, -halfD + t + 0.05], material, false);
    for (const x of [-halfW + t + 0.05, halfW - t - 0.05])
        tbox(THREE, group, [0.1, 0.1, depth - t * 2], [x, shell.wallHeight - 0.05, 0], material, false);
}
// ---------------------------------------------------------------------------
// The buildings.
/**
 ...
 */
export function createBarn(THREE, definition) {
    ...
    for (const x of [-0.6, 0.6])
        tbox(THREE, group, [0.12, 1.45, 0.14], [x, loftDoorY, halfD + 0.04], trim, false);
    ...
    for (const x of [-halfW + 1.1, halfW - 1.1])
        paneWindow(THREE, group, [0.7, 0.7], [x, 2.2, halfD + 0.02], 0, trim, { muntins: [2, 2] });
    for (const x of [-halfW + 1.4, halfW - 1.4])
        wallLantern(THREE, group, [x, shell.door.height + 0.3, halfD + 0.02], 0);
    ...
    for (const y of [loftHay.bottom + 0.27, loftHay.bottom + 0.55])
        for (const dx of [-0.4, 0.4])
            tbox(THREE, group, [0.05, 0.03, loftHay.depth - 0.06], [loftHay.x + dx, y, loftHay.z], standard(THREE, "#a07a28", 1, 0), false);
    interiorStuds(THREE, group, definition, wood);
    const barnCeilingAt = (x) => gambrelRoofHeightAt({ wallHeight: shell.wallHeight, rise: 2.6, halfSpan: halfW, overhang: 0.45, across: x }) - 0.12;
 ...
 */
export function createStable(THREE, definition) {
    const group = new THREE.Group();
    const { width, depth } = definition.footprint;
    const shell = definition.shell;
    const t = shell.wallThickness;
    ...
    for (const z of [halfD + 0.01, -halfD - 0.01]) {
        for (let x = -halfW + 1; x < halfW - 0.5; x += 1) {
            if (z > 0 && Math.abs(x) < shell.door.width / 2 + 0.3)
                continue;
    ...
    for (const x of [halfW + 0.01, -halfW - 0.01]) {
        for (let z = -halfD + 1; z < halfD - 0.5; z += 1)
            tbox(THREE, group, [0.06, shell.wallHeight - 0.1, 0.12], [x, shell.wallHeight / 2, z], dark, false);
    }
    ...
    for (const entry of farmFixtures(definition)) {
        ...
        if (entry.name.startsWith("stall-front")) {
            ...
            for (let x = entry.x - entry.width / 2 + 0.12; x < entry.x + entry.width / 2; x += 0.12)
                cylinder(THREE, group, 0.012, 0.012, 0.7, [x, entry.top + 0.35, entry.z], iron(THREE), 6, false);
 ...
 * hearth.
 */
export function createCottage(THREE, definition) {
    const group = new THREE.Group();
    const { width, depth } = definition.footprint;
    const shell = definition.shell;
    const t = shell.wallThickness;
    ...
    for (const x of [-halfW + 1.3, halfW - 1.3])
        paneWindow(THREE, group, [0.8, 0.9], [x, 1.75, halfD + 0.02], 0, beam, { shutters: shutter, flowerBox: true, muntins: [2, 3] });
    ...
    for (const [x, z] of [[-0.5, -0.3], [0.5, 0.3], [-0.5, 0.3], [0.5, -0.3]])
        tbox(THREE, group, [0.06, table.top - 0.06, 0.06], [table.x + x, (table.top - 0.06) / 2, table.z + z], beam, false);
    ...
    for (const [dx, dz] of [[-0.15, 0.1], [0.12, 0.14], [0, 0.05]])
        cylinder(THREE, group, 0.05, 0.05, 0.4, [hearth.x + dx, 0.2, hearth.z + dz], farmMaterial(THREE, "bark", { metresPerTile: 0.3 }), 7).rotation.z = Math.PI / 2;
    ...
    for (let x = -halfW + t + 0.5; x < halfW - t; x += 0.7)
        tbox(THREE, group, [0.12, 0.14, depth - t * 2], [x, shell.wallHeight - 0.07, 0], beam, false);
...
export function createGreenhouse(THREE, definition) {
    const group = new THREE.Group();
    const { width, depth } = definition.footprint;
    const shell = definition.shell;
    const t = shell.wallThickness;
    ...
    for (let x = -halfW + 1; x < halfW - 0.5; x += 1) {
        ...
        if (Math.abs(x) > shell.door.width / 2 + 0.2)
            tbox(THREE, group, [0.07, shell.wallHeight, t + 0.05], [x, shell.wallHeight / 2, halfD - t / 2], frame, false);
    }
    for (let z = -halfD + 0.9; z < halfD - 0.5; z += 0.9) {
        for (const x of [-halfW + t / 2, halfW - t / 2])
            tbox(THREE, group, [t + 0.05, shell.wallHeight, 0.07], [x, shell.wallHeight / 2, z], frame, false);
    }
    // A horizontal rail halfway up every glass wall.
    tbox(THREE, group, [width + 0.02, 0.06, t + 0.04], [0, 1.5, -halfD + t / 2], frame, false);
    for (const x of [-halfW + t / 2, halfW - t / 2])
        tbox(THREE, group, [t + 0.04, 0.06, depth], [x, 1.5, 0], frame, false);
    ...
    for (const x of [-halfW - 0.06, halfW + 0.06])
        cylinder(THREE, group, 0.05, 0.05, depth + 0.2, [x, shell.wallHeight + 0.02, 0], gutter, 8, false).rotation.x = Math.PI / 2;
    ...
    for (const x of [-0.4, 0.4])
        tbox(THREE, vent, [0.06, 0.06, 0.7], [x, 0, 0.35], frame, false);
    roofFrame.add(vent);
    ...
    for (let z = east.z - 0.5; z <= east.z + 0.5; z += 0.5)
        tbox(THREE, group, [0.4, 0.06, 0.3], [east.x, 0.15, z], standard(THREE, "#2b2b2b", 0.8, 0), false);
...
export function createShed(THREE, definition) {
    const group = new THREE.Group();
    const { width, depth } = definition.footprint;
    const shell = definition.shell;
    const t = shell.wallThickness;
    ...
    for (const y of [shelves.bottom + 0.02, 1.3, shelves.top - 0.02])
        tagFixture(tbox(THREE, group, [shelves.width, 0.04, shelves.depth], [shelves.x, y, shelves.z], shelf), shelves);
    for (const x of [shelves.x - shelves.width / 2 + 0.03, shelves.x + shelves.width / 2 - 0.03])
        tbox(THREE, group, [0.05, shelves.top - shelves.bottom, shelves.depth], [x, (shelves.top + shelves.bottom) / 2, shelves.z], shelf, false);
    ...
    let n = 0;
    for (const y of [shelves.bottom + 0.06, 1.34])
        for (let x = shelves.x - shelves.width / 2 + 0.25; x < shelves.x + shelves.width / 2 - 0.15; x += 0.32) {
            if (n % 5 === 4) {
    ...
    for (let index = 0; index < 6; index += 1)
        box(THREE, group, [0.05, 0.08, 0.02], [tools.x + 0.1, 1.78, tools.z + 0.15 + index * 0.06], metal, false);
    ...
    for (const dz of [-0.05, 0, 0.05])
        box(THREE, group, [0.015, 0.25, 0.015], [tools.x + 0.07, 1.6, tools.z + dz], metal, false);
...
export function createCoop(THREE, definition) {
    const group = new THREE.Group();
    const { width, depth } = definition.footprint;
    const shell = definition.shell;
    const t = shell.wallThickness;
    ...
    for (let index = 0; index < 5; index += 1)
        tbox(THREE, group, [0.04, 0.03, 0.4], [halfW + 0.15 + index * 0.17, 0.22 + index * 0.05, 0.4], dark, false);
    ...
    for (let x = -0.22; x <= 0.22; x += 0.055)
        box(THREE, mesh, [0.006, 0.4, 0.006], [x, 0, 0.01], standard(THREE, "#9aa0a6", 0.4, 0.6), false);
    for (let y = -0.17; y <= 0.17; y += 0.055)
        box(THREE, mesh, [0.5, 0.006, 0.006], [0, y, 0.01], standard(THREE, "#9aa0a6", 0.4, 0.6), false);
    ...
    for (let index = 0; index < boxes; index += 1) {
        ...
        if (index !== 1)
            sphere(THREE, group, 0.05, [x + 0.08, nests.bottom + 0.17, nests.z + 0.05], standard(THREE, "#e8d3b0", 0.7, 0)).scale.set(1, 1.25, 1);
    }
    ...
    for (const x of [roost.x - roost.width / 2 + 0.05, roost.x + roost.width / 2 - 0.05])
        tbox(THREE, group, [0.04, roost.bottom, 0.04], [x, roost.bottom / 2, roost.z], dark, false);
...
function roundBody(THREE, group, definition, material, trim) {
...
export function createSilo(THREE, definition) {
    const group = new THREE.Group();
    const shell = definition.shell;
    ...
    for (let y = 0.9; y < shell.wallHeight; y += 1.1)
        wallBands(THREE, group, definition, band, { y, height: 0.1, thickness: 0.08 });
    ...
    for (const entry of farmFixtures(definition)) {
        if (entry.name.startsWith("catwalk"))
            drawPlatform(THREE, group, entry, grate);
        if (entry.name.startsWith("rail"))
            drawRailing(THREE, group, entry, rail);
    }
...
export function createWindmill(THREE, definition) {
    const group = new THREE.Group();
    const shell = definition.shell;
    const face = roundFace(shell, definition.footprint, 0);
    ...
    for (const y of [shell.wallHeight - 0.08, 2.9, 1.15])
        wallBands(THREE, group, definition, timberMaterial, { y, height: 0.14, thickness: 0.1 });
    ...
    for (let index = 0; index < 4; index += 1) {
        ...
        for (let y = sailLength * 0.25; y < sailLength * 0.98; y += 0.28)
            tbox(THREE, vane, [0.66, 0.04, 0.04], [0.33, y, 0], timberMaterial, false);
    ...
    for (const [dx, dz] of [[0.1, 0.25], [-0.15, -0.2], [0.15, -0.15]]) {
...
export function createGazebo(THREE, definition) {
    const group = new THREE.Group();
    const { width, depth } = definition.footprint;
    const shell = definition.shell;
    ...
    for (const [w, x, z, rot] of [[width, 0, halfD - 0.02, 0], [width, 0, -halfD + 0.02, 0], [depth, halfW - 0.02, 0, Math.PI / 2], [depth, -halfW + 0.02, 0, Math.PI / 2]]) {
    ...
    for (const wall of shellWalls(definition, true)) {
        ...
        for (const [dx, dz] of [[-Math.sign(wall.x), 0], [0, -Math.sign(wall.z)]]) {
    ...
    for (const name of ["rail-back", "rail-west", "rail-east"])
        drawRailing(THREE, group, fixtureNamed(definition, name), white);
    // A frieze of spindles under the eaves all round.
    for (const [len, x, z, rot] of [[width, 0, halfD - 0.09, 0], [width, 0, -halfD + 0.09, 0], [depth, halfW - 0.09, 0, Math.PI / 2], [depth, -halfW + 0.09, 0, Math.PI / 2]]) {
        ...
        for (let s = -len / 2 + 0.3; s < len / 2 - 0.2; s += 0.16)
            cylinder(THREE, frieze, 0.014, 0.014, 0.3, [s, -0.02, 0], white, 6, false);
    ...
    for (const name of ["bench-back", "bench-west", "bench-east"])
        drawSeat(THREE, group, fixtureNamed(definition, name), seat);
