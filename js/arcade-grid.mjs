import { applyCabinetOverrides, loadArcadeCatalog, } from "./arcade-catalog.mjs";
import { buildCatalogFacets, filterArcadeCatalog, } from "./arcade-catalog-query.mjs";
...
const query = { search: "", category: "", dimension: "", mode: "", sort: "factory" };
...
function statusBadge(status) {
    ...
    if (value.includes("release candidate"))
        return "RELEASE CANDIDATE";
    if (value.includes("prototype"))
        return "PLAYABLE PROTOTYPE";
...
function canPlayHoverVideo() {
    return window.matchMedia("(hover: hover) and (pointer: fine)").matches
        && !window.matchMedia("(prefers-reduced-motion: reduce)").matches;
}
function attachHoverVideo(card, frame, game) {
    if (!game.previewVideo || !canPlayHoverVideo())
        return;
...
function setSelectedIndex(index) {
    ...
    if (showGamepadSelection)
        cards[selectedIndex]?.scrollIntoView({ block: "nearest" });
}
function createFilterChip(label, value, kind) {
...
function renderFilters() {
    ...
    categoryFilters.replaceChildren(createFilterChip("All", "", "category"), ...facets.categories.map((facet) => createFilterChip(facet.value, facet.value, "category")));
    dimensionFilters.replaceChildren(createFilterChip("All formats", "", "dimension"), ...facets.dimensions.map((facet) => createFilterChip(facet.value.toUpperCase(), facet.value, "dimension")));
}
function syncFilterState() {
    for (const button of Array.from(categoryFilters.querySelectorAll("button"))) {
        const active = button.dataset.value === (query.category || "");
        button.classList.toggle("is-active", active);
        button.setAttribute("aria-pressed", String(active));
    }
    for (const button of Array.from(dimensionFilters.querySelectorAll("button"))) {
