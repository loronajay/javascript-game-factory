...
import { standard, type ThreeNamespace } from "./arcade-room-decor-primitives.mjs";
import { FARM_BOUNDS, type FarmLayout } from "./farm-layout.mjs";
...
export type FarmScenery = Readonly<{
  /** Re-lay the ground cover around what stands on the field now. */
  rescatter: (layout: FarmLayout) => void;
...
function createClouds(THREE: ThreeNamespace, scene: any, random: () => number): any[] {
...
function createSun(THREE: ThreeNamespace, scene: any, direction: Readonly<{ x: number; y: number; z: number }>): void {
...
function createTuftGeometry(THREE: ThreeNamespace): any {
...
type Sprinkle = Readonly<{ x: number; z: number; scale: number; lean: number; turn: number; tone: number }>;

function sprinkle(count: number, random: () => number): Sprinkle[] {
...
export function createFarmScenery(THREE: ThreeNamespace, scene: any, options: Readonly<{ sunDirection: Readonly<{ x: number; y: number; z: number }> }>): FarmScenery {
  ...
  function lay(mesh: any, spots: readonly Sprinkle[], blocked: (spot: Sprinkle) => boolean, tones: readonly any[], size: (spot: Sprinkle) => readonly [number, number, number]): void {
    let count = 0;
  ...
  function rescatter(layout: FarmLayout): void {
    const exclusions = groundCoverExclusions(layout);
    const blocked = (spot: Sprinkle): boolean => exclusions.some((box) => obstacleBlocks(spot, box, 0.15));
