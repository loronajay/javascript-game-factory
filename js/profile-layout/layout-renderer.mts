...
export interface RenderLayoutOptions {
  ...
  previewModels?: any;
  childEditPanelId?: string | null;
  selectedChildId?: string | null;
}

// Renders layout panel tiles into a container element.
// Preserves any extra classes already on the container (e.g. --overlay).
export function renderLayoutGrid(container: HTMLElement | null, layout: any, options: RenderLayoutOptions = {}): void {
  if (!container) return;
  ...
  const enabledPanels = panels.filter((p: any) => p.enabled !== false);
  const compositionCategories = new Set(
    elements
      .filter((element: any) => element?.enabled !== false && element.category && element.category !== "custom")
      .map((element: any) => element.category),
  );
  const renderedPanels = enabledPanels
    .filter((panel: any) => !compositionCategories.has(panel.id))
  ...
  for (const panel of renderedPanels) {
    ...
    if (editMode && onSelect) {
      ...
      tile.addEventListener("click", (e) => {
        ...
        if (!t?.closest("[data-drag-handle],[data-resize-handle],[data-child-id]")) onSelect(panel.id);
      });
      tile.addEventListener("keydown", (e) => {
        const t = e.target as Element | null;
        if (t?.closest("[data-child-id]")) return;
...
function comparePanelsByFreeformPosition(a: any, b: any): number {
  return (a.y - b.y) || (a.x - b.x);
}

function renderCompositionElements(container: HTMLElement, elements: any[], options: any = {}): void {
  if (!Array.isArray(elements) || elements.length === 0 || !options.previewModels?.hero) return;

  for (const element of elements.filter((item: any) => item?.enabled !== false)) {
...
function renderCompositionElementContent(tile: HTMLElement, element: any, def: any, previewModels: any): void {
  ...
  if (def.type === "surface") {
    ...
    if (def.category === "hero") {
      ...
      const backdrop = tile.querySelector<HTMLElement>(".me-hero-card__backdrop");
  ...
  if (def.type === "badges") {
    ...
    const badgesHtml = badgeItems[0]?.isPlaceholder
      ...
      : `<div class="me-badge-list">${badgeItems.map((item: any) => `<span class="me-badge-chip">${escapeHtml(item.label)}</span>`).join("")}</div>`;
    tile.dataset.compositionScale = "true";
    tile.innerHTML = `
  ...
  if (def.type === "galleryPhoto") {
    ...
    const photo = Array.isArray(previewModels.galleryPhotos) && photoIndex !== null
    ...
    if (photo) {
      ...
      tile.innerHTML = `
        <div class="gallery-item gallery-item--placeholder">
          ...
          <p class="gallery-item__caption">Photo ${(photoIndex ?? 0) + 1}</p>
        </div>
      `;
    }
    return;
  ...
  if (def.type === "identityField") {
    ...
    const child = preview.querySelector<HTMLElement>(`[data-profile-child-id="${getIdentityChildId(element.id)}"]`);
    tile.dataset.compositionScale = "true";
    const stage = document.createElement("div");
  ...
  if (def.type === "thoughtsComposer" || def.type === "thoughtsFeed") {
    ...
    const child = preview.querySelector<HTMLElement>(`[data-profile-child-id="${childId}"]`);
  ...
  const child = hero.querySelector<HTMLElement>(`[data-profile-child-id="${childId}"]`);
  if (child) {
    child.removeAttribute("data-profile-child-id");
    child.style.left = "";
    child.style.top = "";
...
export function applyCompositionElementScaling(root: Document | HTMLElement = document): void {
  root.querySelectorAll("[data-composition-scale]").forEach((tile) => {
    const stage = tile.querySelector<HTMLElement>(".profile-layout-composition-element__scale-stage");
    if (!stage) return;
...
function applyCompositionRectStyle(el: HTMLElement, item: any): void {
...
function getPlayerActionPreviewLabel(elementId: string): string {
...
function hasLivePreview(panelId: any, previewModels: any): boolean {
  return [
    "hero",
    "identity",
    "music",
...
function renderLivePreview(tile: HTMLElement, panel: any, previewModels: any): HTMLElement | null {
...
function applyPreviewChildLayout(preview: HTMLElement, panel: any): void {
  if (!Array.isArray(panel?.children)) return;
  for (const child of panel.children) {
    if (!child?.id || child.enabled === false) continue;
    const childEl = preview.querySelector<HTMLElement>(`[data-profile-child-id="${child.id}"]`);
    if (!childEl) continue;
...
function renderChildEditorOverlay(container: HTMLElement, panel: any, selectedChildId: string | null): void {
...
function applyChildRectStyle(el: HTMLElement, child: any): void {
  el.style.gridColumn = "";
  el.style.gridRow = "";
...
function renderGridOverlay(container: HTMLElement, panels: any[]): void {
  const maxRows = Math.max(
    1,
    ...panels.map((panel: any) => (panel.y || 0) + (panel.h || 1)),
  );
  const overlay = document.createElement("div");
...
function applyTileVisualStyle(tile: HTMLElement, style: any = {}): void {
...
function getPanelDensity(panel: any): string {
