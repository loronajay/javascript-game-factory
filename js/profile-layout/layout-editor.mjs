...
export function resolvePanelDragDrop({ col, row, grabCol, grabRow, w, h, columns = LAYOUT_COLUMNS, }) {
    return {
...
function findSwapTarget(panels, skipId, x, y, w, h) {
    ...
    for (const p of panels) {
        if (p.id === skipId || p.enabled === false)
            continue;
        if (x < p.x + p.w && x + w > p.x && y < p.y + p.h && y + h > p.y) {
            if (found !== null)
                return null; // 2+ overlaps — no swap
            found = p;
        }
    }
    return found;
}
export function resolvePanelResizeDrop({ panelX, panelY, col, row, def, origW, columns = LAYOUT_COLUMNS, }) {
...
export function initLayoutEditor(canvas, { getLayout, updatePanelPosition, updatePanelSize, swapPanels, onDirty, getZoom = () => 1, }) {
