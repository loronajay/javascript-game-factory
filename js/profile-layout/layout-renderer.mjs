...
import { COMPOSITION_GRID_COLUMNS, COMPOSITION_GRID_ROWS, getCompositionElementDef, } from "./composition-layout.mjs";
import { renderMeFriendCodePanel, renderMeFriendsPanel, renderMeGalleryPanel, renderMeHeroCard, renderMeIdentityPanel, renderMeRankingsPanel, renderMeThoughtsPanel, renderMeTopFriendsPanel, } from "../arcade-me-view.mjs";
import { renderProfileMusicPlayerPanel } from "../profile-editor/music-player.mjs";
import { renderAboutPanel, renderBadgesPanel, renderFavoritePanel, } from "../me-page/render-sections.mjs";
...
export function renderLayoutGrid(container, layout, options = {}) {
    ...
    const compositionCategories = new Set(elements
    ...
    for (const panel of renderedPanels) {
        ...
        if (editMode) {
            ...
            if (panel.id === selectedId)
                classes.push("profile-layout-tile--selected");
            if (!def.draggable && !def.resizable)
                classes.push("profile-layout-tile--locked");
            if (hasLivePreview(panel.id, previewModels))
                classes.push("profile-layout-tile--live-preview");
            if (childEditPanelId === panel.id)
                classes.push("profile-layout-tile--child-editing");
        }
        ...
        if (editMode && onSelect) {
            ...
            tile.addEventListener("click", (e) => {
                ...
                const t = e.target;
                if (!t?.closest("[data-drag-handle],[data-resize-handle],[data-child-id]"))
                    onSelect(panel.id);
            });
            tile.addEventListener("keydown", (e) => {
                const t = e.target;
                if (t?.closest("[data-child-id]"))
                    return;
...
function renderCompositionElements(container, elements, options = {}) {
    if (!Array.isArray(elements) || elements.length === 0 || !options.previewModels?.hero)
        return;
...
function renderCompositionElementContent(tile, element, def, previewModels) {
    ...
    if (def.type === "galleryGrid" || def.type === "galleryLink") {
        ...
        if (def.type === "galleryGrid") {
            ...
            if (grid)
                tile.appendChild(grid.cloneNode(true));
            else
            ...
            if (link)
                stage.appendChild(link.cloneNode(true));
            else
                stage.innerHTML = `<a class="gallery-view-all" href="#">View All Photos -></a>`;
            tile.appendChild(stage);
        }
        return;
    }
...
export function applyCompositionElementScaling(root = document) {
    root.querySelectorAll("[data-composition-scale]").forEach((tile) => {
        ...
        if (!tileRect.width || !tileRect.height || !stageRect.width || !stageRect.height)
            return;
...
function renderLivePreview(tile, panel, previewModels) {
    ...
    if (!hasLivePreview(panelId, previewModels))
        return null;
    const preview = document.createElement("div");
...
function renderChildEditorOverlay(container, panel, selectedChildId) {
    ...
    if (!grid || !registry || !Array.isArray(panel.children))
        return;
...
function renderGridOverlay(container, panels) {
    const maxRows = Math.max(1, ...panels.map((panel) => (panel.y || 0) + (panel.h || 1)));
    const overlay = document.createElement("div");
