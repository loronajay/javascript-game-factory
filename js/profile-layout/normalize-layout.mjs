...
export function normalizeCompositionElements(rawElements) {
    if (!Array.isArray(rawElements) || rawElements.length === 0)
        return [];
    if (compositionElementsAreUntouchedDefault(rawElements))
        return [];
...
function selectRawCompositionElement(existing, incoming) {
    ...
    if (String(incoming?.id || "").startsWith("galleryPhoto")) {
        ...
        if (existingOldTiny && !incomingOldTiny)
            return incoming;
        if (!existingOldTiny && incomingOldTiny)
            return existing;
...
function migrateCompositionElementGeometry(id, raw, def) {
    if (def.type !== "galleryPhoto" || !raw || raw.enabled === false)
        return raw || {};
    ...
    if (!looksLikeOldTinyPhotoDefault)
        return raw;
...
function disableUntouchedThoughtsFreeformDefaults(elements, rawById) {
    ...
    if (!badDefaultWasSaved)
        return elements;
    return elements.map((element) => (thoughtsIds.includes(element.id)
...
function migrateCompositionElementStylesFromPanelChildren(elements, panels) {
    ...
    if (childStyleByElementId.size === 0)
        return elements;
...
export function normalizePanelStyle(raw) {
    ...
    if (panelColor)
        style.panelColor = panelColor;
    if (panelColor2)
        style.panelColor2 = panelColor2;
    if (titleColor)
        style.titleColor = titleColor;
    if (elementColor)
        style.elementColor = elementColor;
    if (textColor)
...
export function normalizePanelChildren(panelId, rawChildren) {
    ...
    const rawById = new Map((Array.isArray(migratedChildren) ? migratedChildren : [])
...
function compositionElementsAreUntouchedDefault(rawElements) {
    ...
    if (knownElements.length !== defaultsById.size)
        return false;
...
function migratePanelChildren(panelId, rawChildren) {
    if (panelId === "identity" && Array.isArray(rawChildren)) {
        ...
        return rawChildren.map((child) => {
            const oldDefault = Object.values(oldDefaultById).find((candidate) => ((candidate.id || child?.id) === child?.id &&
                child.x === candidate.x &&
                child.y === candidate.y &&
                child.w === candidate.w &&
                child.h === candidate.h));
    ...
    if (panelId === "topFriends" && Array.isArray(rawChildren)) {
        return rawChildren.map((child) => (child?.id === "friend1" ? { ...child, id: "mainSqueeze" } : child));
    }
    if (panelId === "music" && Array.isArray(rawChildren)) {
        const oldPlayer = rawChildren.find((child) => child?.id === "player");
        const hasSplitChildren = rawChildren.some((child) => (["playerSurface", "deck", "trackLabel", "controls"].includes(child?.id)));
    ...
    if (panelId === "friends" && Array.isArray(rawChildren)) {
        ...
        const hasSplitChildren = rawChildren.some((child) => (["navigatorSurface", "toggle", "search", "list"].includes(child?.id)));
        if (oldContent && !hasSplitChildren) {
            return migrateLegacyFriendsContentChild(rawChildren, oldContent);
        }
    }
    if (panelId !== "hero" || !Array.isArray(rawChildren))
        return rawChildren;
    ...
    const isCrampedOldDefault = portrait?.x === 0 && portrait?.y === 0 && portrait?.w === 4 && portrait?.h === 2 &&
        metrics?.x === 0 && metrics?.y === 2 && metrics?.w === 4 && metrics?.h === 2;
    const isPreviousDefault = portrait?.x === 0 && portrait?.y === 0 && portrait?.w === 4 && portrait?.h === 3 &&
    ...
    const looksLikeLegacyGrid = rawChildren.every((child) => (!child ||
        (toInt(child.x, 0) <= 4 &&
            toInt(child.y, 0) <= 5 &&
            toInt(child.w, 1) <= 4 &&
            toInt(child.h, 1) <= 5)));
    if (!looksLikeLegacyGrid)
        return rawChildren;
    return rawChildren.map((child) => {
