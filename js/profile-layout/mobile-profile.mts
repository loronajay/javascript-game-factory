...
export function isMobileProfileViewport(win: ViewportWindow | undefined = globalThis as any): boolean {
  if (!win) return false;
  ...
  return Number.isFinite(win.innerWidth) && (win.innerWidth as number) <= MOBILE_MAX_WIDTH;
}
...
export function watchMobileProfileViewport(
