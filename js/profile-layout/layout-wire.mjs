...
function renderMobileEditorNotice(document) {
    const stage = document.querySelector(".me-layout-stage");
    if (!stage)
        return;
    stage.innerHTML = `
    ...
    if (!session?.ok || !session?.playerId) {
        ...
        function initCompositionElementEditor(canvasEl) {
            canvasEl.addEventListener("pointerdown", (event) => {
                ...
                if (!elementTarget || !canvasEl.contains(elementTarget))
                    return;
        ...
        function beginElementDrag(event, elementId, mode) {
            ...
            if (elementIdx < 0 || !element || !def || !canvas)
                return;
        ...
        function initChildLayoutEditor(canvasEl) {
            canvasEl.addEventListener("pointerdown", (event) => {
                const target = event.target;
                const childTarget = target?.closest("[data-child-id]");
                if (!childTarget || !canvasEl.contains(childTarget))
                    return;
                ...
                if (!panelId || childEditPanelId !== panelId || !childId)
                    return;
                event.preventDefault();
                event.stopPropagation();
        ...
        function beginChildDrag(event, panelId, childId, mode) {
            ...
            if (panelIdx < 0 || childIdx < 0 || !child || !grid || !childDef || !overlay)
                return;
        ...
        resetBtn?.addEventListener("click", () => {
            if (!confirm("Reset to default layout? This will clear your current changes."))
                return;
...
function resolvePreviewAssetUrls(value) {
    ...
    return Object.fromEntries(Object.entries(value).map(([key, child]) => [key, resolvePreviewAssetUrls(child)]));
}
function resolvePreviewAssetUrl(value) {
    ...
    if (value.startsWith("../grid-previews/"))
        return `../../${value.slice(3)}`;
    return value;
}
