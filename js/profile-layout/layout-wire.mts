...
if (typeof doc?.getElementById === "function") {
  ...
  if (!session?.ok || !session?.playerId) {
    ...
    let currentLayout: any = getDefaultLayout();
    let isDirty = false;
    let selectedPanelId: any = null;
    let childEditPanelId: any = null;
    let selectedChildId: any = null;
    let gridOverlayOn = true;
    let zoom = 1;
    let previewModels: any = {};
    let childDrag: any = null;
    let elementDrag: any = null;
    let panelQuery = "";
    // Explicit rail group open/closed decisions; anything absent falls back to the
    // derived default (see buildPanelListGroups).
    let panelGroupOverrides: Record<string, boolean> = {};

    const canvas = doc.getElementById("meLayoutCanvas") as HTMLElement;
    const canvasWrap = doc.getElementById("meLayoutCanvasWrap") as HTMLElement;
    const canvasStage = doc.getElementById("meLayoutCanvasStage") as HTMLElement;
    const inspector = doc.getElementById("meLayoutInspector") as HTMLElement;
    const panelListEl = doc.getElementById("meLayoutPanelList");
    const panelSearchEl = doc.getElementById("meLayoutPanelSearch") as HTMLInputElement | null;
    const addTitleBtn = doc.getElementById("meLayoutAddTitleBtn");
    const dirtyFlag = doc.getElementById("meLayoutDirtyFlag");
    const saveBtn = doc.getElementById("meLayoutSaveBtn") as HTMLButtonElement;
    ...
    function selectPanel(id: any): void {
    ...
    function panelGroupKeyFor(id: any): string | null {
      if (!id) return null;
      if (currentLayout.desktop.panels.some((panel: any) => panel.id === id)) return "panels";
      return getCompositionElementDef(id)?.category ?? null;
    }

    function selectChild(id: any): void {
      selectedChildId = id;
      refreshAll();
    }
    ...
    function getCanvasViewportSize(): { width: number; height: number } {
    ...
    function updatePanelPosition(id: any, x: number, y: number): void {
      const idx = currentLayout.desktop.panels.findIndex((p: any) => p.id === id);
      if (idx < 0) return;
      currentLayout.desktop.panels[idx] = { ...currentLayout.desktop.panels[idx], x, y };
      refreshAll();
    }

    function updatePanelSize(id: any, w: number, h: number): void {
      const idx = currentLayout.desktop.panels.findIndex((p: any) => p.id === id);
      if (idx < 0) return;
    ...
    function updatePanelStyle(id: any, patch: any): void {
      const idx = currentLayout.desktop.panels.findIndex((p: any) => p.id === id);
      if (idx < 0) {
    ...
    function updateElementStyle(id: any, patch: any): void {
      const idx = currentLayout.desktop.elements?.findIndex((element: any) => element.id === id) ?? -1;
      if (idx < 0) return;
    ...
    function applyLivePreviewScaling(): void {
    ...
    function resetPanelStyle(id: any): void {
      const idx = currentLayout.desktop.panels.findIndex((p: any) => p.id === id);
      if (idx < 0) {
    ...
    function resetElementStyle(id: any): void {
      const idx = currentLayout.desktop.elements?.findIndex((element: any) => element.id === id) ?? -1;
      if (idx < 0) return;
    ...
    function swapPanels(idA: any, xA: number, yA: number, idB: any, xB: number, yB: number): void {
      const panels = currentLayout.desktop.panels;
      const idxA = panels.findIndex((p: any) => p.id === idA);
      const idxB = panels.findIndex((p: any) => p.id === idB);
    ...
    function initCompositionElementEditor(canvasEl: HTMLElement): void {
      canvasEl.addEventListener("pointerdown", (event) => {
        const target = event.target as Element | null;
        const elementTarget = target?.closest<HTMLElement>("[data-element-id]");
        if (!elementTarget || !canvasEl.contains(elementTarget)) return;
        const elementId = elementTarget.dataset.elementId;
        const mode = target?.closest("[data-element-resize-handle]") ? "resize" : "move";
    ...
    function beginElementDrag(event: PointerEvent, elementId: any, mode: string): void {
      const elementIdx = currentLayout.desktop.elements?.findIndex((element: any) => element.id === elementId) ?? -1;
    ...
    function onElementDragEnd(): void {
      finishElementDrag(true);
    }

    function onElementDragCancel(): void {
      finishElementDrag(false);
    }

    function finishElementDrag(commit: boolean): void {
      if (!elementDrag) return;
    ...
    function pointerToCompositionPoint(clientX: number, clientY: number, canvasEl: HTMLElement) {
    ...
    function clampCompositionRect(element: any, def: any) {
    ...
    function paintElementDragPreview(element: any): void {
      const el = canvas.querySelector<HTMLElement>(`[data-element-id="${element.id}"]`);
      if (!el) return;
    ...
    function initChildLayoutEditor(canvasEl: HTMLElement): void {
      canvasEl.addEventListener("pointerdown", (event) => {
        const target = event.target as Element | null;
        const childTarget = target?.closest<HTMLElement>("[data-child-id]");
        if (!childTarget || !canvasEl.contains(childTarget)) return;
        const panelTile = childTarget.closest<HTMLElement>("[data-panel-id]");
        ...
        event.preventDefault();
        event.stopPropagation();
        beginChildDrag(event, panelId, childId, target?.closest("[data-child-resize-handle]") ? "resize" : "move");
      });
    }

    function beginChildDrag(event: PointerEvent, panelId: any, childId: any, mode: string): void {
      const panelIdx = currentLayout.desktop.panels.findIndex((panel: any) => panel.id === panelId);
      const panel = currentLayout.desktop.panels[panelIdx];
      const childIdx = panel?.children?.findIndex((child: any) => child.id === childId) ?? -1;
      ...
      const overlay = canvas.querySelector<HTMLElement>(`[data-panel-id="${panelId}"] .profile-layout-child-grid`);
    ...
    function onChildDragMove(event: PointerEvent): void {
      if (!childDrag) return;
      event.preventDefault();
    ...
    function onChildDragCancel(): void {
      finishChildDrag(false);
    }

    function finishChildDrag(commit: boolean): void {
      if (!childDrag) return;
    ...
    function pointerToChildPoint(clientX: number, clientY: number, overlay: HTMLElement) {
      const rect = overlay.getBoundingClientRect();
      const grid = getPanelChildGrid(overlay.closest<HTMLElement>("[data-panel-id]")?.dataset.panelId ?? "");
    ...
    function clampChildRect(child: any, grid: any, def: any) {
    ...
    function roundChildUnit(value: number): number {
      return Math.round(value * 10) / 10;
    }

    function paintChildDragPreview(child: any): void {
      const box = canvas.querySelector<HTMLElement>(`[data-panel-id="${childDrag.panelId}"] .profile-layout-child-grid__box[data-child-id="${child.id}"]`);
      const slot = canvas.querySelector<HTMLElement>(`[data-panel-id="${childDrag.panelId}"] [data-child-slot="${child.id}"]`);
      const liveChild = canvas.querySelector<HTMLElement>(`[data-panel-id="${childDrag.panelId}"] [data-profile-child-id="${child.id}"]`);
    ...
    function renderPanelList(): void {
      ...
      panelListEl.querySelectorAll<HTMLElement>("[data-panel-select]").forEach((btn) => {
      ...
      panelListEl.querySelectorAll<HTMLElement>("[data-panel-visibility]").forEach((btn) => {
        btn.addEventListener("click", () => setItemEnabled(btn.dataset.panelVisibility, btn.getAttribute("aria-pressed") !== "true"));
      });
      panelListEl.querySelectorAll<HTMLElement>("[data-panel-group-toggle]").forEach((btn) => {
    ...
    function revealSelectedPanelRow(): void {
      if (!panelListEl || !selectedPanelId) return;
      const row = panelListEl.querySelector<HTMLElement>(`[data-panel-select="${CSS.escape(selectedPanelId)}"]`);
      row?.scrollIntoView({ block: "nearest" });
    }

    /** Single entry point for show/hide so panels and elements behave the same. */
    function setItemEnabled(id: any, enabled: boolean): void {
      if (!id) return;
      if (currentLayout.desktop.panels.some((panel: any) => panel.id === id)) {
    ...
    function renderInspector(): void {
      ...
      const panel = currentLayout.desktop.panels.find((p: any) => p.id === selectedPanelId);
      ...
      inspector.querySelector("[data-toggle-panel]")?.addEventListener("change", (e) => {
        togglePanel(panel.id, (e.target as HTMLInputElement).checked);
      });
      inspector.querySelectorAll<HTMLInputElement>("[data-panel-style]").forEach((input) => {
        input.addEventListener("input", () => {
          ...
          if (output && input.type === "range") {
            output.textContent = key === "gradientAngle" ? `${Math.round(Number(value))}deg` : `${Math.round(Number(value) * 100)}%`;
          }
      ...
      inspector.querySelectorAll<HTMLElement>("[data-child-select]").forEach((btn) => {
        btn.addEventListener("click", () => {
    ...
    function renderElementInspector(id: any): void {
      const element = currentLayout.desktop.elements?.find((item: any) => item.id === id);
      ...
      inspector.querySelector("[data-toggle-element]")?.addEventListener("change", (e) => {
        toggleElement(element.id, (e.target as HTMLInputElement).checked);
      });
    ...
    function addCustomTitleBubble(): void {
    ...
    function deleteElement(id: any): void {
      if (!isCustomTitleElementId(id)) return;
      currentLayout.desktop.elements = (currentLayout.desktop.elements || []).filter((element: any) => element.id !== id);
      if (selectedPanelId === id) selectedPanelId = null;
      markDirty();
      refreshAll();
    }

    function togglePanel(id: any, enabled: boolean): void {
      const idx = currentLayout.desktop.panels.findIndex((p: any) => p.id === id);
      if (idx < 0) return;
    ...
    function toggleElement(id: any, enabled: boolean): void {
      const idx = currentLayout.desktop.elements?.findIndex((element: any) => element.id === id) ?? -1;
      if (idx < 0) return;
    ...
    function updateElementText(id: any, text: string): void {
      const idx = currentLayout.desktop.elements?.findIndex((element: any) => element.id === id) ?? -1;
      if (idx < 0) return;
...
async function buildPreviewModels(playerId: any, storage: any, apiClient: any): Promise<any> {
...
function resolvePreviewAssetUrls(value: any): any {
...
function resolvePreviewAssetUrl(value: any): any {
