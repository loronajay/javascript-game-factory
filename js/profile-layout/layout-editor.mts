...
export function getGridMetrics(canvas: HTMLElement): GridMetrics {
...
function pointerToCell(clientX: number, clientY: number, canvas: HTMLElement, zoom = 1) {
...
function findSwapTarget(panels: any[], skipId: string, x: number, y: number, w: number, h: number): any {
  let found = null;
...
export interface LayoutEditorCallbacks {
  ...
  updatePanelSize: (panelId: string, w: number, h: number) => void;
  swapPanels?: (...args: any[]) => void;
  onDirty: () => void;
  getZoom?: () => number;
}
...
export function initLayoutEditor(canvas: HTMLElement, {
  getLayout,
  updatePanelPosition,
  updatePanelSize,
  swapPanels,
  onDirty,
  getZoom = () => 1,
}: LayoutEditorCallbacks): void {
  let drag: any = null;   // active drag state
  let resize: any = null; // active resize state
  ...
  function placeGhost(x: number, y: number, w: number, h: number, state: string): void {
  ...
  canvas.addEventListener("pointerdown", (e) => {
    ...
    if (resizeHandle) {
      const tile = resizeHandle.closest<HTMLElement>("[data-panel-id]");
      if (!tile) return;

      const panelId = tile.dataset.panelId as string;
      const def = PROFILE_PANEL_REGISTRY[panelId];
      if (!def?.resizable) return;

      const layout = getLayout();
      const panel = layout.desktop.panels.find((p: any) => p.id === panelId);
      if (!panel) return;
    ...
    if (dragHandle) {
      const tile = dragHandle.closest<HTMLElement>("[data-panel-id]");
      if (!tile) return;
