...
function material(THREE: ThreeNamespace, color: string, roughness = 0.52, metalness = 0.08): any {
  return new THREE.MeshStandardMaterial({ color, roughness, metalness });
}

function emissive(THREE: ThreeNamespace, color: string, intensity = 2.8): any {
...
function createSignTexture(THREE: ThreeNamespace): any | null {
...
function addBasketball(
  ...
  for (let line = 0; line < 3; line += 1) {
    ...
    if (line === 0) ring.rotation.x = Math.PI / 2;
    if (line === 1) ring.rotation.y = Math.PI / 2;
    if (line === 2) ring.rotation.z = Math.PI / 2;
    group.add(ring);
  }
  parent.add(group);
}

function addSevenSegmentDigit(
  THREE: ThreeNamespace,
  parent: any,
  prefix: string,
  x: number,
...
function addCage(THREE: ThreeNamespace, root: any, metal: any, dark: any): void {
...
function addHoopAssembly(THREE: ThreeNamespace, root: any, shell: any, white: any, red: any, metal: any): void {
  ...
  for (const side of [-1, 1]) box(THREE, root, "backboard-target-side", [0.035, 0.305, 0.012], [side * 0.252, 1.795, -1.226], red);
  ...
  for (let index = 0; index < 12; index += 1) {
    ...
    const strand = cylinder(
      THREE,
      net,
      `net-strand-${index + 1}`,
      0.007,
      0.42,
      [Math.cos(angle) * 0.22, 1.62, -0.91 + Math.sin(angle) * 0.22],
      white,
      6,
    );
...
export function createMiniHoopsCarnivalCabinet(THREE: ThreeNamespace, definition: CabinetDefinition): any {
  ...
  for (const [x, color] of [[-0.72, definition.palette.sky], [0.72, definition.palette.warning]] as const) {
