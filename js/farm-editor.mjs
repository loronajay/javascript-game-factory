...
export function createFarmEditor(options) {
    ...
    let handleDrag = null;
    const history = createEditHistory({ equal: farmLayoutsEqual });
    ...
    function removeBlockedReason() {
        ...
        if (!definition || definition.habitat !== "water")
            return "";
    ...
    function renderScene() {
        ...
        if (model)
            selectionBounds.setFromObject(model);
    ...
    function explain(reason) {
        if (reason === "blocked")
            return "That spot is taken · move it somewhere clear.";
        if (reason === "outside")
            return "That is off the field.";
        if (reason === "spawn")
    ...
    function editLength(result, phase) {
        if (phase === "preview") {
            if (!result.valid) {
    ...
    function resizeSelected(direction, phase) {
        ...
        if (result.valid && farmLayoutsEqual(result.layout, layout))
            setStatus(direction > 0 ? "That's as long as it goes." : "That's as short as it goes.", "error");
        else
            editLength(result, phase);
    }
    function undo() {
    ...
    function select(instanceId, quiet = false) {
    ...
    function remove(instanceId) {
        ...
        if (!row)
            return;
        if (row.itemId === "decor.prop.pet-tombstone" && !confirm("Remove this memorial stone permanently? The pet's history will stay in your farm records, but the stone cannot be restored."))
            return;
    ...
    function applyPlacement(next, quiet = false) {
        const row = selected();
        if (!row)
            return false;
    ...
    function resizeFromPointer(event) {
        const row = selected();
        if (!row || !handleDrag || handleDrag.kind !== "stretch")
            return;
        const point = groundPoint(event);
        if (!point)
            return;
    ...
    function endHandleDrag() {
        ...
        gizmos.setGuides([]);
        if (row && history.isOpen())
            editLength({ valid: true, layout, instanceId: row.instanceId, reason: "" }, "commit");
    }
    function enter() {
        if (editing || !canEnter())
            return;
    ...
    elements.resetButton.addEventListener("click", () => {
        if (!confirm("Reset the field to the starter farm? Fences, buildings, plants and ponds go back; your pets stay (swimmers need a pond to stay)."))
            return;
