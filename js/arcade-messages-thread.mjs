...
async function run() {
    ...
    if (!api.isConfigured) {
        if (flashEl)
            flashEl.textContent = "Messages require an internet connection.";
        return;
    }
    if (!viewerPlayerId) {
        if (flashEl)
            flashEl.textContent = "Sign in to send and receive messages.";
        if (inputEl)
    ...
    async function loadConversation() {
        ...
        if (!data)
            return;
        if (!toPlayerId)
            toPlayerId = data.conversation?.otherPlayerId || "";
        if (!otherName)
            otherName = data.conversation?.otherProfileName || toPlayerId;
        if (otherNameEl)
    ...
    async function sendMessage() {
        ...
        if (result) {
            ...
            if (flashEl) {
                ...
                setTimeout(() => { if (flashEl)
                    flashEl.textContent = ""; }, 3000);
            }
        }
        if (sendBtn)
