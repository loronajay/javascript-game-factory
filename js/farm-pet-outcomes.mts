...
export type PetWarningStage = "safe" | "watch" | "urgent" | "critical";
export type PetOutcomeWarning = Readonly<{ stage: PetWarningStage; label: string; message: string }>;

export function petOutcomeWarning(profile: PetProfile, speciesId: string): PetOutcomeWarning {
...
export function reactToPetCall(profile: PetProfile): Readonly<{ ok: boolean; message: string }> {
  if (profile.happiness <= URGENT_HAPPINESS) return Object.freeze({ ok: false, message: "Is too distressed to come when called." });
  if (profile.affection < AFFECTION_CALL_THRESHOLD) return Object.freeze({ ok: false, message: "Does not trust the call yet." });
  return Object.freeze({ ok: true, message: "Comes when called." });
}

function stableRandom(identity: string): number {
...
function memorialId(layout: FarmLayout, pet: FarmPet): string {
  const base = `memory-${pet.instanceId}`.slice(0, 40);
  if (!layout.petHistory.some((entry) => entry.id === base)) return base;
  let number = 2;
  while (layout.petHistory.some((entry) => entry.id === `${base.slice(0, 36)}-${number}`)) number += 1;
  return `${base.slice(0, 36)}-${number}`;
}

function remember(layout: FarmLayout, pet: FarmPet, outcome: PetDepartureOutcome, farmMinute: number): FarmLayout {
  if (!pet.profile) return withFarmPets(layout, layout.pets.filter((row) => row.instanceId !== pet.instanceId));
  const id = memorialId(layout, pet);
  const profile = pet.profile;
  const entry: PetMemorial = Object.freeze({
  ...
  if (outcome === "runaway") return next;
...
export function resolvePetOutcomes(layout: FarmLayout, previousFarmMinute: number, targetFarmMinute: number, random?: () => number): FarmLayout {
  let next = layout;
  ...
  for (const original of layout.pets) {
    ...
    for (let day = firstDay; day <= lastDay && next.pets.some((row) => row.instanceId === pet.instanceId); day += 1) {
      ...
      if (runawayChance > 0 && roll("runaway") < runawayChance) { next = remember(next, pet, "runaway", day * FARM_DAY_MINUTES); break; }
      if (pet.profile.happiness <= 5 && roll("neglect") < RARE_NEGLECT_DEATH_CHANCE_PER_DAY) { next = remember(next, pet, "neglect", day * FARM_DAY_MINUTES); break; }
    }
  }
  return next;
}
