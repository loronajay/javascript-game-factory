...
async function run() {
    ...
    if (!api.isConfigured) {
        if (flashEl)
            flashEl.textContent = "Notifications require an internet connection.";
        return;
    }
    if (!session?.playerId) {
        if (flashEl)
            flashEl.textContent = "Sign in to view your notifications.";
        return;
    }
    ...
    for (const notif of notifications) {
        const item = renderNotificationItem(notif, async (requestId, itemEl) => {
            ...
            if (ok && gameSlug) {
                ...
                if (p)
                    p.innerHTML = `${p.innerHTML} <a class="notif-item__game-link" href="${href}">Play Now →</a>`;
