...
export function renderGalleryPanel(container, title, photos = [], options = {}) {
  if (!container) return;
  ...
  const uploadHtml = isOwner
    ? `
      <div class="gallery-upload">
        <input id="meGalleryFileInput" type="file" accept="image/jpeg,image/png,image/webp" class="gallery-upload__input" aria-label="Upload a photo">
        <label for="meGalleryFileInput" class="gallery-upload__label">${uploadState.previewUrl ? "Choose Different Photo" : "Upload Photo"}</label>
        <p id="meGalleryUploadStatus" class="gallery-upload__status" aria-live="polite">${escapeHtml(uploadState.statusMessage || "")}</p>
      </div>
      ${uploadState.previewUrl
        ? `
          <form id="meGalleryUploadForm" class="gallery-upload__composer">
            ...
            <div class="gallery-upload__fields">
              <label class="gallery-upload__field" for="meGalleryCaption">
              ...
              <label class="gallery-upload__field" for="meGalleryVisibility">
                <span class="gallery-upload__field-label">Visibility</span>
                <select id="meGalleryVisibility" class="gallery-upload__select">
              ...
              <label class="gallery-upload__toggle" for="meGalleryPostToFeed">
                <input id="meGalleryPostToFeed" type="checkbox"${uploadState.postToFeed ? " checked" : ""}>
            ...
            <div class="gallery-upload__actions">
              ...
              <button class="gallery-upload__button" type="button" data-cancel-gallery-upload="me">Cancel</button>
            </div>
          </form>
        `
        : ""}
  ...
  const gridHtml = photos.length > 0
    ? photos.map((photo) => `
      ...
      `).join("")
    : `<p class="me-panel__empty">${escapeHtml(isOwner ? "No photos yet. Upload one above." : "No photos yet.")}</p>`;
    ...
    ? `
      <form id="meThoughtComposer" class="thought-composer thought-composer--owner">
            ...
            <div class="thought-composer__photo-card">
              <div class="thought-composer__photo-preview">
                ...
                <div class="thought-composer__photo-preview-copy">
                  ...
                  <button class="thought-composer__photo-clear" type="button" data-clear-thought-photo="me">Remove Photo</button>
                </div>
              </div>
              <div class="thought-composer__photo-fields">
                <label class="thought-composer__field" for="meThoughtPhotoCaption">
                  <span class="thought-composer__field-label">Gallery Caption</span>
                  <textarea
                    id="meThoughtPhotoCaption"
                ...
                <label class="thought-composer__field" for="meThoughtVisibility">
                  <span class="thought-composer__field-label">Visibility</span>
                  <select id="meThoughtVisibility" class="thought-composer__field-input">
                ...
                <label class="thought-composer__toggle" for="meThoughtSaveToGallery">
                  <input id="meThoughtSaveToGallery" type="checkbox"${composerState.saveToGallery !== false ? " checked" : ""}>
        ...
        <div class="thought-composer__actions">
          ...
          <label class="thought-composer__attach-label" for="meThoughtPhotoInput" title="Attach photo">
            <span class="thought-composer__attach-icon" aria-hidden="true">&#128247;</span>
            <span id="meThoughtPhotoName" class="thought-composer__attach-name">${escapeHtml(composerState.fileName || "")}</span>
          </label>
          <input id="meThoughtPhotoInput" type="file" accept="image/jpeg,image/png,image/webp" class="thought-composer__photo-input">
...
export function renderMePageView(doc, model, options = {}) {
  ...
  renderGalleryPanel(doc.getElementById("meGalleryPanel"), "Photo Gallery", options?.galleryPhotos || [], {
    isOwner: true,
