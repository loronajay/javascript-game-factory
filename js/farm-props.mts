...
export function createBed(THREE: ThreeNamespace): any {
  const group = new THREE.Group();
  ...
  for (const x of [-0.56, 0.56]) for (const z of [-0.91, 0.91]) box(THREE, group, [0.12, 0.45, 0.12], [x, 0.225, z], wood);
  box(THREE, group, [1.18, 0.22, 1.9], [0, 0.5, 0], linen);
  box(THREE, group, [1.2, 0.08, 1.25], [0, 0.65, 0.28], quilt);
  for (const x of [-0.31, 0.31]) box(THREE, group, [0.52, 0.14, 0.42], [x, 0.67, -0.67], pillow);
  box(THREE, group, [1.34, 0.9, 0.12], [0, 0.72, -0.99], wood);
  for (const x of [-0.56, 0.56]) box(THREE, group, [0.12, 1.25, 0.12], [x, 0.625, -0.99], wood);
  return group;
}
