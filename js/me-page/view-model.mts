...
export interface MePageViewModel {
  playerId: string;
  pageTitle: string;
  pageSubtitle: string;
  heroName: string;
  ...
  friendCodeValue: string;
  friendCodeDisplay: string;
  friendCodeFlashMessage: string;
  avatarSrc: string;
  avatarAlt: string;
...
export interface BuildMePageViewModelOptions {
  thoughtFeed?: unknown[];
...
function resolveOwnerPresence(_presence: unknown): string {
  return "online";
}

export function buildMePageViewModel(profile: any, options: BuildMePageViewModelOptions = {}): MePageViewModel {
