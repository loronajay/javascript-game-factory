...
export function applyProfileLayout(doc, layout, { panelToDom = ME_PANEL_TO_DOM, required = ME_REQUIRED, layoutSelector = ".me-layout", galleryPhotos = [], isMobile, } = {}) {
    ...
    const promotedOverlays = new Map();
    layoutEl.querySelectorAll("[data-profile-composition-overlay]").forEach((el) => {
...
function restorePromotedOverlays(doc, layoutEl) {
    layoutEl.querySelectorAll("[data-profile-composition-promoted='true']").forEach((el) => {
...
function promotedOverlayHomePanel(doc, overlayId) {
...
function renderCompositionOverlays(doc, layoutEl, elements, panels = [], { galleryPhotos = [], promotedOverlays = new Map() } = {}) {
    if (!Array.isArray(elements))
        return;
...
function claimPromotedOverlay(promotedOverlays, id) {
...
function renderIdentityFieldOverlay(doc, layoutEl, element, panels, promotedOverlays) {
    const childId = getIdentityChildId(element.id);
    const source = doc.querySelector(`#meIdentityPanel [data-profile-child-id='${childId}'], #playerIdentityPanel [data-profile-child-id='${childId}']`)
    ...
    if (source)
        fieldEl.dataset.profileCompositionPromoted = "true";
...
function renderPlayerActionOverlay(doc, layoutEl, element, panels, promotedOverlays) {
    const childId = getPlayerActionChildId(element.id);
    const source = doc.querySelector(`#playerIdentityPanel [data-profile-action-id='${childId}']`)
        || claimPromotedOverlay(promotedOverlays, element.id);
    ...
    if (source)
        actionEl.dataset.profileCompositionPromoted = "true";
...
function renderThoughtsComposerOverlay(doc, layoutEl, element, panels, promotedOverlays) {
    ...
    if (source)
        composerEl.dataset.profileCompositionPromoted = "true";
...
function renderThoughtsFeedOverlay(doc, layoutEl, element, panels, promotedOverlays) {
    ...
    if (source)
        feedEl.dataset.profileCompositionPromoted = "true";
