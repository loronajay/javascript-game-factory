...
function getCssToken(name: string, fallback: number): number {
...
function ensureZoomShell(panelEl: HTMLElement): HTMLElement {
  let shell = panelEl.querySelector<HTMLElement>(":scope > ." + ZOOM_SHELL_CLASS);
  if (!shell) {
...
function unwrapZoomShell(panelEl: HTMLElement): void {
  const shell = panelEl.querySelector<HTMLElement>(":scope > ." + ZOOM_SHELL_CLASS);
  if (!shell) return;
  while (shell.firstChild) panelEl.insertBefore(shell.firstChild, shell);
  shell.remove();
}

function ensureChildZoomShell(childEl: HTMLElement): HTMLElement {
  let shell = childEl.querySelector<HTMLElement>(":scope > ." + CHILD_ZOOM_SHELL_CLASS);
  if (!shell) {
    shell = document.createElement("div");
...
function applyChildScaling(childEl: HTMLElement): void {
...
function applyPanelChildScaling(panelEl: HTMLElement): void {
  panelEl.querySelectorAll<HTMLElement>(":scope .panel-zoom-shell > [data-profile-child-id]").forEach((childEl) => {
    applyChildScaling(childEl);
  });
}

function hasHeroComposition(layout: any): boolean {
  return Array.isArray(layout?.desktop?.elements) &&
    layout.desktop.elements.some((element: any) => element?.category === "hero" && element.enabled !== false);
}

function hasEnabledComposition(layout: any): boolean {
  return Array.isArray(layout?.desktop?.elements) &&
    layout.desktop.elements.some((element: any) => element?.enabled !== false);
}

function applyHeroCompositionScaling(panelEl: HTMLElement): void {
  unwrapZoomShell(panelEl);
  panelEl.style.zoom = "";
  panelEl.style.width = "";
  panelEl.style.height = "";
  panelEl.style.overflow = "hidden";
  panelEl.querySelectorAll<HTMLElement>(":scope > [data-profile-composition-id]").forEach((childEl) => {
    applyChildScaling(childEl);
  });
}

function applyCompositionOverlayScaling(layoutEl: HTMLElement): void {
  layoutEl.querySelectorAll<HTMLElement>(":scope > [data-profile-composition-overlay][data-profile-child-id]").forEach((overlayEl) => {
    applyChildScaling(overlayEl);
  });
}

function getNaturalShellSize(shell: HTMLElement): { width: number; height: number } {
...
export function applyPanelScaling(doc: Document, layout: any, panelToDom: Record<string, string>, layoutSelector: string): void {
  ...
  if (layoutEl.classList.contains("layout--flow")) return;
  ...
  for (const panel of getRenderablePanelsForScaling(layout.desktop.panels, layoutSelector)) {
    ...
    const def = (PROFILE_PANEL_REGISTRY as Record<string, any>)[panel.id];
    if (!def) continue;
    ...
    const fitZoom = (targetW: number, targetH: number) => parseFloat(Math.max(
      MIN_ZOOM,
...
function getRenderablePanelsForScaling(panels: any[], layoutSelector: string): any[] {
...
export function applyMeScaling(doc: Document, layout: any): void {
  applyPanelScaling(doc, layout, ME_PANEL_TO_DOM, ".me-layout");
}

export function applyPlayerScaling(doc: Document, layout: any): void {
