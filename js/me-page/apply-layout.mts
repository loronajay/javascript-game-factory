...
export const PLAYER_PANEL_TO_DOM: Record<string, string> = {
  hero: "playerHeroCard",
...
interface ApplyProfileLayoutOptions {
  panelToDom?: Record<string, string>;
  required?: Set<string>;
  layoutSelector?: string;
  galleryPhotos?: any[];
  // Test seam — lets the suite drive both branches without a real viewport.
  isMobile?: boolean;
}

// Applies saved panel layout onto the 12-column CSS grid.
export function applyProfileLayout(doc: Document, layout: any, {
  panelToDom = ME_PANEL_TO_DOM,
  required = ME_REQUIRED,
  layoutSelector = ".me-layout",
  galleryPhotos = [],
  isMobile,
}: ApplyProfileLayoutOptions = {}): void {
  ...
  const staleFlowColumns = [...layoutEl.querySelectorAll<HTMLElement>(":scope > ." + FLOW_COLUMN_CLASS)];
...
export function getLayoutRowCount(panels: any[], compositionCategories: Set<string>, required: Set<string>): number {
  let rows = 1;
  for (const panel of panels) {
    if (!panel || panel.enabled === false) continue;
    if (compositionCategories.has(panel.id) && !required.has(panel.id)) continue;
...
function resetHeroCompositionForFlow(el: HTMLElement): void {
  el.classList.remove("profile-composition-hero", "profile-composition-surface--hidden");
  el.querySelectorAll<HTMLElement>("[data-profile-composition-id]").forEach((childEl) => {
...
export function comparePanelsByFreeformPosition(a: any, b: any): number {
  return (a.y - b.y) || (a.x - b.x);
}

function applyPanelChildLayout(panelEl: HTMLElement, panel: any): void {
...
function findPanelLayoutChild(panelEl: HTMLElement, childId: string): HTMLElement | null {
  const all = [...panelEl.querySelectorAll<HTMLElement>(`[data-profile-child-id="${childId}"]`)];
  ...
  const shell = panelEl.querySelector<HTMLElement>(":scope > .panel-zoom-shell");
  return all.find((childEl) => childEl.parentElement === panelEl || (shell && childEl.parentElement === shell)) || null;
}

function applyHeroCompositionLayout(doc: Document, heroEl: HTMLElement, elements: any): void {
  if (!heroEl?.classList?.contains("me-hero-card") && !heroEl?.classList?.contains("player-hero-card")) return;
  if (!Array.isArray(elements)) return;

  const heroElements = elements.filter((element: any) => element?.category === "hero");
  if (!heroElements.some((element: any) => element.enabled !== false)) return;

  const byId = new Map<string, any>(heroElements.map((element: any) => [element.id, element]));
  ...
  applyHeroCompositionChild(heroEl, "heroPortrait", heroEl.querySelector<HTMLElement>('[data-profile-child-id="portrait"]'), byId.get("heroPortrait"), surface);
  applyHeroCompositionChild(heroEl, "heroMetrics", heroEl.querySelector<HTMLElement>('[data-profile-child-id="metrics"]'), byId.get("heroMetrics"), surface);

  const title = byId.get("heroTitle");
  let titleEl = heroEl.querySelector<HTMLElement>('[data-profile-composition-id="heroTitle"]');
...
function applyHeroCompositionChild(heroEl: HTMLElement, elementId: string, childEl: HTMLElement | null, element: any, surface: any): void {
...
function isCustomTitleElement(element: any): boolean {
  return element?.type === "title" && typeof element.id === "string" && element.id.startsWith(CUSTOM_TITLE_PREFIX);
}

export function getCompositionElementsForRender(elements: any, { galleryPhotos = [] }: { galleryPhotos?: any[] } = {}): any[] {
  void galleryPhotos;
...
function removeLegacyTinyGalleryPhotos(elements: any[]): any[] {
  return elements.filter((element) => !isLegacyTinyGalleryPhotoElement(element));
}

function isLegacyTinyGalleryPhotoElement(element: any): boolean {
...
export function getCompositionCategories(elements: any): Set<string> {
...
function renderTitleOverlay(doc: Document, layoutEl: HTMLElement, element: any, text: string): void {
...
function renderSurfaceOverlay(doc: Document, layoutEl: HTMLElement, element: any, category: string): void {
...
function renderAboutTextOverlay(doc: Document, layoutEl: HTMLElement, element: any, panels: any[]): void {
...
function renderBadgesContentOverlay(doc: Document, layoutEl: HTMLElement, element: any, panels: any[]): void {
...
function renderFriendCodeContentOverlay(doc: Document, layoutEl: HTMLElement, element: any, panels: any[]): void {
...
function renderGalleryContentOverlay(doc: Document, layoutEl: HTMLElement, element: any, panels: any[], galleryPhotos: any[] = []): void {
...
function renderGalleryLinkOverlay(doc: Document, layoutEl: HTMLElement, element: any, panels: any[]): void {
...
function renderGalleryPhotoOverlay(doc: Document, layoutEl: HTMLElement, element: any, panels: any[], galleryPhotos: any[] = []): void {
  const index = getGalleryPhotoIndex(element.id);
  const sourceItems = doc.querySelectorAll<HTMLElement>("#meGalleryPanel .gallery-item, #playerGalleryPanel .gallery-item");
  const source = Number.isInteger(index) ? sourceItems[index as number] : null;
  const photo = Array.isArray(galleryPhotos) && Number.isInteger(index) ? galleryPhotos[index as number] : null;
  ...
  if (photo) {
    ...
    photoEl.innerHTML = `<div class="gallery-item__img-frame"></div><p class="gallery-item__caption">Photo ${(index as number) + 1}</p>`;
  }
...
function renderGalleryGridHtml(galleryPhotos: any[] = []): string {
...
function renderGalleryItemHtml(photo: any): string {
  return `
...
function getThoughtsFeedRenderElement(feedElement: any, elements: any, isOwnerLayout: boolean): any {
  if (isOwnerLayout) return feedElement;
  const composer = (Array.isArray(elements) ? elements : [])
    .find((element: any) => element.id === "thoughtsComposer" && element.enabled !== false);
...
function mergeLegacyChildStyle(element: any, panels: any[], panelId: string, childId: string): any {
  const legacyStyle = (Array.isArray(panels) ? panels : [])
    .find((panel) => panel.id === panelId)
    ?.children
    ?.find((child: any) => child.id === childId)
    ?.style;
  return {
    ...(legacyStyle || {}),
    ...(element.style || {}),
...
function getDefaultTitleText(element: any): string {
...
function getIdentityChildId(elementId: string): string {
...
function getPlayerActionChildId(elementId: string): string {
...
function getGalleryPhotoIndex(elementId: string): number | null {
...
function getDefaultIdentityFieldLabel(elementId: string): string {
  if (elementId === "identityName") return "Name";
...
function getDefaultPlayerActionLabel(elementId: string): string {
...
function getPanelCssSlug(category: string): string {
...
function applyCompositionOverlayRect(el: HTMLElement, element: any): void {
...
function compositionToSurfacePercent(value: number, surfaceStart: any, surfaceSize: any, gridSize: number): number {
...
function compositionSizeToSurfacePercent(value: number, surfaceSize: any, gridSize: number): number {
  const size = Number.isFinite(surfaceSize) && surfaceSize > 0 ? surfaceSize : gridSize;
  return (value / size) * 100;
}

function applyPanelVisualStyle(el: HTMLElement, style: any = {}): void {
...
function setOrClear(el: HTMLElement, name: string, value: string): void {
...
function adjustHexColor(hex: string, saturation: number, brightness: number): string {
  const rgb = hexToRgb(hex);
  if (!rgb) return "";
  const gray = rgb.r * 0.299 + rgb.g * 0.587 + rgb.b * 0.114;
  const adjust = (channel: number) => Math.round(Math.min(255, Math.max(0, (gray + (channel - gray) * saturation) * brightness)));
  return `${adjust(rgb.r)}, ${adjust(rgb.g)}, ${adjust(rgb.b)}`;
}

export function applyMeLayout(doc: Document, layout: any, options: { galleryPhotos?: any[] } = {}): void {
...
export function applyPlayerLayout(doc: Document, layout: any, options: { galleryPhotos?: any[] } = {}): void {
  applyProfileLayout(doc, layout, {
