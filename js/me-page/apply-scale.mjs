...
function ensureZoomShell(panelEl) {
    ...
    if (!shell) {
        ...
        while (panelEl.firstChild)
            shell.appendChild(panelEl.firstChild);
...
function unwrapZoomShell(panelEl) {
    ...
    while (shell.firstChild)
        panelEl.insertBefore(shell.firstChild, shell);
    shell.remove();
}
function ensureChildZoomShell(childEl) {
    ...
    if (!shell) {
        ...
        while (childEl.firstChild)
            shell.appendChild(childEl.firstChild);
...
function applyChildScaling(childEl) {
    ...
    const z = parseFloat(Math.max(MIN_ZOOM, Math.min(Math.max(1, availableW - CHILD_SCALE_FIT_BUFFER) / naturalW, Math.max(1, availableH - CHILD_SCALE_FIT_BUFFER * 2) / naturalH, MAX_ZOOM)).toFixed(4));
...
export function applyPanelScaling(doc, layout, panelToDom, layoutSelector) {
    ...
    for (const panel of getRenderablePanelsForScaling(layout.desktop.panels, layoutSelector)) {
        ...
        const fitZoom = (targetW, targetH) => parseFloat(Math.max(MIN_ZOOM, Math.min(availableW / Math.max(1, targetW), availableH / Math.max(1, targetH + SCALE_FIT_BUFFER), MAX_ZOOM)).toFixed(4));
...
function getRenderablePanelsForScaling(panels, layoutSelector) {
    ...
    if (!identity || identity.h >= PLAYER_IDENTITY_MIN_ROWS)
        return renderPanels;
    return renderPanels.map((panel) => (panel.id === "identity"
