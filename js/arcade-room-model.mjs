...
function createScreenTexture(THREE) {
    return canvasTexture(THREE, 384, 288, (context) => {
        ...
        for (let x = 0; x < 384; x += 24)
            context.fillRect(x, 216 + (x % 48 ? 4 : 0), 16, 10);
...
function createSideArtTexture(THREE) {
...
export function createBirdDutyCabinet(THREE, definition) {
    ...
    const marquee = new THREE.Mesh(new THREE.PlaneGeometry(0.77, 0.225), new THREE.MeshStandardMaterial({ map: createMarqueeTexture(THREE), emissive: "#78cfff", emissiveIntensity: 0.62, roughness: 0.35 }));
    marquee.name = "marquee";
    marquee.position.set(0, 1.84, 0.262);
    root.add(marquee);
    addBox(THREE, root, "screen-bezel", [0.73, 0.53, 0.055], [0, 1.47, 0.225], black);
    const screen = new THREE.Mesh(new THREE.PlaneGeometry(0.625, 0.42), new THREE.MeshStandardMaterial({ map: createScreenTexture(THREE), emissive: "#4caedd", emissiveIntensity: 0.48, roughness: 0.22 }));
    screen.name = "screen";
    ...
    for (const side of [-1, 1]) {
        const sideArt = new THREE.Mesh(new THREE.PlaneGeometry(0.62, 1.54), new THREE.MeshStandardMaterial({ map: sideTexture, roughness: 0.62 }));
