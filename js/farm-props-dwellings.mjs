...
const stone = (THREE, colors = ["#72777a", "#4f5558", "#9aa0a2"]) => farmMaterial(THREE, "fieldstone", { colors, metresPerTile: 0.65, bumpScale: 0.04 });
const straw = (THREE) => farmMaterial(THREE, "straw", { colors: ["#d7b65e", "#9b772f", "#efd988"], metresPerTile: 0.55 });
const dark = (THREE) => standard(THREE, "#161a1c", 1, 0);
function framedHut(THREE, definition, colors, raised = 0) {
    ...
    for (const side of [-1, 1])
        tbox(THREE, group, [sideWidth, wallHeight, 0.12], [side * (entrance.width / 2 + sideWidth / 2), base + wallHeight / 2, depth / 2 - 0.06], wall);
...
export function createDuckCoop(THREE, definition) {
    ...
    for (const z of [0.62, 0.82, 1.02])
        tbox(THREE, group, [0.65, 0.035, 0.045], [0, 0.2 - (z - 0.82) * 0.35, z], timber(THREE, "#6b4528"), false);
    return group;
}
export function createTreetopDen(THREE, definition) {
    ...
    for (const x of [-0.28, 0.28])
        tbox(THREE, group, [0.06, 1.25, 0.06], [x, 0.62, definition.footprint.depth / 2 + 0.18], ladder);
    for (let y = 0.18; y < 1.2; y += 0.22)
        tbox(THREE, group, [0.62, 0.045, 0.05], [0, y, definition.footprint.depth / 2 + 0.18], ladder, false);
    return group;
}
export function createBurrowLodge(THREE, definition) {
    const group = new THREE.Group();
    ...
    for (const [x, y, z, r] of [[-0.48, 0.45, -0.05, 0.72], [0.48, 0.42, -0.08, 0.68], [0, 0.7, -0.25, 0.75]])
        sphere(THREE, group, r, [x, y, z], soil);
    for (const [x, z] of [[-0.45, -0.3], [0.35, -0.45], [0.7, 0.1]])
        sphere(THREE, group, 0.24, [x, 0.75, z], grass);
    const entrance = definition.dwelling.entrance;
    ...
    for (const x of [-entrance.width / 2, entrance.width / 2])
        tcylinder(THREE, group, 0.055, 0.065, entrance.height, [x, entrance.height / 2, definition.footprint.depth / 2], frame, 7);
...
function openShade(THREE, definition, roofMaterial, postMaterial, mud = false) {
    const group = new THREE.Group();
    ...
    for (const x of [-width * 0.4, width * 0.4])
        for (const z of [-depth * 0.36, depth * 0.36])
            tbox(THREE, group, [0.14, height, 0.14], [x, height / 2, z], postMaterial);
...
export function createMudWallow(THREE, definition) {
    return openShade(THREE, definition, straw(THREE), timber(THREE, "#6e5035"), true);
}
export function createRhinoShade(THREE, definition) {
...
export function createRoostingBox(THREE, definition) {
...
function rockArch(THREE, definition, rock, glowColor) {
    const group = new THREE.Group();
    const entrance = definition.dwelling.entrance;
    ...
    for (const x of [-0.55, 0, 0.55])
        sphere(THREE, group, 0.5, [x * entrance.width, entrance.height + 0.25, front - 0.15], rock);
    for (const [x, z, r] of [[-0.9, -0.45, 0.45], [0.9, -0.35, 0.4], [-0.65, 0.25, 0.32], [0.7, 0.3, 0.36]])
        sphere(THREE, group, r, [x, r * 0.65, z], rock);
    if (glowColor) {
        const glow = new THREE.MeshStandardMaterial({ color: glowColor, emissive: glowColor, emissiveIntensity: 2.2, roughness: 0.45 });
        for (const [x, y, z] of [[-0.55, 0.7, front + 0.18], [0.62, 1.05, front + 0.12], [0.1, 1.45, front + 0.05]])
            sphere(THREE, group, 0.055, [x, y, z], glow);
    }
    return group;
}
export function createReefGrotto(THREE, definition) {
...
export function createDarkwaterCave(THREE, definition) {
    return rockArch(THREE, definition, stone(THREE, ["#343247", "#252439", "#5f5b73"]), "#67d9d0");
}
export function createJellyfishLagoon(THREE, definition) {
    const group = new THREE.Group();
    ...
    for (let index = 0; index < 12; index += 1) {
        ...
        if (Math.abs(Math.sin(angle) * radius) < definition.dwelling.entrance.width / 2 && Math.cos(angle) > 0.7)
            continue;
...
export const FARM_DWELLING_BUILDERS = Object.freeze({
