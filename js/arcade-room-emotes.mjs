...
const _protocolCheck = EMOTE_CATALOG.map((emote) => emote.id);
void _protocolCheck;
...
export function createEmoteWheel(options) {
    ...
    function steer(dx, dy) {
        if (!open || !Number.isFinite(dx) || !Number.isFinite(dy))
            return;
        steerX += dx;
        steerY += dy;
