...
export function requestedPlayerIdFromSearch(search: string): string {
  try {
...
async function run(): Promise<void> {
  ...
  const backEl = document.getElementById("achBackLink") as HTMLAnchorElement | null;
  ...
  if (backEl) backEl.href = isOwner || !playerId ? "../me/" : `../player/?id=${encodeURIComponent(playerId)}`;
  ...
  if (titleEl) titleEl.textContent = model.heading.toUpperCase();
  if (subtitleEl) subtitleEl.textContent = model.subheading;
  if (rootEl) rootEl.innerHTML = renderAchievementsPage(model);
}
...
if (typeof document !== "undefined" && document.getElementById("achPageRoot")) {
  run().catch((error) => {
    ...
    const rootEl = document.getElementById("achPageRoot");
    if (rootEl) rootEl.innerHTML = '<p class="ach-page__flash">Achievements could not be loaded right now.</p>';
  });
}
