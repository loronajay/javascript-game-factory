...
import type { AnimalPalette } from "./farm-catalog/animals.mjs";

export type PalettePixel = Readonly<{ r: number; g: number; b: number; a: number }>;
export type PaletteRamp = readonly [string, string, string];

const textureCache = new WeakMap<object, Map<string, any>>();
...
function sourceHue(r: number, g: number, b: number): number {
  const max = Math.max(r, g, b);
  const min = Math.min(r, g, b);
  const range = max - min;
  if (range === 0) return 0;
  if (max === r) return (60 * ((g - b) / range) + 360) % 360;
  if (max === g) return 60 * ((b - r) / range + 2);
  return 60 * ((r - g) / range + 4);
}

function rampIndex(r: number, g: number, b: number): number {
  const chroma = Math.max(r, g, b) - Math.min(r, g, b);
  if (chroma < 20) return 1;
  const hue = sourceHue(r, g, b);
  if (hue < 48 || hue >= 330) return 0;
  if (hue < 175) return 2;
  if (hue < 285) return 1;
  return 2;
}
...
export function recolorPalettePixel(pixel: PalettePixel, colors: PaletteRamp): PalettePixel {
  ...
  if (protectedDetail) return pixel;
...
function paletteTexture(THREE: Record<string, any>, source: any, palette: AnimalPalette): any {
  if (!source?.image || !palette.colors || typeof document === "undefined") return source;
...
export function materialForAnimalPalette(THREE: Record<string, any>, material: any, palette: AnimalPalette): any {
  const copy = material.clone();
  copy.color?.set?.("#ffffff");
  if (palette.id !== "standard") copy.map = paletteTexture(THREE, material.map, palette);
...
export function animalPaletteDisplayName(palette: AnimalPalette): string {
