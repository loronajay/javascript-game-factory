...
import { MAX_PONDS, pondRegions } from "./farm-pond.mjs";
...
function cutPondHoles(material, uniforms) {
...
export function createFarmWorld(THREE, scene) {
    ...
    let dayFog = SKY.horizon;
    let daylight = 1;
    function underwaterColour() {
        return new THREE.Color(UNDERWATER.color).multiplyScalar(0.3 + 0.7 * daylight);
    }
    function setUnderwater(next) {
        if (next === underwater)
            return;
        underwater = next;
        if (next) {
