...
export function createRemoteMotion(options = {}) {
    ...
    let speed = 0;
    let gait = "idle";
    function observe(next, at) {
        if (pose && at === poseAt && next.x === pose.x && next.z === pose.z && next.moving === pose.moving)
            return;
        if (pose && at > poseAt) {
        ...
        if (!next.moving)
            ...
            gait = "run";
        else if (speed <= walkSpeed || gait === "idle")
            gait = "walk";
    }
    function target(now) {
        if (!pose)
