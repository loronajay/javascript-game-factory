...
import type { RoomPoint } from "./arcade-room-decor-layout.mjs";

type ThreeNamespace = Record<string, any>;

export type EditorGizmos = Readonly<{
  /** Show these handles (none hides them). `normal` lifts them off the surface the item is on. */
  setHandles: (handles: readonly DecorHandle[], normal: RoomPoint) => void;
  /** The handle under a raycaster already aimed by the pointer, or undefined. */
  pick: (raycaster: any) => DecorHandle | undefined;
  /** Mark one handle as held; `null` releases it. */
  setActive: (handle: DecorHandle | null) => void;
  setGuides: (guides: readonly AlignGuide[]) => void;
  /** Re-scale the handles for this camera; call once per frame while they are visible. */
...
function sameHandle(a: DecorHandle | null, b: DecorHandle | null): boolean {
  if (!a || !b || a.kind !== b.kind) return false;
  return a.kind === "stretch" ? a.end === (b as typeof a).end : a.u === (b as typeof a).u && a.v === (b as typeof a).v;
}

export function createEditorGizmos(THREE: ThreeNamespace, scene: any): EditorGizmos {
  ...
  type Slot = { root: any; meshes: any[]; handle: DecorHandle | null };
  const slots: Slot[] = [];
  ...
  let active: DecorHandle | null = null;
  const up = new THREE.Vector3(0, 1, 0);
  const direction = new THREE.Vector3();

  function clearSlot(slot: Slot): void {
    for (const mesh of slot.meshes) slot.root.remove(mesh);
    slot.meshes = [];
    slot.handle = null;
    slot.root.visible = false;
  }

  function fill(slot: Slot, handle: DecorHandle, normal: RoomPoint): void {
    clearSlot(slot);
    const parts: any[] = [];
    const add = (geometry: any, position: readonly [number, number, number]): void => {
  ...
  function setHandles(handles: readonly DecorHandle[], normal: RoomPoint): void {
    handles.slice(0, MAX_HANDLES).forEach((handle, index) => fill(slots[index]!, handle, normal));
    for (let index = handles.length; index < MAX_HANDLES; index += 1) clearSlot(slots[index]!);
    group.visible = handles.length > 0;
    setActive(active);
  }

  function setActive(handle: DecorHandle | null): void {
    active = handle;
    for (const slot of slots) {
      const held = sameHandle(slot.handle, handle);
      for (const mesh of slot.meshes) {
        if (mesh.material === material || mesh.material === activeMaterial) mesh.material = held ? activeMaterial : material;
      }
    }
  }

  function pick(raycaster: any): DecorHandle | undefined {
  ...
  function update(camera: any): void {
  ...
  function setGuides(guides: readonly AlignGuide[]): void {
    if (!guides.length) {
      guideLines.visible = false;
      dots.visible = false;
      return;
  ...
  function dispose(): void {
    ...
    for (const geometry of [arrowShaft, arrowHead, grip, guideGeometry, dotGeometry]) geometry.dispose();
    for (const item of [material, activeMaterial, outline, guideMaterial, dotMaterial]) item.dispose();
