...
const cleanElapsed = (value: number): number => Number.isFinite(value) ? Math.max(0, value) : 0;
const tidy = (value: number): number => Number(value.toFixed(10));
...
export function advancePetLifecycle(profile: PetProfile, speciesId: string, elapsedFarmMinutes: number): PetProfile {
  ...
  if (ageDays === profile.ageDays && current === profile.size.current) return profile;
