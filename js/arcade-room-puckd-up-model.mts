...
function material(THREE: ThreeNamespace, color: string, roughness = 0.45, metalness = 0.08): any {
  return new THREE.MeshStandardMaterial({ color, roughness, metalness });
}

function emissive(THREE: ThreeNamespace, color: string, intensity = 3): any {
...
function addScoreTower(
  ...
  ] as const) box(THREE, group, `${name}-digit-${index}`, [width, height, 0.009], [x, 1.14 + y, 0.059], lit);
  parent.add(group);
}

function addChevron(THREE: ThreeNamespace, parent: any, name: string, x: number, z: number, turn: number, surface: any): void {
...
export function createPuckdUpAirHockeyTable(THREE: ThreeNamespace, definition: CabinetDefinition): any {
