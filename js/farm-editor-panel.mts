...
export function createFarmEditorPanel(elements: FarmPanelElements, actions: FarmPanelActions, options: FarmPanelOptions = {}): FarmEditorPanel {
  ...
  let catalogCategory: FarmDecorCategory | null = null;
  ...
  function buildGroundPicker(): void {
    ...
    groundBuilt = true;
    elements.groundPicker.replaceChildren(...GROUND_CATALOG.map((ground: GroundDefinition) => {
  ...
  function buildInspector(row: FarmDecorRow, definition: FarmDecorDefinition): InspectorRefs {
  ...
  function renderInspector(state: FarmPanelState): void {
    ...
    if (!inspector || inspector.instanceId !== row.instanceId) inspector = buildInspector(row, definition);
  ...
  elements.groundPicker.addEventListener("click", (event) => {
    ...
    if (swatch?.dataset.groundId) actions.setGround(swatch.dataset.groundId);
  });
