...
export function titleFromSlug(slug: unknown): string {
  return String(slug || "")
    .split("-")
    .filter(Boolean)
...
export function gameHrefFromSlug(slug: unknown = ""): string {
  const normalized = String(slug || "").trim();
  return normalized ? `../games/${encodeURIComponent(normalized)}/index.html` : "../grid.html";
}

export function gamePreviewSrcFromSlug(slug: unknown = ""): string {
  const normalized = String(slug || "").trim();
  return normalized ? `../grid-previews/${encodeURIComponent(normalized)}.png` : "";
}

export function buildProfileInitials(name: unknown): string {
...
export function formatPresenceLabel(presence: unknown): string {
...
export function buildPresenceToneClass(presence: unknown): string {
  const normalized = String(presence || "").trim().toLowerCase();
...
export function buildFavoriteGameItems(publicView: any, favoriteTitleResolver: (slug: any) => string, options: any = {}): any[] {
...
function createFriendCardItem({ title, value, meta, isPlaceholder = false, avatarSrc = "", playerId = "" }: any) {
  return {
    title,
    value,
    meta,
...
export function buildFriendItems(publicView: any, relationshipsRecord: any): any[] {
  ...
  const friendPreviewItems = resolvedSlots.friendSlots.map((friend: any) => (
    friend
      ? createFriendCardItem({
...
function createFriendNavigatorCandidate(entry: any, insertionIndex: number, options: any = {}) {
...
function createFriendNavigatorItem(candidate: any) {
...
export function buildFriendNavigatorItems(publicView: any, relationshipsRecord: any): any[] {
  const normalizedRelationships = relationshipsRecord?.playerId
    ? normalizeProfileRelationshipsRecord(relationshipsRecord)
    : normalizeProfileRelationshipsRecord({ playerId: publicView.playerId });
  ...
  const preferredNavigatorOrder = [
    ...
    ...resolvedFriendSlots.friendSlots.map((friend: any) => friend?.playerId || ""),
  ].filter(Boolean);
  const candidatesByKey = new Map<string, any>();
  let insertionIndex = 0;

  function upsertCandidate(entry: any, options: any = {}) {
  ...
  (Array.isArray(publicView?.friendsPreview) ? publicView.friendsPreview : []).forEach((friend: any) => {
    upsertCandidate(friend);
  });
  normalizedRelationships.friendPlayerIds.forEach((playerId: string) => {
    upsertCandidate({
      playerId,
...
export function filterFriendNavigatorItems(items: any = [], query: unknown = ""): any[] {
...
export function buildHeroStats(publicView: any, resolvedThoughtCount: number, metricsRecord: any, relationshipsRecord: any): any[] {
...
export function buildBadgeItems(badgeIds: any = []): any[] {
  if (Array.isArray(badgeIds) && badgeIds.length > 0) {
    return badgeIds.map((badgeId: any) => ({
