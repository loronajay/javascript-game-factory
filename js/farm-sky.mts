...
export type StarSize = "faint" | "medium" | "bright";
export type FarmStar = Readonly<{ x: number; y: number; z: number; size: StarSize; warmth: number }>;
export type UnitVector = Readonly<{ x: number; y: number; z: number }>;
export type CelestialOrbit = Readonly<{ sun: UnitVector; moon: UnitVector }>;
...
export function generateStarField(count: number, seed = 20260922): readonly FarmStar[] {
  const random = mulberry32(seed);
  const total = Math.max(0, Math.floor(Number.isFinite(count) ? count : 0));
  const stars: FarmStar[] = [];
...
export function celestialOrbit(minutes: number): CelestialOrbit {
