...
export type DecorModelSpec =
  | Readonly<{ kind: "strip"; mount?: DecorMount }>
  ...
  | Readonly<{ kind: "ceiling-light"; style: "spot" | "pendant" | "disco" | "tube" | "chandelier" | "string" | "lantern" | "track" | "blacklight" }>
  | Readonly<{ kind: "prop"; prop: "plant" | "bench" | "stool" | "floor-lamp" | "trash-can" | "stanchion" | "jukebox" | "vending" | "claw" | "pinball" | "popcorn" | "counter" | "speaker-stack" | "beanbag" | "table"
    | "round-table" | "long-table" | "coffee-table" | "high-top" | "sofa" | "armchair" | "bar" | "bookcase" | "booth" | "lockers"
    | "gumball" | "change-machine" | "prize-wheel" | "photo-booth" | "foosball" | "dance-pad" | "crates" | "barrel" | "neon-palm" | "lava-lamp" | "trophy-case" | "balloons" | "cone" | "water-cooler" }>
  | Readonly<{ kind: "wall-prop"; prop: "clock" | "shelf" | "speaker" | "tv" | "mirror" | "exit-sign" | "coat-hook"
    | "dartboard" | "extinguisher" | "cork-board" | "whiteboard" | "camera" | "ac-unit" | "window" | "trophy-shelf" | "switch" | "vent" | "sconce" }>
  /** A backlit letter board — a cinema marquee — with the player's words on it. */
  | Readonly<{ kind: "marquee"; text: string }>
  /** A painted board hung from the ceiling on two chains, with the player's words on it. */
  | Readonly<{ kind: "hanging-sign"; text: string }>
  | Readonly<{ kind: "ceiling-prop"; prop: "speaker" | "fan" | "banner" | "projector" | "vent" }>;
...
export function decorLightCount(definition: DecorDefinition, finish: DecorFinish = {}): number {
  if (!definition.light) return 0;
...
export function decorLightOffsets(count: number, width: number): number[] {
  if (count <= 1) return [0];
...
export function decorSignText(definition: DecorDefinition, finish: DecorFinish): string {
  ...
  if (model.kind !== "text-sign" && model.kind !== "marquee" && model.kind !== "hanging-sign") return "";
