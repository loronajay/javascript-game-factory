...
export const JUKEBOX_TRACKS = Object.freeze([
...
export function adjacentJukeboxTrack(id, direction) {
    const index = JUKEBOX_TRACKS.findIndex((track) => track.id === id);
    if (index < 0)
        return JUKEBOX_TRACKS[0];
...
export function jukeboxTrackUrl(track, siteRoot) {
...
export function jukeboxGain(distance, range = JUKEBOX_RANGE) {
    if (!Number.isFinite(distance) || distance <= range.near)
        return range.max;
...
export const SPEAKER_ITEM_IDS = Object.freeze(["decor.prop.speaker-stack", "decor.wall.speaker", "decor.ceiling.speaker"]);
export function isJukeboxEmitter(itemId) {
 ...
 */
export function jukeboxGainAt(listener, emitters, range = JUKEBOX_RANGE) {
    let loudest = 0;
...
export function isJukeboxCommand(value) {
    if (!value || typeof value !== "object")
        return false;
    const message = value;
    if (message.type !== JUKEBOX_MESSAGE.command)
        return false;
...
export function isJukeboxState(value) {
    if (!value || typeof value !== "object")
        return false;
    const message = value;
