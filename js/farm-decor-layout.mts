...
export type PlacementVerdict = "ok" | "outside" | "blocked" | "spawn" | "needs_pond";

/** The aquatic rows standing in a pond row: those whose centre is in its dug ellipse. */
export function pondContents(layout: FarmLayout, pond: FarmDecorRow): FarmDecorRow[] {
...
function tryPlace(layout: FarmLayout, item: FarmDecorRow, definition: FarmDecorDefinition, wanted: RoomPlacement, bounds: RoomBounds): FarmDecorResult {
  ...
  for (const row of moved.values()) {
    ...
    if (judgePlacement(placed, row.instanceId, rowDefinition, farmDecorBox(row, rowDefinition), bounds) !== "ok") return { valid: false, layout, instanceId: item.instanceId, reason: "blocked" };
  }
...
export function addFarmDecor(layout: FarmLayout, definition: FarmDecorDefinition, near: Readonly<{ x: number; z: number; rotationY?: number }>, bounds: RoomBounds = FARM_BOUNDS): FarmDecorResult {
  ...
  if (definition.aquatic) {
    ...
    const ponds = layout.decor
      ...
      .sort((a, b) => Math.hypot(a.x - near.x, a.z - near.z) - Math.hypot(b.x - near.x, b.z - near.z));
    if (!ponds.length) return { valid: false, layout, instanceId: "", reason: "needs_pond" };
    for (const pond of ponds) {
      ...
      for (let z = -halfD; z <= halfD; z += 0.25) {
        for (let x = -halfW; x <= halfW; x += 0.25) localSpots.push({ x, z });
      }
      localSpots.sort((a, b) => Math.hypot(a.x, a.z) - Math.hypot(b.x, b.z));
