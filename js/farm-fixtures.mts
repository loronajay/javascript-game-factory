...
import { roundFace, type BuildingShell } from "./farm-shell.mjs";
import type { FarmDecorDefinition } from "./farm-catalog/decor.mjs";

export type FarmFixtureKind = "solid" | "platform" | "ladder" | "seat" | "door";

/** How a door fixture hangs: on which end of its width, how far it swings (signed, three's y-rotation), from how far off, and what to call it. */
export type FarmFixtureDoor = Readonly<{
  hinge: -1 | 1;
  swing: number;
  reach: number;
  label: string;
...
export type FarmFixture = Readonly<{
  name: string;
  kind: FarmFixtureKind;
  /** Box centre and turn in the item's frame. */
  x: number;
  z: number;
  rotationY: number;
  /** The box on the ground plane, in the fixture's own frame. */
  width: number;
  depth: number;
  /** The vertical span the box occupies. A wall-height shaft is `Infinity` tall. */
  bottom: number;
  top: number;
  /** The platform a ladder climbs to; "" for every other kind. */
  climbsTo: string;
  /** The hinge of a door; null for every other kind. */
  door: FarmFixtureDoor | null;
}>;
...
type Spec = Readonly<{
  ...
  top: number;
  climbsTo?: string;
  door?: FarmFixtureDoor;
}>;

function fixture(name: string, spec: Spec): FarmFixture {
...
function ladder(name: string, x: number, z: number, rotationY: number, top: number, climbsTo: string): FarmFixture {
 ...
 */
function doorSwing(hinge: -1 | 1, toward: -1 | 1, angle = DOOR_OPEN_ANGLE): number {
...
function door(name: string, spec: Readonly<{ hingeX: number; z: number; width: number; top: number; hinge: -1 | 1; toward: -1 | 1; reach: number; label: string }>): FarmFixture {
...
function stableFixtures({ footprint, shell }: Building): FarmFixture[] {
  ...
  for (let index = 0; index <= stalls; index += 1) {
    ...
    if (index > 0 && index < stalls) fixtures.push(fixture(`stall-wall-${index}`, { x, z: -halfD + t + stallDepth / 2, width: 0.06, depth: stallDepth, top: 1.4 }));
...
function greenhouseFixtures({ footprint, shell }: Building): FarmFixture[] {
  const t = shell!.wallThickness;
  const halfW = footprint.width / 2;
...
function shedFixtures({ footprint, shell }: Building): FarmFixture[] {
  const t = shell!.wallThickness;
  const halfW = footprint.width / 2;
  const halfD = footprint.depth / 2;
  return [
...
function catwalkFixtures(building: Building, ladderFace: number, top: number, width: number): FarmFixture[] {
...
function siloFixtures(building: Building): FarmFixture[] {
  const shell = building.shell!;
...
function windmillFixtures(building: Building): FarmFixture[] {
  const shell = building.shell!;
  const face = roundFace(shell, building.footprint, 0);
  const inner = face.apothem - shell.wallThickness;
  const loftTop = 3.0;
...
function gazeboFixtures({ footprint, shell }: Building): FarmFixture[] {
...
function benchFixtures(): FarmFixture[] {
  return [fixture("seat", { kind: "seat", x: 0, z: 0.02, rotationY: 0, width: 1.6, depth: 0.5, top: 0.47 })];
}

const FIXTURES_BY_MODEL: Readonly<Record<string, (building: FarmDecorDefinition) => FarmFixture[]>> = Object.freeze({
  barn: barnFixtures,
...
const NONE: readonly FarmFixture[] = Object.freeze([]);
const cache = new Map<string, readonly FarmFixture[]>();

/** Every fixture of a catalog item, in its frame; empty for an item that is only its footprint. */
export function farmFixtures(definition: FarmDecorDefinition): readonly FarmFixture[] {
...
export function fixtureNamed(definition: FarmDecorDefinition, name: string): FarmFixture {
  const found = farmFixtures(definition).find((entry) => entry.name === name);
  if (!found) throw new Error(`${definition.id} has no fixture ${name}`);
  return found;
}

/** The models whose fixtures this file describes; a builder for one of them draws from the list. */
export function fixtureModelNames(): string[] {
  return Object.keys(FIXTURES_BY_MODEL);
}

/** A point in a fixture's frame → the item's frame (the fixture's own turn applied). */
export function fixtureLocalToItem(entry: FarmFixture, local: Readonly<{ x: number; z: number }>): Readonly<{ x: number; z: number }> {
...
export function fixtureDoorHinge(entry: FarmFixture): Readonly<{ x: number; z: number }> {
 ...
 */
export function fixtureDoorPose(entry: FarmFixture, open: boolean): Readonly<{ x: number; z: number; rotationY: number }> {
  if (!open || !entry.door) return { x: entry.x, z: entry.z, rotationY: entry.rotationY };
...
export function ladderFoot(entry: FarmFixture): Readonly<{ x: number; z: number }> {
 ...
 */
export function ladderExit(entry: FarmFixture, platform: FarmFixture): Readonly<{ x: number; z: number }> {
