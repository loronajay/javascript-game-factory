...
import { hexToHsv, hsvToHex, normalizeHex, type Hsv } from "./arcade-room-color.mjs";

export type ColorPickerOptions = Readonly<{
  presets: readonly Readonly<{ hex: string; title: string }>[];
  onPreview: (hex: string) => void;
  onCommit: (hex: string) => void;
}>;

export type ColorPicker = Readonly<{
  element: HTMLElement;
  /** Show this colour. Ignored mid-drag so the room's echo never fights the hand. */
...
export function createColorPicker(options: ColorPickerOptions): ColorPicker {
  ...
  function paint(): void {
    ...
    for (const button of presets.querySelectorAll<HTMLElement>("[data-preset-color]")) {
      button.setAttribute("aria-pressed", String(button.dataset.presetColor === value));
    }
  }

  function apply(next: Hsv, phase: "preview" | "commit"): void {
    hsv = next;
    value = hsvToHex(hsv);
    paint();
    if (phase === "preview") options.onPreview(value);
    else options.onCommit(value);
  }

  function fromPad(event: PointerEvent): Hsv {
  ...
  function fromHue(event: PointerEvent): Hsv {
    const bounds = hue.getBoundingClientRect();
    return { ...hsv, h: clamp01((event.clientX - bounds.left) / bounds.width) * 359.99 };
  }

  function track(surface: HTMLElement, name: "pad" | "hue", read: (event: PointerEvent) => Hsv): void {
  ...
  presets.addEventListener("click", (event) => {
    const button = (event.target as HTMLElement).closest<HTMLElement>("[data-preset-color]");
    const preset = button?.dataset.presetColor;
    if (preset) apply(hexToHsv(preset), "commit");
  });

  function setValue(next: string): void {
    const parsed = normalizeHex(next);
    if (!parsed || active || parsed === value) return;
