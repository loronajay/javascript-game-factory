...
export function shade(hex, amount) {
    ...
    const mix = (channel) => Math.round(amount >= 0 ? channel + (255 - channel) * amount : channel * (1 + amount)).toString(16).padStart(2, "0");
...
function wrapped(size, x, y, reach, draw) {
    const xs = [x];
    const ys = [y];
    if (x < reach)
        xs.push(x + size);
...
function grain(colour, height, x, y, w, h, along, dark, light, random, density = 1) {
...
function knot(colour, height, x, y, r, dark) {
...
const planks = (colour, height, size, colors, random) => {
    ...
    for (let row = 0; row < rows; row += 1) {
        ...
        if (random() < 0.6)
            knot(colour, height, random() * size, y + rowH * (0.3 + random() * 0.4), 3 + random() * 4, dark);
...
const battens = (colour, height, size, colors, random) => {
    ...
    for (let column = 0; column < columns; column += 1) {
        ...
        if (random() < 0.5)
            knot(colour, height, x + colW * (0.3 + random() * 0.4), random() * size, 3 + random() * 3, dark);
        ...
        if (column === 0)
            colour.fillRect(size - bw / 2, 0, bw, size);
        ...
        if (column === 0)
            height.fillRect(size - bw / 2, 0, bw, size);
...
const shingles = (colour, height, size, colors, random) => {
...
const clayTiles = (colour, height, size, colors, random) => {
    const [base, dark, light, shadow] = colors;
    fill(colour, size, shadow);
    fill(height, size, grey(60));
    const rows = 4;
...
const corrugated = (colour, height, size, colors, random) => {
...
const fieldstone = (colour, height, size, colors, random) => {
    ...
    for (let row = 0; row < rows; row += 1) {
        for (let column = 0; column < columns; column += 1) {
            ...
            const drawStone = (context, x, y, style, inset) => {
                context.fillStyle = style;
                context.beginPath();
...
const plaster = (colour, height, size, colors, random) => {
...
const brick = (colour, height, size, colors, random) => {
...
const galvanised = (colour, height, size, colors, random) => {
...
const wood = (colour, height, size, colors, random) => {
    const [base, dark, light] = colors;
    fill(colour, size, base);
    fill(height, size, grey(128));
    ...
    for (let index = 0; index < 3; index += 1)
        if (random() < 0.7)
            knot(colour, height, random() * size, random() * size, 4 + random() * 6, dark);
};
/** Colours: straw, straw dark, straw light. */
const straw = (colour, height, size, colors, random) => {
    const [base, dark, light] = colors;
...
const bark = (colour, height, size, colors, random) => {
    const [base, dark, light] = colors;
    fill(colour, size, base);
    fill(height, size, grey(128));
    colour.lineCap = "round";
...
const foliage = (colour, height, size, colors, random) => {
    const [base, dark, light] = colors;
...
const soil = (colour, height, size, colors, random) => {
    const [base, dark, light] = colors;
    fill(colour, size, base);
    fill(height, size, grey(128));
...
export const FARM_TEXTURE_KINDS = Object.freeze(Object.keys(KINDS));
/** The palette a kind draws with when the caller gives none (or gives only the first few). */
export function farmTextureColors(kind, colors = []) {
...
export function drawFarmTexture(kind, colors) {
    ...
    if (!colourContext || !heightContext)
        return null;
    let seed = 7;
    for (const char of kind + colors.join(""))
 ...
 */
export function farmMaterial(THREE, kind, options = {}) {
...
export function disposeFarmMaterials() {
 ...
 */
export function metricUvs(geometry, metresPerTile, offset = { u: 0, v: 0 }) {
    const position = geometry?.attributes?.position;
    const normal = geometry?.attributes?.normal;
    const uv = geometry?.attributes?.uv;
    if (!position || !normal || !uv || typeof uv.setXY !== "function")
        return geometry;
...
export function scaleUvs(geometry, metresPerTile, uMetres, vMetres) {
    const uv = geometry?.attributes?.uv;
    if (!uv || typeof uv.setXY !== "function")
        return geometry;
    for (let index = 0; index < uv.count; index += 1)
        uv.setXY(index, uv.getX(index) * uMetres / metresPerTile, uv.getY(index) * vMetres / metresPerTile);
    uv.needsUpdate = true;
    return geometry;
}
/** A box in a farm material: the room's `box`, with its UVs in metres so the pattern never stretches. */
export function tbox(THREE, group, size, position, material, shadow = true) {
...
export function tcylinder(THREE, group, radiusTop, radiusBottom, height, position, material, segments = 16, shadow = true) {
...
export function tsphere(THREE, group, radius, position, material, widthSegments = 18, heightSegments = 14) {
...
export function tmesh(THREE, group, geometry, material, position = [0, 0, 0], shadow = true) {
