...
export function applyCabinetOverrides(
  games: ArcadeGameEntry[],
  overrides: readonly any[] = [],
): ArcadeGameEntry[] {
  if (!Array.isArray(overrides) || overrides.length === 0) return games;

  const bySlug = new Map<string, any>();
  for (const override of overrides) {
    const slug = typeof override?.slug === "string" ? override.slug.trim() : "";
    if (slug) bySlug.set(slug, override);
  }
  if (bySlug.size === 0) return games;

  const merged: ArcadeGameEntry[] = [];
