...
function renderConversationItem(conv) {
  ...
  `;
    ...
    li.addEventListener("keydown", (e) => { if (e.key === "Enter" || e.key === " ")
        openThread(); });
    return li;
}
async function run() {
    ...
    if (!authSession?.playerId) {
        if (flashEl)
            flashEl.textContent = "Sign in to view your messages.";
        if (newBtnEl)
    ...
    if (conversations === null) {
        if (flashEl)
            flashEl.textContent = "Could not load messages. Try again later.";
        if (newBtnEl)
            newBtnEl.hidden = true;
        return;
    }
