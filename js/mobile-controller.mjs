...
function normalizeKeySpec(spec) {
    ...
    if (typeof spec === 'string')
        return CONTROL_KEY_SPECS[spec] || { key: spec, code: spec };
    return {
...
function resolveMobileControllerProfile(profileOrId) {
    if (typeof profileOrId === 'string')
        return MOBILE_CONTROL_PROFILES[profileOrId] || null;
...
function createKeyboardDispatcher({ target, KeyboardEventCtor } = {}) {
    ...
    function dispatch(type, spec) {
        ...
        if (!eventTarget || !EventCtor || !keySpec?.key || !keySpec?.code)
            return false;
...
function directionFromPoint(x, y, rect, options = {}) {
    ...
    if (distance > radius * 1.12)
        return options.activeDirection || null;
    ...
    if (activeCenter !== null && candidateCenter !== null && candidate !== options.activeDirection) {
        ...
        if (candidateError + 10 >= activeError)
            return options.activeDirection;
    }
    return candidate;
}
...
function ensureStyle(doc, profile) {
    if (doc.getElementById('mobile-controller-style'))
        return;
...
function createPad(doc, dispatcher, padConfig, placement) {
    ...
    for (const zone of zones) {
        ...
        segment.setAttribute('d', ringSegmentPath(center, center, innerRadius, outerRadius, zone.center - zone.half, zone.center + zone.half));
    ...
    function renderPadState() {
        ...
        for (const segment of segmentByDirection.values())
...
function mountMobileController(options = {}) {
    ...
    if (!win || !doc || !profile)
        return null;
    if (!options.force && !isMobileLike(win))
        return null;
    if (doc.querySelector('[data-mobile-controller-root]'))
        return null;
    ...
    if (profile.layout === 'dual-dpad') {
        ...
        if (pads[0])
            root.appendChild(createPad(doc, dispatcher, pads[0], { left: '18px', bottom: '18px' }));
        if (pads[1])
            root.appendChild(createPad(doc, dispatcher, pads[1], { right: '18px', bottom: '18px' }));
    }
    else {
...
export { CONTROL_KEY_SPECS, MOBILE_CONTROL_PROFILES, createKeyboardDispatcher, getPadZones, getDirectionKeys, isMobileLike, mountMobileController, normalizeKeySpec, resolveMobileControllerProfile, };
