...
function createTopFriendFallback(index: number) {
  return {
...
function normalizeTopFriendItems(items: any = []): any[] {
  const source = Array.isArray(items) ? items : [];
  return TOP_FRIEND_CHILD_IDS.map((_, index) => source[index] || createTopFriendFallback(index));
}

function createRankingFallback(index: number) {
  return {
...
function normalizeRankingItems(items: any = []): any[] {
  const source = Array.isArray(items) ? items : [];
  return RANKING_CHILD_IDS.map((_, index) => source[index] || createRankingFallback(index));
}

function renderPageHeader(doc: Document, model: any): void {
  ...
  const stage = doc.querySelector?.(".me-stage") as HTMLElement | null;
  const body = doc.body;
...
export function renderMeHeroCard(container: HTMLElement | null, model: any): void {
  if (!container) return;
  const el = container;

  const statsHtml = (Array.isArray(model.heroStats) ? model.heroStats : []).map((item: any) => `
  ...
  const backdrop = container.querySelector<HTMLElement>(".me-hero-card__backdrop");
  ...
  const portraitImage = container.querySelector<HTMLImageElement>(".me-hero-card__portrait-image");
  if (!portraitImage) return;

  function showFallback() {
    el.classList.add("me-hero-card--avatar-fallback");
  }

  function showImage() {
    el.classList.remove("me-hero-card--avatar-fallback");
  }
...
export function renderMeIdentityPanel(container: HTMLElement | null, model: any): void {
  if (!container) return;

  const factoryId = model.heroMeta.find((item: any) => item.label === "Factory ID")?.value || "PENDING-ID";
  const realNameValue = model.heroRealName || "Not shared";
  const pageViewCount = model.pageViewCount || "0";

  const linksHtml = model.identityLinkItems.map((item: any) => {
...
export function renderMeGalleryPanel(container: HTMLElement | null, photos: any[] = [], options: any = {}): void {
...
export function renderMeThoughtsPanel(container: HTMLElement | null, model: any, options: any = {}): void {
...
export function renderMeRankingsPanel(container: HTMLElement | null, model: any): void {
  if (!container) return;

  const rankingCards = normalizeRankingItems(model.rankingItems).map((item: any, index: number) => `
    <article
...
export function renderMeTopFriendsPanel(container: HTMLElement | null, model: any): void {
  if (!container) return;

  const friendItems = normalizeTopFriendItems(model.friendItems);
  const friendCards = friendItems.map((item: any, index: number) => {
...
export function renderMeFriendCodePanel(container: HTMLElement | null, model: any): void {
  renderFriendCodePanel(container, "Friend Code", model);
}

export function renderMeFriendsPanel(container: HTMLElement | null, model: any, options: any = {}): void {
...
export function renderMePageView(doc: Document, model: any, options: any = {}): void {
  if (typeof doc?.getElementById !== "function") return;
