...
type RoomEditorElements = PanelElements & Readonly<{
  /** The whole build-mode chrome; shown and hidden as one. It covers the viewport, so it is never the thing the view is centred around. */
  panel: HTMLElement;
  /** The catalog drawer beside the tab rail: the block that eats the most of the room. */
  drawer: HTMLElement;
...
export type RoomEditor = Readonly<{
  ...
  /**
   * Pick the house record ("" for none) and save straight away. Unlike every other
   * change it arrives from the jukebox page while the player is WALKING, with no
   * panel and no Save button in sight, so it cannot sit as "unsaved".
   */
  setDefaultTrack: (trackId: string) => Promise<void>;
...
export function createRoomEditor(options: RoomEditorOptions): RoomEditor {
  ...
  const panel = createEditorPanel(elements, {
    ...
    removeStarterNeon: () => {
      ...
      if (selection?.kind === "decor" && !next.decor.some((item) => item.instanceId === selection!.instanceId)) selection = null;
   ...
   */
  function editDecor(result: DecorResult, message: string, phase: EditPhase): void {
    if (phase === "preview") {
      if (!result.valid) {
  ...
  function resizeSelected(direction: -1 | 1, phase: EditPhase): void {
    ...
    const atLimit = (): void => setStatus(direction > 0 ? "That's as big as it goes." : "That's as small as it goes.", "error");
    if (definition.scale.enabled) {
      ...
      if (result.valid && roomLayoutsEqual(result.layout, layout)) atLimit();
      else editDecor(result, "Resized", phase);
      ...
      if (result.valid && roomLayoutsEqual(result.layout, layout)) atLimit();
      else editDecor(result, "Length changed", phase);
    } else {
  ...
  async function setDefaultTrack(trackId: string): Promise<void> {
    const result = setRoomDefaultTrack(layout, trackId);
    if (!result.valid || result.layout === layout) return;
  ...
  function resizeFromPointer(event: PointerEvent): void {
    const item = selectedDecor();
    if (!item || !handleDrag) return;
    updatePointer(event);
    if (!raycaster.ray.intersectPlane(handlePlane, planeHit)) return;
    const point = { x: planeHit.x, y: planeHit.y, z: planeHit.z };
    const result: ResizeResult = handleDrag.kind === "stretch"
  ...
  function hoverCursor(event: PointerEvent): string {
    updatePointer(event);
    const handle = gizmos.pick(raycaster);
    if (handle) return handle.kind === "stretch" ? "ew-resize" : "nwse-resize";
  ...
  canvas.addEventListener("wheel", (event) => {
    ...
    if (event.altKey && selectedDecor()) {
      ...
      wheelGestureTimer = setTimeout(() => {
        const item = selectedDecor();
        if (item) editDecor({ valid: true, layout, instanceId: item.instanceId, reason: "" }, "Resized", "commit");
      }, WHEEL_GESTURE_MS);
      return;
    }
    cancelTransition();
