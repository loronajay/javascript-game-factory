...
export function createRoomVisitors(THREE, scene) {
    ...
    function paintBubble(body, text) {
        const canvas = document.createElement("canvas");
        const context = canvas.getContext("2d");
        if (!context)
            return;
    ...
    function showCard(body, emoteId, now) {
    ...
    function say(clientId, text, now) {
    ...
    function update(dt, now, members) {
        ...
        for (const body of bodies.values()) {
            ...
            if (body.bubble.visible) {
                ...
                if (left <= 0)
                    body.bubble.visible = false;
                else
                    body.bubble.material.opacity = Math.min(1, left / BUBBLE_FADE_SECONDS);
            }
            if (body.card.visible) {
                const left = (body.cardUntil - now) / 1000;
                if (left <= 0)
                    body.card.visible = false;
                else {
    ...
    return Object.freeze({
        ...
        dispose: () => {
            for (const body of bodies.values())
                removeBody(body);
            bodies.clear();
            for (const texture of cardTextures.values())
                texture.dispose?.();
            cardTextures.clear();
            scene.remove(root);
        },
