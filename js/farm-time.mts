...
export function advanceFarmTime(minutes: number, realSeconds: number, rate = FARM_MINUTES_PER_REAL_SECOND): number {
...
export function quantizeFarmTime(minutes: number): number {
...
type Keyframe = Omit<FarmLightProfile, "phase" | "sunHeight"> & Readonly<{ minute: number }>;
...
function phaseAt(minutes: number): FarmDayPhase {
  if (minutes < 330 || minutes >= 1230) return "night";
...
export function farmLightProfile(minutes: number): FarmLightProfile {
