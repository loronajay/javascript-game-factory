...
export function decorHandles(layout, instanceId, room) {
 ...
 */
function largestFitting(min, max, fits) {
    if (max <= min)
        return fits(min) ? min : null;
    if (fits(max))
        return max;
    if (!fits(min))
        return null;
...
function placedExactly(result, centre, mount) {
    if (!result.valid)
        return false;
 ...
 */
export function stretchDecorEnd(layout, instanceId, end, point, room, catalog, snapThreshold = 0) {
    const found = findItem(layout, instanceId);
    if (!found)
 ...
 */
export function scaleDecorCorner(layout, instanceId, u, v, point, room, catalog) {
    const found = findItem(layout, instanceId);
    if (!found)
        return { valid: false, layout, instanceId, reason: "missing", guides: [] };
    const { item, definition } = found;
    ...
    const centreAt = (scale) => add(anchor, diagonal, scale / 2);
    const attempt = (scale) => {
