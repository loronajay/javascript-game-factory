...
import { buildPlayerThoughtFeed, loadThoughtFeed, publishThoughtPostWithApi, } from "../platform/thoughts/thoughts.mjs";
export function createPlayerThoughtComposerActions({ doc = globalThis.document, storage, apiClient, mediaComposer, loadCurrentProfile, getCurrentPageData, loadGallery, rerender, publishThoughtPostWithApiImpl = publishThoughtPostWithApi, loadThoughtFeedImpl = loadThoughtFeed, buildPlayerThoughtFeedImpl = buildPlayerThoughtFeed, syncThoughtPostCountWithApiImpl = syncThoughtPostCountWithApi, }) {
    ...
    async function handleSubmit(form) {
        ...
        if (photoResult.thought) {
            ...
            const updatedThoughtCount = buildPlayerThoughtFeedImpl(await loadThoughtFeedImpl(storage), currentProfile.playerId).length;
