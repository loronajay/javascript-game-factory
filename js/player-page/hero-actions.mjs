...
export function createPlayerHeroActions({ authSession = null, authSessionPlayerId = "", storage, apiClient, getCurrentPageData, setCurrentPageData, loadCurrentProfile, renderCurrent, profilePanel = null, createNotificationsApiClientImpl = createNotificationsApiClient, createFriendshipBetweenPlayersImpl = createFriendshipBetweenPlayers, removeFriendBetweenPlayersImpl = removeFriendBetweenPlayers, loadProfileRelationshipsRecordImpl = loadProfileRelationshipsRecord, saveFactoryProfileImpl = saveFactoryProfile, loadThoughtFeedImpl = loadThoughtFeed, locationObject = globalThis.location, } = {}) {
    ...
    async function handleClick(event) {
        ...
        const challengePickerToggle = target?.closest("[data-gesture-challenge]");
        ...
        const challengePickerCancel = target?.closest("[data-challenge-picker-cancel]");
        if (challengePickerCancel) {
            challengePickerOpen = false;
            await renderWithCurrentState();
            return true;
        }
        const challengeGameBtn = target?.closest("[data-challenge-game]");
        if (challengeGameBtn) {
            ...
            if (!gameSlug || !targetPlayerId || !authSession?.playerId)
                return true;
            challengeGameBtn.disabled = true;
            let challenge = false;
            try {
                const notifApi = createNotificationsApiClientImpl();
                challenge = !!(await notifApi.sendChallenge(targetPlayerId, gameSlug, gameTitle || gameSlug, currentProfile.profileName || "UNNAMED PILOT"));
            }
            catch (_error) {
                challenge = false;
            }
        ...
        const addFriendButton = target?.closest("[data-add-friend]");
        if (addFriendButton) {
            ...
            if (authSession?.playerId) {
                ...
                const requestResult = typeof notifApi.sendFriendRequestDetailed === "function"
                    ? await notifApi.sendFriendRequestDetailed(targetPlayerId, currentProfile.profileName || "UNNAMED PILOT")
                    : {
                        ok: !!(await notifApi.sendFriendRequest(targetPlayerId, currentProfile.profileName || "UNNAMED PILOT")),
                        error: "",
                        request: null,
                    };
        ...
        const unfriendButton = target?.closest("[data-unfriend]");
        if (unfriendButton) {
            ...
            const cleanedFriendsPreview = (updatedProfile.friendsPreview || []).filter((friend) => friend.playerId !== targetPlayerId);
