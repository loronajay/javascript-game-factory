...
import { incrementProfileViewCountWithApi, loadProfileMetricsRecord, } from "../platform/metrics/metrics.mjs";
import { loadPlayerPageData, loadRequestedPlayerProfile, sanitizePlayerId, } from "./loader.mjs";
...
export function renderPlayerPage(doc = globalThis.document, options = {}) {
    ...
    if (!isOwnerView && profile && !options?.disableProfileViewTracking) {
        void incrementProfileViewCountWithApi(profile.playerId, { source: "direct" }, storage, options?.apiClient || createPlatformApiClient(options));
    }
...
if (typeof doc?.getElementById === "function") {
    ...
    try {
        ...
        if (layout)
            savedLayout = normalizeLayout(layout);
    }
    catch { /* keep default */ }
