...
import { createUnlockQueue, type UnlockToastItem, type UnlockQueue } from "./unlock-queue.mjs";
...
export interface AchievementToasterOptions {
  doc?: Document | null;
  durationMs?: number;
  queue?: UnlockQueue;
...
export interface AchievementToaster {
  /** Queues every unseen unlock in `unlocked` (the server's list) for display. */
  show(gameSlug: string, gameTitle: string, unlocked: unknown): number;
  /** Currently displayed item, for tests and debugging. */
  current(): UnlockToastItem | null;
  queue: UnlockQueue;
}

function glyphFor(item: UnlockToastItem): string {
  const words = item.name.split(/\s+/).filter(Boolean);
  const initials = words.slice(0, 2).map((word) => word[0]!.toUpperCase()).join("");
  return initials || "★";
}

export function createAchievementToaster(options: AchievementToasterOptions = {}): AchievementToaster {
  ...
  let current: UnlockToastItem | null = null;
  let showing = false;
  ...
  function render(item: UnlockToastItem, layer: HTMLElement): HTMLElement {
  ...
  function pump(): void {
    ...
    if (!layer) { current = null; return; }   // no document: drop silently, never block
    showing = true;
    current = item;
