...
export interface UnlockQueue {
  /** Adds unseen unlocks; returns how many were actually queued. */
  enqueue(gameSlug: string, gameTitle: string, unlocked: unknown): number;
  next(): UnlockToastItem | null;
  size(): number;
  hasSeen(gameSlug: string, id: string): boolean;
}
