...
import { DIRECT_INTERACTION_POINTS, DIRECT_INTERACTION_WINDOW_LIMIT, DIRECT_INTERACTION_WINDOW_MS, FRIENDSHIP_CREATION_POINTS, SHARED_EVENT_POINTS, SHARED_SESSION_POINTS, } from "./relationships-schema.mjs";
import { buildDefaultProfileRelationshipsRecord, normalizePlayerIdList, normalizeProfileRelationshipsRecord, sanitizeCount, sanitizeGameSlug, sanitizePlayerId, sanitizeSingleLine, sanitizeTimestamp, } from "./relationships-normalize.mjs";
import { loadProfileRelationshipLedger, loadProfileRelationshipsRecord, saveProfileRelationshipLedger, saveProfileRelationshipsRecord, } from "./relationships-store.mjs";
...
function buildEmptyPairResult() {
    return buildPairResult(buildDefaultProfileRelationshipsRecord(""), buildDefaultProfileRelationshipsRecord(""), false, 0);
}
...
function applyRemoveFriendLocalMutations(pair) {
    ...
    if (pair.leftRecord.mainSqueezePlayerId === pair.rightPlayerId)
        pair.leftRecord.mainSqueezePlayerId = "";
    if (pair.rightRecord.mainSqueezePlayerId === pair.leftPlayerId)
        pair.rightRecord.mainSqueezePlayerId = "";
