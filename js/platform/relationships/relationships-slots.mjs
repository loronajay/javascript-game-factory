import { normalizeProfileRelationshipsRecord, sanitizeCount, sanitizeDisplayName, sanitizePlayerId, sanitizeSingleLine, } from "./relationships-normalize.mjs";
...
export function resolveProfileFriendSlots(profileView = {}, relationshipsRecord = {}) {
    ...
    if (normalizedRelationships.mainSqueezeMode === "manual" && hasManualMainSqueezeSelection) {
        mainSqueeze = takeFriendCandidateByPlayerId(candidatesByPlayerId, usedKeys, normalizedRelationships.mainSqueezePlayerId, relationshipPoints);
    }
    else if (normalizedRelationships.mainSqueezeMode !== "manual") {
        mainSqueeze = takeFriendCandidateByPlayerId(candidatesByPlayerId, usedKeys, preferredMainSqueezePlayerId || normalizedRelationships.mostPlayedWithPlayerId || normalizedRelationships.lastPlayedWithPlayerId, relationshipPoints) || takeAutomaticFriendCandidate(automaticCandidates, usedKeys, relationshipPoints);
    }
    else if (preferredMainSqueezePlayerId) {
        mainSqueeze = takeFriendCandidateByPlayerId(candidatesByPlayerId, usedKeys, preferredMainSqueezePlayerId, relationshipPoints);
    }
    const friendSlots = [];
