...
import { PROFILE_RELATIONSHIP_LEDGER_STORAGE_KEY, PROFILE_RELATIONSHIPS_STORAGE_KEY, } from "./relationships-schema.mjs";
import { buildDefaultProfileRelationshipsRecord, isPlainObject, normalizeProfileRelationshipsRecord, sanitizeCount, sanitizePlayerId, sanitizeSingleLine, sanitizeTimestamp, } from "./relationships-normalize.mjs";
...
export function loadProfileRelationshipLedger(storage = getDefaultPlatformStorage()) {
    return normalizeProfileRelationshipLedger(parseStoredRelationshipsMap(readStorageText(storage, PROFILE_RELATIONSHIP_LEDGER_STORAGE_KEY)));
}
