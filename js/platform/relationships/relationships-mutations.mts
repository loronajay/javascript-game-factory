...
interface PairRecords {
  ...
  leftRecord: ProfileRelationshipsRecord;
  rightRecord: ProfileRelationshipsRecord;
}
...
interface RemoveFriendResult {
  removed: boolean;
  leftRecord: ProfileRelationshipsRecord | null;
  rightRecord: ProfileRelationshipsRecord | null;
}

interface RelationshipsMutationApiClient {
  createFriendshipBetweenPlayers?: (...args: any[]) => Promise<any>;
  removeFriend?: (...args: any[]) => Promise<any>;
  recordSharedSessionBetweenPlayers?: (...args: any[]) => Promise<any>;
  recordSharedEventBetweenPlayers?: (...args: any[]) => Promise<any>;
  recordDirectInteractionBetweenPlayers?: (...args: any[]) => Promise<any>;
}

interface MutationOptions {
  storage?: StorageLike | null;
  apiClient?: RelationshipsMutationApiClient | null;
  createdAt?: unknown;
  sessionId?: unknown;
  gameSlug?: unknown;
  startedTogether?: unknown;
  reachedResults?: unknown;
  occurredAt?: unknown;
  eventId?: unknown;
  isLinkedEntry?: unknown;
  source?: unknown;
  thoughtId?: unknown;
  commentId?: unknown;
  sharedThoughtId?: unknown;
  reactionId?: unknown;
  windowMs?: unknown;
  windowLimit?: unknown;
}

function sortPlayerIds(leftPlayerId: unknown, rightPlayerId: unknown): string[] {
...
function buildRelationshipPairKey(leftPlayerId: unknown, rightPlayerId: unknown): string {
  const [left, right] = sortPlayerIds(leftPlayerId, rightPlayerId);
  if (!left || !right || left === right) return "";
  return `${left}::${right}`;
}

function createTimestamp(value: unknown): string {
...
function incrementRecordMapCount(record: ProfileRelationshipsRecord, field: CountMapField, playerId: unknown, amount: number): void {
...
function setRecordMapTimestamp(record: ProfileRelationshipsRecord, field: TimestampMapField, playerId: unknown, timestamp: unknown): void {
  const key = sanitizePlayerId(playerId);
...
function pushRecentlyPlayedWith(record: ProfileRelationshipsRecord, playerId: unknown): void {
  const key = sanitizePlayerId(playerId);
  if (!key) return;
...
function compareRelationshipPriority(record: ProfileRelationshipsRecord, leftPlayerId: string, rightPlayerId: string): number {
...
function reorderFriendPlayerIds(record: ProfileRelationshipsRecord): void {
...
function deriveMostPlayedWithPlayerId(record: ProfileRelationshipsRecord): string {
...
function awardFriendPoints(record: ProfileRelationshipsRecord, otherPlayerId: unknown, points: number): void {
  incrementRecordMapCount(record, "friendPointsByPlayerId", otherPlayerId, points);
}

function ensureFriendPlayer(record: ProfileRelationshipsRecord, otherPlayerId: unknown): void {
...
function buildPairResult(leftRecord: unknown, rightRecord: unknown, awarded: boolean, awardedPoints: number): PairResult {
  return {
    awarded,
    awardedPoints,
...
function buildEmptyPairResult(): PairResult {
...
function loadPairRecords(leftPlayerId: unknown, rightPlayerId: unknown, storage: StorageLike | null): PairRecords | null {
  const left = sanitizePlayerId(leftPlayerId);
  const right = sanitizePlayerId(rightPlayerId);
...
function savePairRecords(leftRecord: unknown, rightRecord: unknown, storage: StorageLike | null) {
...
function applyRemoveFriendLocalMutations(pair: PairRecords): void {
...
export async function createFriendshipBetweenPlayers(leftPlayerId: unknown, rightPlayerId: unknown, options: MutationOptions = {}): Promise<PairResult> {
...
export async function removeFriendBetweenPlayers(leftPlayerId: unknown, rightPlayerId: unknown, options: MutationOptions = {}): Promise<RemoveFriendResult> {
  const storage = options.storage || getDefaultPlatformStorage();
  const apiClient = options?.apiClient;
  const pair = loadPairRecords(leftPlayerId, rightPlayerId, storage);
...
export async function recordSharedSessionBetweenPlayers(leftPlayerId: unknown, rightPlayerId: unknown, options: MutationOptions = {}): Promise<PairResult> {
  const storage = options.storage || getDefaultPlatformStorage();
  const apiClient = options?.apiClient;
...
export async function recordSharedEventBetweenPlayers(leftPlayerId: unknown, rightPlayerId: unknown, options: MutationOptions = {}): Promise<PairResult> {
  const storage = options.storage || getDefaultPlatformStorage();
  const apiClient = options?.apiClient;
...
export async function recordDirectInteractionBetweenPlayers(leftPlayerId: unknown, rightPlayerId: unknown, options: MutationOptions = {}): Promise<PairResult> {
  const storage = options.storage || getDefaultPlatformStorage();
  const apiClient = options?.apiClient;
