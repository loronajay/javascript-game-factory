...
import {
  ...
  ONLINE_ACCOUNT_REQUIRED_MESSAGE,
} from "../api/factory-account-gate.mjs";
...
export interface OnlineAccountFeatureState {
  enabled: boolean;
  message: string;
}
...
interface FeatureControl {
  disabled?: boolean;
  title?: string;
  classList?: { toggle?(name: string, force?: boolean): void } | null;
  removeAttribute?(name: string): void;
  setAttribute?(name: string, value: string): void;
}

interface FeatureGateRoot {
  querySelectorAll?(selector: string): Iterable<FeatureControl>;
}
...
export function onlineAccountFeatureState(
  account: unknown = readFactoryAccountSession(),
  { message = ONLINE_ACCOUNT_FEATURE_MESSAGE }: { message?: string } = {},
...
export function syncOnlineAccountFeatureControls(
  root: FeatureGateRoot = document,
  {
    account = readFactoryAccountSession(),
    selector = ONLINE_ACCOUNT_FEATURE_SELECTOR,
    message = ONLINE_ACCOUNT_FEATURE_MESSAGE,
  }: SyncOptions = {},
): OnlineAccountFeatureState {
