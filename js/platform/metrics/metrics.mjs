import { getDefaultPlatformStorage, getPlatformStorageKey, readStorageText, writeStorageText, } from "../storage/storage.mjs";
...
function normalizeProfileOpenSourceKey(value) {
    ...
    return PROFILE_OPEN_SOURCE_KEYS.includes(normalized)
        ? normalized
        : "direct";
}
...
export function updateProfileMetricsRecord(playerId, updater, storage = getDefaultPlatformStorage()) {
    const normalizedPlayerId = sanitizePlayerId(playerId);
    if (!normalizedPlayerId || typeof updater !== "function")
        return null;
...
export async function syncProfileMetricsFromApi(playerId, storage = getDefaultPlatformStorage(), apiClient = createPlatformApiClient()) {
    const normalizedPlayerId = sanitizePlayerId(playerId);
...
export async function syncThoughtPostCountWithApi(playerId, thoughtPostCount, storage = getDefaultPlatformStorage(), apiClient = createPlatformApiClient()) {
    const normalizedPlayerId = sanitizePlayerId(playerId);
    if (!normalizedPlayerId)
        return null;
...
export async function incrementProfileViewCountWithApi(playerId, options = {}, storage = getDefaultPlatformStorage(), apiClient = createPlatformApiClient()) {
    const normalizedPlayerId = sanitizePlayerId(playerId);
    if (!normalizedPlayerId)
        return null;
    const isAuth = typeof apiClient?.savePlayerMetrics === "function";
