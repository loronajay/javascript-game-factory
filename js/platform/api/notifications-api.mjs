...
export function createNotificationsApiClient(options = {}) {
    ...
    return {
        ...
        async sendFriendRequestDetailed(toPlayerId, fromDisplayName = "") {
            const result = await requestEnvelope(fetchImpl, baseUrl, "/friend-requests", buildJsonOptions("POST", { toPlayerId, fromDisplayName }));
            return {
                ok: result.ok,
        ...
        async acceptFriendRequest(requestId, acceptorDisplayName = "") {
            ...
            const payload = await requestJson(fetchImpl, baseUrl, `/friend-requests/${encoded}/accept`, buildJsonOptions("POST", { acceptorDisplayName }));
            return payload?.ok === true;
        },
        async rejectFriendRequest(requestId) {
            const encoded = encodeURIComponent(requestId);
            const payload = await requestJson(fetchImpl, baseUrl, `/friend-requests/${encoded}/reject`, buildJsonOptions("POST", {}));
            return payload?.ok === true;
        },
        async sendChallenge(toPlayerId, gameSlug, gameTitle, fromDisplayName = "") {
            const payload = await requestJson(fetchImpl, baseUrl, "/challenges", buildJsonOptions("POST", { toPlayerId, gameSlug, gameTitle, fromDisplayName }));
            return payload?.challenge || null;
        },
        async acceptChallenge(challengeId) {
            const encoded = encodeURIComponent(challengeId);
            const payload = await requestJson(fetchImpl, baseUrl, `/challenges/${encoded}/accept`, buildJsonOptions("POST", {}));
            return payload?.ok === true;
        },
        async declineChallenge(challengeId) {
            const encoded = encodeURIComponent(challengeId);
            const payload = await requestJson(fetchImpl, baseUrl, `/challenges/${encoded}/decline`, buildJsonOptions("POST", {}));
            return payload?.ok === true;
        },
        async sendGesture(toPlayerId, gestureType, fromDisplayName = "") {
            const encoded = encodeURIComponent(toPlayerId);
            const payload = await requestJson(fetchImpl, baseUrl, `/players/${encoded}/gesture`, buildJsonOptions("POST", { gestureType, fromDisplayName }));
            return payload?.ok === true;
        },
    };
}
