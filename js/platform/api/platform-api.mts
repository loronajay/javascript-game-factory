...
export function createPlatformApiClient(options: PlatformApiClientOptions = {}) {
  ...
  return {
     ...
     */
    async uploadRoomPoster(file: File | Blob | null) {
      if (!fetchImpl || !baseUrl || !file) return null;
      const formData = new FormData();
      formData.append("file", file);
      try {
    ...
    updateGameRating(gameSlug: string, { opponentPlayerId, outcome, sessionId, progression, ranked }: { opponentPlayerId?: unknown; outcome?: unknown; sessionId?: unknown; progression?: unknown; ranked?: unknown }) {
    ...
    // opponent's mastery.
    getGameProgression(gameSlug: string, playerId: string) {
      const gs = encodePathSegment(gameSlug);
      const pid = encodePathSegment(playerId);
    ...
    activateGameConsumable(gameSlug: string, { itemId, activationId }: { itemId?: unknown; activationId?: unknown }) {
      const encoded = encodePathSegment(gameSlug);
      return encoded
        ? post(`/game-progress/${encoded}/consumables/activate`, { itemId, activationId })
        : Promise.resolve(null);
    },
    redeemGameSkinVoucher(gameSlug: string, { entitlementId, redemptionId }: { entitlementId?: unknown; redemptionId?: unknown }) {
      const encoded = encodePathSegment(gameSlug);
      return encoded
        ? post(`/game-progress/${encoded}/vouchers/redeem`, { entitlementId, redemptionId }).then((result: any) => (
    ...
    createCalendarCheckoutSession({ quantity }: { quantity?: unknown } = {}) {
    ...
    fulfillCalendarCheckout({ sessionId }: { sessionId?: unknown } = {}) {
    ...
    claimGameTournamentRound(gameSlug: string, { eventId, roundIndex, bowlerSlug }: { eventId?: unknown; roundIndex?: unknown; bowlerSlug?: unknown }) {
      const encoded = encodePathSegment(gameSlug);
    ...
    fetchLadder(gameSlug: string, { limit }: { limit?: number } = {}) {
      const gs = encodePathSegment(gameSlug);
      if (!gs) return Promise.resolve(null);
      const query = limit ? `?limit=${encodeURIComponent(String(limit))}` : "";
      return get(`/ladders/${gs}${query}`, "ladder");
    },
    fetchPlayerLadderPlacements(playerId: string, { limit }: { limit?: number } = {}) {
      const pid = encodePathSegment(playerId);
    ...
    sendRankedHeartbeat(gameSlug: string, { matchId }: any = {}, options: RequestInit = {}) {
      const gs = encodePathSegment(gameSlug);
    ...
    fetchRankedMatches(gameSlug: string, playerId: string, limit?: number) {
      const gs = encodePathSegment(gameSlug);
      const pid = encodePathSegment(playerId);
      if (!gs || !pid) return Promise.resolve(null);
    ...
    fetchRankedMatchDetail(gameSlug: string, matchId: string, { perspective }: any = {}) {
      const gs = encodePathSegment(gameSlug);
      const mid = encodePathSegment(matchId);
      if (!gs || !mid) return Promise.resolve(null);
    ...
    fetchGamePlayerRelationship(gameSlug: string, playerId: string) {
      const gs = encodePathSegment(gameSlug);
      const pid = encodePathSegment(playerId);
      return gs && pid ? get(`/games/${gs}/social/relationship/${pid}`, "relationship") : Promise.resolve(null);
    },
    fetchGamePlayerBadges(gameSlug: string, playerId: string) {
      const gs = encodePathSegment(gameSlug);
      const pid = encodePathSegment(playerId);
