...
export async function syncThoughtFeedFromApi(storage = getDefaultPlatformStorage(), apiClient = createPlatformApiClient(), viewerPlayerId = "") {
...
export async function syncThoughtCommentsFromApi(thoughtId, storage = getDefaultPlatformStorage(), apiClient = createPlatformApiClient()) {
...
export async function publishThoughtPostWithApi(post, storage = getDefaultPlatformStorage(), options = {}) {
    ...
    if (!candidate.subject && !candidate.text && !candidate.imageUrl)
        return null;
...
export async function deleteThoughtPostWithApi(id, storage = getDefaultPlatformStorage(), options = {}) {
    const apiClient = options?.apiClient || createPlatformApiClient(options);
...
export async function shareThoughtPostWithApi(thoughtId, viewerActor, storage = getDefaultPlatformStorage(), options = {}) {
    const actor = normalizeThoughtShareActor(viewerActor);
    const apiClient = options?.apiClient || createPlatformApiClient(options);
    ...
    // Auth: API first
    if (!sanitizeThoughtShareId(thoughtId) || !actor.playerId || !actor.authorDisplayName)
        return null;
    const remoteShare = await apiClient
        .shareThought(thoughtId, actor.playerId, actor.authorDisplayName, { caption: sanitizeTextBlock(options?.caption, 500) })
        .catch(() => null);
    ...
    // Update local cache from API response
    cacheThoughtPosts(storage, [resolvedShare.originalThought, resolvedShare.sharedThought].filter(Boolean), { removeIds: [resolvedShare.removedSharedThoughtId].filter(Boolean) });
...
export async function reactToThoughtPostWithApi(thoughtId, viewerPlayerId, reactionId, storage = getDefaultPlatformStorage(), options = {}) {
    const apiClient = options?.apiClient || createPlatformApiClient(options);
