import { THOUGHT_REACTION_IDS, THOUGHT_REACTION_ID_SET, THOUGHT_REACTION_LABELS, THOUGHT_REACTION_GLYPHS, THOUGHT_VISIBILITIES, } from "./thoughts-schema.mjs";
...
export function sanitizeReactionTotals(value) {
    ...
    return Object.entries(value).reduce((totals, [key, count]) => {
        ...
        if (!THOUGHT_REACTION_ID_SET.has(normalizedKey))
            return totals;
...
export function normalizeThoughtShareActor(rawActor = {}) {
...
export function normalizeThoughtPost(rawPost = {}, index = 0) {
    const post = rawPost;
...
export function normalizeThoughtComment(rawComment = {}, index = 0) {
