...
export const PROFILE_LINK_KIND_VALUES = ["external", "social", "support", "portfolio"];
const PROFILE_LINK_KINDS = new Set(PROFILE_LINK_KIND_VALUES);
export const PROFILE_PRESENCE_VALUES = ["online", "away", "busy", "offline"];
const PROFILE_PRESENCES = new Set(PROFILE_PRESENCE_VALUES);
...
function normalizeLadderPlacements(value) {
    ...
    value.forEach((entry) => {
        ...
        if (!placement || seen.has(placement.gameSlug))
            return;
...
export function normalizeProfileMusicPlaylist(value) {
    ...
    const normalized = [];
    for (const entry of value) {
        if (normalized.length >= MUSIC_PLAYLIST_MAX)
            break;
...
function resolveNestedAvatar(entry, resolver) {
    if (!entry)
        return entry;
