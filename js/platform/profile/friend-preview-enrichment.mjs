...
export async function enrichProfileFriendPreviewsFromApi(profile, relationshipsRecord, apiClient) {
    ...
    if (!loadPlayerProfile)
        return profile;
    const normalizedRelationships = normalizeProfileRelationshipsRecord(relationshipsRecord?.playerId ? relationshipsRecord : { playerId: profile?.playerId || "" });
    const existingPreview = Array.isArray(profile?.friendsPreview) ? profile.friendsPreview : [];
    const previewById = new Map(existingPreview
    ...
    const fetchedById = new Map(fetchedProfiles
    ...
    const nextMainSqueeze = mainSqueezeId
        ? (() => {
            ...
            if (!fetchedMainSqueeze)
                return profile?.mainSqueeze || null;
