...
if (typeof doc?.getElementById === "function") {
    ...
    if (!session?.ok || !session?.playerId) {
        ...
        doc.getElementById("meDeleteAccountBtn")?.addEventListener("click", async () => {
            ...
            if (!confirm("Delete your account permanently? All your data will be removed and cannot be recovered."))
                return;
            if (btn) {
                btn.disabled = true;
            ...
            if (!result?.ok) {
                if (flashEl)
                    flashEl.textContent = "Could not delete account. Try again.";
                if (btn) {
                    btn.disabled = false;
