...
export function mountPlayerController(container, tracks, uid, doc) {
    ...
    function setReelSpinning(spinning) {
        reelEls.forEach((r) => {
            if (spinning)
                r.classList.add("cassette-player__reel--spinning");
            else
                r.classList.remove("cassette-player__reel--spinning");
        });
    }
    function updateDisplay() {
        ...
        if (indexEl)
            indexEl.textContent = tracks.length > 1 ? `${currentIndex + 1} / ${tracks.length}` : "";
