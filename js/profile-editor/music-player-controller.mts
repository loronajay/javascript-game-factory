...
export function mountPlayerController(container: HTMLElement, tracks: PlayerTrack[], uid: string, doc: Document): PlayerController {
  ...
  const volSlider = doc.getElementById(`${uid}-volume`) as HTMLInputElement | null;

  const reelEls = container.querySelectorAll(".cassette-player__reel");

  function setReelSpinning(spinning: boolean) {
    reelEls.forEach((r) => {
