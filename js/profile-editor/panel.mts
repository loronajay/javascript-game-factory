...
function byId<T extends HTMLElement = HTMLElement>(doc: DocLike, id: string): T | null {
  return (doc?.getElementById?.(id) as T | null) ?? null;
}

interface InitProfileEditorPanelArgs {
  doc?: Document;
...
function uploadErrorMessage(errorCode: unknown, fileLabel: string): string {
...
function dispatchProfileUpdatedEvent(doc: Document, action: string, profile: any): void {
  ...
  const event = typeof globalThis.CustomEvent === "function"
    ...
    : ({ type: PROFILE_UPDATED_EVENT, detail } as unknown as Event);
  doc.dispatchEvent(event as Event);
}

function renderSelectOptions(select: HTMLSelectElement | null, options: ReadonlyArray<{ value: string; label: string }>, value: string): void {
  if (!select) return;
...
export function initProfileEditorPanel({
  ...
  const form = byId<HTMLFormElement>(doc, "playerProfileForm");
  const profileNameInput = byId<HTMLInputElement>(doc, "playerProfileName");
  const realNameInput = byId<HTMLInputElement>(doc, "playerProfileRealName");
  const bioInput = byId<HTMLTextAreaElement>(doc, "playerProfileBio");
  const taglineInput = byId<HTMLInputElement>(doc, "playerProfileTagline");
  const favoriteGameInput = byId<HTMLSelectElement>(doc, "playerProfileFavoriteGame");
  const mainSqueezeModeInput = byId<HTMLSelectElement>(doc, "playerProfileMainSqueezeMode");
  const mainSqueezePlayerIdInput = byId<HTMLSelectElement>(doc, "playerProfileMainSqueezePlayerId");
  const friendRailModeInput = byId<HTMLSelectElement>(doc, "playerProfileFriendRailMode");
  const discoverableInput = byId<HTMLInputElement>(doc, "playerProfileDiscoverable");
  const clearButton = byId(doc, "playerProfileClear");
  const avatarButton = byId(doc, "playerProfileAvatarButton");
  const avatarInput = byId<HTMLInputElement>(doc, "playerProfileAvatarInput");
  const avatarPreview = byId<HTMLImageElement>(doc, "playerProfileAvatarPreview");
  ...
  const bgInput = byId<HTMLInputElement>(doc, "playerProfileBgInput");
  const bgPreview = byId<HTMLImageElement>(doc, "playerProfileBgPreview");
  const bgFallback = byId(doc, "playerProfileBgFallback");
  const bgStatus = byId(doc, "playerProfileBgStatus");
  const bgStyleInput = byId<HTMLInputElement>(doc, "playerProfileBgStyle");
  ...
  function syncAvatarPreview(src: string, initials = "") {
  ...
  function setAvatarStatus(text: string) {
    if (avatarStatus) avatarStatus.textContent = text;
  }

  function syncBgPreview(src: string) {
    if (!bgPreview) return;
    if (src) {
      bgPreview.src = src;
  ...
  function setBgStatus(text: string) {
  ...
  const friendSlotInputs = Array.from({ length: FRIEND_RAIL_SLOT_COUNT }, (_, index) => (
    byId<HTMLSelectElement>(doc, `playerProfileFriendSlot${index + 1}`)
  ));
  ...
  function render(flashMessage = "") {
    ...
    model.linkRows.forEach((row, index) => {
      const labelInput = byId<HTMLInputElement>(doc, `playerProfileLinkLabel${index + 1}`);
      const urlInput = byId<HTMLInputElement>(doc, `playerProfileLinkUrl${index + 1}`);
      const kindInput = byId<HTMLSelectElement>(doc, `playerProfileLinkKind${index + 1}`);
      const idInput = byId<HTMLInputElement>(doc, `playerProfileLinkId${index + 1}`);
