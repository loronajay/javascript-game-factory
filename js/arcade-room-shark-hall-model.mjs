...
export const SHARK_HALL_RACK = Object.freeze([0, 1, 2, 3, 4].flatMap((row) => Array.from({ length: row + 1 }, (_, index) => Object.freeze({
    x: 0.48 + row * BALL_RADIUS * 1.78,
    z: (index - row / 2) * BALL_RADIUS * 2.04,
    color: BALL_COLORS[(row * (row + 1)) / 2 + index],
}))));
function material(THREE, color, roughness = 0.52, metalness = 0.06) {
...
function addBall(THREE, parent, name, x, z, color) {
    const ball = new THREE.Mesh(new THREE.SphereGeometry(BALL_RADIUS, 18, 12), material(THREE, color, 0.2, 0.02));
    ball.name = name;
...
export function createSharkHallPoolTable(THREE, definition) {
    ...
    for (const [x, z] of [[-1.19, -0.61], [-1.19, 0.61], [1.19, -0.61], [1.19, 0.61]]) {
