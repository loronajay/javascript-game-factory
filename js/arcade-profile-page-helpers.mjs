...
import { normalizeProfileRelationshipsRecord, resolveProfileFriendSlots, } from "./platform/relationships/relationships.mjs";
...
function buildRankingMeta(placement) {
    const parts = [];
...
export function buildFriendItems(publicView, relationshipsRecord) {
    ...
    const friendPreviewItems = resolvedSlots.friendSlots.map((friend) => (friend
        ? createFriendCardItem({
...
export function buildFriendNavigatorItems(publicView, relationshipsRecord) {
    ...
    function upsertCandidate(entry, options = {}) {
        ...
        if (existing) {
            ...
            if (!existing.avatarUrl && candidate.avatarUrl)
