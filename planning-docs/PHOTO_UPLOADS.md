...
- Pass 4: reaction/comment counts on grid cards + viewer selected-reaction state polish - **complete (2026-05-02)**
- Pass 5: albums
---
...
- `/me` gallery wiring: `js/me-page/wire.mjs` — hardcodes `isOwner: true`, always owner mode
...
- ~~Prevent double-submit on gallery upload.~~ **Already done.** `js/me-page/wire.mjs` and `js/player-page/media-actions.mjs` both check `galleryUploadState?.isUploading` and early-return. `social-view.mjs` renders all gallery form inputs with `disabled` and swaps the label to “Uploading...” when that flag is set.
