...
- profile music: a 5-track playlist that autoplays on profile page load, in the Myspace tradition ✓ shipped
...
### ✓ Profile Music — Shipped

`profileMusicPlaylist` (up to 5 tracks, each `{ url, title, artist }`) stored as JSONB on `player_profiles`. Drag-to-reorder playlist editor in the profile panel. Autoplay mini-player on both `/me` and `/player` with prev/next/play/mute controls. Confirmed working end-to-end.

Architecture note: shared player widget lives in `js/profile-editor/music-player.mjs`; editor in `js/profile-editor/music-editor.mjs`; styles in `css/profile-music.css`. Both wire files import from `profile-editor/` — no cross-subsystem imports.
...
- Profile music richer embeds: YouTube, SoundCloud (direct audio file uploads + 5-track playlist are live; embed kinds remain deferred)
...
```js
// profileMusicPlaylist — up to 5 entries, stored as JSONB on player_profiles
[
  { url: "", title: "", artist: "" }
]
```

- Direct `<audio>` src URLs only. YouTube/SoundCloud embed kinds belong to a later pass.
- Autoplay on profile load; mini player widget is always visible so visitors can pause or mute immediately.
- Empty array when unset. Empty profiles render gracefully with no player widget shown.
- Profile editor: up to 5 slots, drag-to-reorder, per-slot upload (mp3/ogg/wav via `POST /upload/music`, 20MB cap), title and artist text fields, per-slot clear. Playlist saved as `profileMusicPlaylist` on the profile patch.
