...
import { detectMadeBasket, resolveRimContact } from "../scripts/sim/collision.js";
...
test("an over-powered shot can still bank in, but never cleanly", () => {
  // Overpowering is forgiven by the back wall rather than by the solver — the
  // ball reaches the wall above the rim and caroms down through it. Worth
  // pinning: it is the cabinet's only 'lucky' shot, and it has to stay a bank.
  const hard = shoot({ power: 1 });
  assert(hard.scored, "a hard shot off the back wall does drop");
  assert(
    hard.contacts.indexOf("wall") >= 0 || hard.contacts.indexOf("backboard") >= 0,
    "and it must have hit something on the way",
  );
  assert(hard.contacts.indexOf("score") > 0, "the make comes after the contact, not before it");
});
