  ...
  return `
    <div class="mobile-landscape-gate-card">
      ...
      <div class="mobile-landscape-gate-copy" data-mobile-gate-copy>
        Lovers Lost is built for a wide split-screen run. Rotate your device, then enter fullscreen so the full course and touch controls fit cleanly.
      </div>
...
function initMobileLandscapeGate(options = {}) {
  ...
  function update() {
    ...
    if (status.needsRotation) {
      ...
      copy.textContent = 'Lovers Lost is a split-screen runner built for a wide view. Rotate your device, then enter fullscreen for the cleanest run.';
      ...
      copy.textContent = 'Landscape is set. Fullscreen hides browser chrome so the bottom of the game and touch controls are not clipped.';
