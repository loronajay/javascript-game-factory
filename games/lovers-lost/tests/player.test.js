...
test('at speed 5 (floor): sqrt(0.5) ≈ 0.707', () => {
  assertClose(distPerFrame(5), Math.sqrt(0.5));
});

test('at speed 10 (start): sqrt(1.0) = 1.0', () => {
  assertClose(distPerFrame(10), 1.0);
});

test('at speed 20: sqrt(2.0) ≈ 1.414', () => {
  assertClose(distPerFrame(20), Math.sqrt(2));
});

test('at speed 50: sqrt(5.0) ≈ 2.236', () => {
  assertClose(distPerFrame(50), Math.sqrt(5));
});

test('floor player covers ~71% of run in 5400 frames — clearly fails at cutoff', () => {
  const floorDist = distPerFrame(5) * 5400;
  assert(floorDist < 5400 * 0.80, `floor player should cover <80% of run, got ${(floorDist/5400*100).toFixed(1)}%`);
});

test('scaling is smooth: each +1 speed gives a smaller % change at higher speeds', () => {
  const d10 = distPerFrame(10), d11 = distPerFrame(11);
  const d30 = distPerFrame(30), d31 = distPerFrame(31);
  const changeLow  = (d11 - d10) / d10;
  const changeHigh = (d31 - d30) / d30;
  assert(changeLow > changeHigh, 'scaling should taper as speed increases');
});
...
test('at chain 0, speed 10: score clamps at 0 after miss', () => {
  let p = { ...createPlayer('boy'), speed: 10 };
  p = applyMiss(p);
  assertEq(p.score, 0);
});

test('miss subtracts 150 when score is above 0', () => {
  let p = { ...createPlayer('boy'), speed: 10, score: 300 };
  p = applyMiss(p);
...
test('at speed 5: distance increases by distPerFrame(5) per frame', () => {
  const p = advanceDistance(createPlayer('boy'));
  assertClose(p.distance, distPerFrame(5));
});
...
test('advances correctly over multiple frames', () => {
  ...
  p = advanceDistance(p);
  p = advanceDistance(p);
  assertClose(p.distance, distPerFrame(5) * 3);
});
...
test('at speed 5, distance 0, elapsed 0: projects well past 90s (floor player cannot finish)', () => {
  assert(projectedFinish(createPlayer('boy'), 0) > 90, 'floor player should project past the 90s cutoff');
});

test('at speed 5, distance 2700, elapsed 45: still projects well past 90s', () => {
  let p = { ...createPlayer('boy'), distance: 2700 };
  assert(projectedFinish(p, 45) > 90, 'floor player at half-distance should still project past cutoff');
});

test('at speed 20, distance 0, elapsed 0: projects below 90s', () => {
  let p = { ...createPlayer('boy'), speed: 20 };
