...
import { createPlayer, distPerFrame, SPEED_FLOOR, RUN_DISTANCE } from './player.js';
import { createObstacle, generateWarmup, WAVE_COUNTS } from './obstacles.js';
import { OUTCOMES } from './scoring.js';
import {
  projectIncomingX,
  projectDyingGoblinX,
...
test('both players start at speed floor', () => {
  const s = createGameState('single', 1);
  assertEq(s.boy.speed, SPEED_FLOOR);
  assertEq(s.girl.speed, SPEED_FLOOR);
});
...
test('correct action at perfect timing → applyPerfect', () => {
  const p = playerAt(100);
  const obs = [createObstacle('goblin', 100, { twoPhase: true })];
  const { player, obstacles } = processAction(p, obs, 'block');
  assert(player.chain === 1, 'chain should increment on perfect');
  assertEq(obstacles.length, 1, 'phase 0 should advance to the attack phase');
});

test('correct action at good timing → applyGood', () => {
  const p = playerAt(100);
  const obs = [createObstacle('goblin', 104, { twoPhase: true })]; // delta=4, outside fireball perfect window
  const { player, obstacles } = processAction(p, obs, 'block');
  assert(player.score === 100, `good should give 100 score, got ${player.score}`);
  assertEq(obstacles.length, 1);
});

test('correct action but too late → miss', () => {
  const p = playerAt(110);
  const obs = [createObstacle('goblin', 100, { twoPhase: true })]; // delta=10, past window
  const { player, obstacles } = processAction(p, obs, 'block');
  assert(player.score < 0, 'miss should give negative score');
  assertEq(obstacles.length, 1, 'phase 0 miss should still consume into next phase result');
});

test('wrong action within window → miss', () => {
  const p = playerAt(100);
  const obs = [createObstacle('goblin', 100, { twoPhase: true })];
  const { player, obstacles } = processAction(p, obs, 'attack'); // wrong action
  assert(player.score < 0, 'wrong action should result in miss');
  assertEq(obstacles.length, 0, 'wrong raw input consumes the phase-0 obstacle');
});
...
test('action far from any obstacle (window not yet open) → no effect', () => {
  const p = playerAt(0);
  const obs = [createObstacle('goblin', 200, { twoPhase: true })]; // far ahead
  const { player, obstacles } = processAction(p, obs, 'block');
  assertEq(player.score, 0, 'should not grade obstacle that is far ahead');
  assertEq(obstacles.length, 1, 'obstacle should remain');
});

test('two-phase goblin: correct block on phase 0 → advances to phase 1', () => {
  const p = playerAt(100);
  const obs = [createObstacle('goblin', 100, { twoPhase: true })];
  const { player, obstacles } = processAction(p, obs, 'block');
  assertEq(obstacles.length, 1, 'two-phase goblin should remain after phase 0 cleared');
  assertEq(obstacles[0].phase, 1, 'should advance to phase 1');
});

test('two-phase goblin: correct attack on phase 1 → removed', () => {
  const p = playerAt(100);
  const phaseOneGoblin = { ...createObstacle('goblin', 100, { twoPhase: true }), phase: 1 };
  const { player, obstacles } = processAction(p, [phaseOneGoblin], 'attack');
  assertEq(obstacles.length, 1, 'phase 1 now waits for sword contact');
});

test('bird obstacle requires crouch', () => {
  const p = playerAt(100);
  const obs = [createObstacle('bird', 100)];
  const { player } = processAction(p, obs, 'crouch');
  assert(player.chain === 1, 'crouch on bird should be perfect');
});
...
test('successful spikes clear lingers and reports good feedback', () => {
  const before = { ...playerAt(118, 'boy'), state: 'jumping', jumpY: 21, jumpVY: 5 };
...
test('successful phase-0 goblin block reports fireball perfect feedback without death', () => {
  const before = playerAt(100, 'boy');
  const frontObs = createObstacle('goblin', 100, { twoPhase: true });
  const result = processAction(before, [frontObs], 'block');
  const summary = summarizeObstacleOutcome(before, result, frontObs);

  assertEq(summary.feedback, 'perfect');
  assertEq(summary.effectType, 'fireball');
  assertEq(summary.goblinDeath, false);
});
...
test('wrong input reports hit feedback', () => {
  const before = playerAt(100, 'boy');
  const frontObs = createObstacle('goblin', 100, { twoPhase: true });
  const result = processAction(before, [frontObs], 'attack');
  const summary = summarizeObstacleOutcome(before, result, frontObs);

  assertEq(summary.feedback, 'hit');
  assertEq(summary.hit, true);
...
test('jumping player clears spikes on contact without a fresh button press', () => {
  const jumping = { ...playerAt(118, 'boy'), state: 'jumping', jumpY: 21, jumpVY: 5 };
  const obs = [createObstacle('spikes', 100)];
...
test('jumping too low into spikes counts as a hit', () => {
  const jumping = { ...playerAt(110, 'boy'), state: 'jumping', jumpY: 19, jumpVY: 3 };
  const obs = [createObstacle('spikes', 100)];
  const result = resolveContactAction(jumping, obs, { state: 'running', actionTick: 0 });

  assert(result.player.score < 0, 'low jump should still get hit by spikes');
  assertEq(result.obstacles.length, 0);
  assertEq(result.action, 'jump');
});
...
test('running into spikes counts as a hit on contact', () => {
  ...
  assert(result.player.score < 0, 'touching spikes on the ground should hurt');
  assertEq(result.obstacles.length, 0);
...
test('arrows hit the player body when no shield is active', () => {
  ...
  assert(result.player.score < 0, 'body contact with arrows should hurt');
  assertEq(result.obstacles.length, 0);
...
test('goblin hits the player body when no sword is active', () => {
  ...
  assert(result.player.score < 0, 'body contact with a goblin should hurt');
  assertEq(result.obstacles.length, 0);
...
test('wrong active state at contact causes a hit', () => {
  ...
  assert(result.player.score < 0, 'wrong active action should hurt');
  assertEq(result.obstacles.length, 0);
});
...
test('standing into a bird counts as a hit on contact', () => {
  ...
  assert(result.player.score < 0, 'touching the bird should hurt');
  assertEq(result.obstacles.length, 0);
...
test('crouching under a bird clears once it is fully behind the player', () => {
  const player = playerAt(112, 'boy');
  const obs = [createObstacle('bird', 100)];
  const result = resolveContactAction(player, obs, { state: 'crouch', actionTick: 8 });

  assertEq(result.player.score, 100);
  assertEq(result.obstacles.length, 0);
  assertEq(result.action, 'crouch');
});
...
test('expired obstacle → miss applied, obstacle removed', () => {
  ...
  const obs = [createObstacle('arrowwall', 100)];
  const { player, obstacles } = processMissedObstacles(p, obs);
  assert(player.score < 0, 'miss should decrement score');
  assertEq(obstacles.length, 0);
});
...
test('multiple expired obstacles → all removed, all miss', () => {
  ...
  const obs = [
    ...
    createObstacle('bird', 110),
  ];
  const { player, obstacles } = processMissedObstacles(p, obs);
  assertEq(obstacles.length, 0);
  assert(player.score < -150, 'two misses should have heavy penalty');
});
...
test('processMissedObstacles clears a fully passed spike jump instead of auto-missing it', () => {
  const p = { ...playerAt(118), state: 'jumping', jumpY: 21, jumpVY: 4 };
  const obs = [createObstacle('spikes', 100)];
  const { player, obstacles } = processMissedObstacles(p, obs);

  assertEq(player.score, 100);
  assertEq(obstacles.length, 0);
});

test('classifyAutoResolvedObstacle reports a passed spike as good feedback', () => {
  const before = { ...playerAt(118, 'boy'), state: 'jumping', jumpY: 21, jumpVY: 4 };
  const result = processMissedObstacles(before, [createObstacle('spikes', 100)]);
  const outcome = classifyAutoResolvedObstacle(before, result.player, createObstacle('spikes', 100));

  assertEq(outcome.feedback, 'good');
...
test('player reaching RUN_DISTANCE gets state=finished', () => {
  const s = playingState();
  s.boy = { ...s.boy, distance: RUN_DISTANCE - 0.5, speed: 5 };
  const next = tickFrame(s);
