...
import { CONFIG, FLOOR_DEFS, HIDER_CONFIG, ROUND_CONFIG, SANITY_CONFIG, STAMINA_CONFIG, floorY, inspectionViews, keyIdForFloor, keyLabelForFloor } from './modules/game-config.js';
...
if (!window.HotelLayout || !window.HotelCollision) throw new Error('Hotel layout helpers failed to load');
...
const world = createWorld({ THREE, scene: rendering.scene, materials: rendering.materials, config: CONFIG, layout: window.HotelLayout, logic: window.HotelCollision, document, window });
...
if (hiders) monster.setPlayers(() => hiders.list());
const round = hiders ? createRound({ camera: rendering.camera, world, player, elevator, hiders, monster, logic: window.HotelRound, config: ROUND_CONFIG, document, window }) : null;
...
window.HotelPrototype = {
  ...
  soundtrack, soundEffects, avatars, sanity, stamina, menu, round, hiders, world, rendering,
  events: ['hotel:key-found', 'hotel:door-unlocked', 'hotel:secret-discovered', 'hotel:secret-opened', 'hotel:elevator-called', 'hotel:elevator-start', 'hotel:elevator-arrive', 'hotel:floor-change', 'hotel:drawer-searched', 'hotel:monster-state', 'hotel:sanity-full', 'hotel:sanity-hunt', 'hotel:round-over', 'hotel:caught'],
};
