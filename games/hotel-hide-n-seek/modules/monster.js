export function createMonster({ THREE, GLTFLoader, scene, camera, config: CONFIG, floorY, layout, world, player, logic, sanity, document, window }) {
  const { ENEMY_STATES } = logic;
  const root = new THREE.Group();
  root.name = 'The Bellhop';
  scene.add(root);
  ...
  const demonSkin = withDreadRim(new THREE.MeshStandardMaterial({ color: 0x08070a, roughness: 0.99, metalness: 0.02 }), { color: 0x5c141a, power: 2.6, strength: 0.75 });
  ...
  const eyeCore = new THREE.MeshBasicMaterial({ color: 0xff1008 });
  const eyeGlow = new THREE.MeshBasicMaterial({ color: 0xff0000, transparent: true, opacity: 0.24, depthWrite: false });
  ...
  function loadAnimatedBody() {
    ...
    }, undefined, (error) => console.warn('The Bellhop model could not load; using its shadow-form.', error));
  }
  ...
  function updateAwareness(delta) {
    ...
    if (awareness.state !== previousState) {
      ...
      if (awareness.state === ENEMY_STATES.CHASE && detectedTargetId === 'local') world.notify('THE BELLHOP HAS SEEN YOU.', 1600);
  ...
  function updateHunt() {
    ...
    if (huntZone !== target.zone || huntTargetId !== target.id) {
      ...
      if (target.id === 'local') world.notify('THE BELLHOP HAS TURNED TOWARD YOU.', 2400);
      world.emit('sanity-hunt', { id: target.id, zone: target.zone, floor: target.floor });
    } else if (arrived) {
  ...
  function updateHud() {
    ...
    document.body.classList.toggle('monster-hunt', stalking);
    monsterStatus.textContent = hunting ? 'THE BELLHOP IS IN PURSUIT' : awareness.state === ENEMY_STATES.SEARCH ? 'THE BELLHOP IS SEARCHING' : stalking ? 'THE BELLHOP IS HUNTING' : 'THE BELLHOP IS ROAMING';
  ...
  return {
    ...
    getState: () => ({ ...awareness, floor: nearestFloor(), position: root.position.clone(), routePurpose, huntZone, huntTargetId, detectedTargetId }),
  };
}
