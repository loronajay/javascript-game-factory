...
export function createAvatars({ THREE, GLTFLoader, scene, config: CONFIG, logic }) {
  ...
  function update(delta) {
    for (const avatar of avatars.values()) {
      const pose = avatar.pendingPose
        || { x: avatar.motion.position.x, y: avatar.motion.position.y, z: avatar.motion.position.z, yaw: avatar.motion.facing, crouching: avatar.motion.crouchBlend > 0.5 };
  ...
  function describe(id) {
    const avatar = avatars.get(id);
    if (!avatar) return null;
    return { id, role: avatar.role, rig: avatar.model ? 'base-character' : 'placeholder', motion: avatar.motion.motionState, speed: Number(avatar.motion.speed.toFixed(2)), position: { ...avatar.motion.position } };
  }
