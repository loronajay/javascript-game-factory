export function createFurnishings({ THREE, materials: MAT, world, keyLabelForFloor }) {
  ...
  function registerStatic(object) { let box = null; world.registerCollider(object, () => box || (box = world.aabbFromObject(object))); }
  ...
  function addDesk(parent, x, z, rotationY = 0) {
    ...
    group.position.set(x, 0, z); group.rotation.y = rotationY; parent.add(group); registerStatic(top); return group;
  }
  function addTableLamp(parent, x, z, y = 0.9) {
    ...
    const light = new THREE.PointLight(0xffddaa, 0.58, 5.5, 2); light.position.y = 0.55; group.add(base, stem, shade, light); group.position.set(x, y, z); parent.add(group); return group;
  }
  function addBed(parent, x, z, rotationY = 0) {
    ...
    group.position.set(x, 0, z); group.rotation.y = rotationY; parent.add(group); registerStatic(base); registerStatic(mattress); return group;
  }
  function addCouch(parent, x, z, rotationY = 0) {
    ...
    group.add(seat, back, arm1, arm2); group.position.set(x, 0, z); group.rotation.y = rotationY; parent.add(group); for (const part of [seat, back, arm1, arm2]) registerStatic(part); return group;
  }
  ...
  function addDresser(parent, x, z, rotationY = 0, { keyId = null, keyLabel = null, label = 'drawer' } = {}) {
    ...
    const body = new THREE.Mesh(new THREE.BoxGeometry(1.35, 0.9, 0.58), MAT.wood); body.position.y = 0.45; body.castShadow = true; body.receiveShadow = true; root.add(body); registerStatic(body);
