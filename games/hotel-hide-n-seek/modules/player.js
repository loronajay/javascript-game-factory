export function createPlayer({ THREE, camera, renderer, scene, config: CONFIG, floorY, world, elevator, controls, performance, document, window }) {
  ...
  function tryMove(dx, dz) {
    let feetY = camera.position.y - CONFIG.eyeHeight; const nextX = camera.position.x + dx; let ground = world.resolveGroundHeight(nextX, camera.position.z, feetY);
    if (ground !== null && !world.collidesAt(nextX, camera.position.z, ground)) { camera.position.x = nextX; feetY = ground; camera.position.y = ground + CONFIG.eyeHeight; }
    const nextZ = camera.position.z + dz; ground = world.resolveGroundHeight(camera.position.x, nextZ, feetY);
    if (ground !== null && !world.collidesAt(camera.position.x, nextZ, ground)) { camera.position.z = nextZ; camera.position.y = ground + CONFIG.eyeHeight; }
  }
  ...
  function refreshLocation() {
    ...
    const near = nearestFloorFromFeet(camera.position.y - CONFIG.eyeHeight);
  ...
  function update(delta, elapsed) {
    if (!world.state.isLocked) { world.promptEl.classList.remove('visible'); return; }
    const forward = (keys.KeyW || keys.ArrowUp ? 1 : 0) - (keys.KeyS || keys.ArrowDown ? 1 : 0); const strafe = (keys.KeyD || keys.ArrowRight ? 1 : 0) - (keys.KeyA || keys.ArrowLeft ? 1 : 0); const speed = keys.ShiftLeft || keys.ShiftRight ? CONFIG.sprintSpeed : CONFIG.walkSpeed; const step = speed * delta;
    ...
    if (elevator.elevator.state !== 'moving' && elevator.isPlayerInsideXZ() && Math.abs((camera.position.y - CONFIG.eyeHeight) - elevator.elevator.car.position.y) < 0.72) camera.position.y = elevator.elevator.car.position.y + CONFIG.eyeHeight;
    refreshLocation(); if (shouldScanInteractions(elapsed)) updateInteractionTarget();
  }
  setupInput(); refreshLocation();
  return { update, refreshLocation, interact };
}
