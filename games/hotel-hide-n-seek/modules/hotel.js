export function createHotel({ THREE, scene, materials: MAT, config: CONFIG, floorY, keyIdForFloor, keyLabelForFloor, floorDefs, layout, world, furnishings, elevator, performance }) {
  ...
  function createRoomDoor(parent, { x, z, width = 1.45, side = 'left', roomNumber = '', locked = false, requiredKey = null, openInitially = false }) {
    ...
    const colliderCache = performance.createInvalidatedCache(() => world.aabbFromObject(door));
    const item = { hinge, door, open: openInitially, target: 0, side, locked, roomNumber, requiredKey, colliderCache };
    const direction = side === 'left' ? -1 : 1; if (openInitially) { item.target = direction * CONFIG.doorOpenAngle; hinge.rotation.y = item.target; }
    collections.dynamicDoors.push(item); collections.roomDoors.set(roomNumber, item); world.registerCollider(door, () => colliderCache.get());
  ...
  function createSecretPanel(parent, { x, z, width = 1.15, side = 'left', id }) {
    ...
    const colliderCache = performance.createInvalidatedCache(() => world.aabbFromObject(panel));
    const item = { id, hinge, panel, side, open: false, target: 0, discovered: false, colliderCache }; const direction = side === 'left' ? -1 : 1;
    collections.dynamicDoors.push(item); collections.secretPanels.set(id, item); world.registerCollider(panel, () => colliderCache.get(), () => Math.abs(item.hinge.rotation.y) < 1.25);
  ...
  function addSecretTunnel(parent, def, roomA, roomB) {
    ...
    world.addFloor(parent, x, centerZ, 2.55, depth, def.id === 4 ? MAT.floor4 : MAT.dark); world.registerGroundRect(parent, x, centerZ, 2.55, depth); world.addCeiling(parent, x, centerZ, 2.55, depth, 2.55);
    ...
    const red = new THREE.PointLight(0x9b1d1d, 0.45, 7, 2); red.position.set(x, 2.05, centerZ); parent.add(red); const fixture = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.08, 0.18), MAT.redLight); fixture.position.set(x, 2.4, centerZ); parent.add(fixture);
  }
  function addWallLamp(parent, x, y, z, rotationY = 0) {
    ...
    const light = new THREE.PointLight(0xffe2b2, 0.38, 5.8, 2); light.position.set(x, y - 0.05, z); parent.add(light);
  }
  function addRailSegment(start, end, material) {
    ...
    const rail = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, vector.length(), 8), material); rail.position.copy(a).add(b).multiplyScalar(0.5); rail.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), vector.normalize()); rail.castShadow = true; stairwellGroup.add(rail);
  }
  ...
  function addStairwellShell(parent, def, shell, railMat) {
    ...
    const light = new THREE.PointLight(0xf0dfbd, 0.34, 7, 2); light.position.set(6.75, 2.7, 48.9); parent.add(light);
  }
  function buildContinuousStairwell(shell, railMat) {
    ...
    for (const flight of stairLayout.flights) {
      ...
      for (let i = 0; i < flight.steps; i += 1) { const t = (i + 0.5) / flight.steps; const topT = (i + 1) / flight.steps; const tread = new THREE.Mesh(new THREE.BoxGeometry(flight.width, 0.15, run + 0.035), MAT.wood); tread.position.set(flight.startX + dx * t, flight.startY + (flight.endY - flight.startY) * topT - 0.075, flight.startZ + dz * t); tread.rotation.y = rotation; stairwellGroup.add(tread); }
  ...
  function addElevatorHall(parent, def) {
    ...
    const doors = { group, left, right, amount: 0 }; collections.hallElevatorDoors.set(def.id, doors); world.registerCollider(left, () => world.aabbFromObject(left), () => doors.amount < 0.62); world.registerCollider(right, () => world.aabbFromObject(right), () => doors.amount < 0.62);
  ...
  function buildFloor(def, shell, railMat) {
    ...
    for (let z = -35; z <= 35; z += 8) { addWallLamp(group, -3.88, 2, z, -Math.PI / 2); addWallLamp(group, 3.88, 2, z, Math.PI / 2); const light = new THREE.PointLight(def.id === 2 ? 0xd8f0ff : 0xfff0cc, 0.45, 7.2, 2); light.position.set(0, 2.86, z); group.add(light); }
    const serviceLight = new THREE.PointLight(0xf9f2dd, def.id === 4 ? 0.45 : 0.8, 8.5, 2); serviceLight.position.set(0, 2.9, 50.2); group.add(serviceLight);
  }
  function build() {
    ...
    scene.updateMatrixWorld(true); for (const item of collections.dynamicDoors) item.colliderCache.get();
  }
