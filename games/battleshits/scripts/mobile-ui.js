  ...
  return `
    <div class="mobile-landscape-gate-card">
      ...
      <div class="mobile-landscape-gate-copy" data-mobile-gate-copy>
        Battleshits plays best with both boards visible. Rotate your device, then enter fullscreen so the fleet and target boards stay easy to tap.
      </div>
...
function initMobileLandscapeGate(options = {}) {
  ...
  function update() {
    ...
    if (status.needsRotation) {
      ...
      copy.textContent = 'Battleshits needs a wide board view for clean targeting. Rotate your device, then enter fullscreen for the most playable setup.';
      ...
      copy.textContent = 'Landscape is set. Fullscreen hides browser chrome so both boards and fleet status stay easy to tap.';
