// Two-player post-match handshake. The transport owns relay messages; this module
// owns results availability, mutual consent, decline, and one synchronized start.
export function createRematchFlow({
  sendState = () => {},
  sendStart = () => {},
  isCoordinator = () => false,
  buildStart = (round) => ({ round }),
  onState = () => {},
  onAccepted = () => {},
} = {}) {
  let active = false;
  ...
  function enterResults({ round: nextRound = 0, enabled = true } = {}) {
    ...
    sendState({ round, available: true, requested: false });
    notify();
    return true;
  }
