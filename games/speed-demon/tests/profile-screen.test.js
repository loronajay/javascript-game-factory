...
import { MAX_FAVOURITES, createProfile } from "../scripts/profile/profile.js";
...
test("picking a face closes the picker; pinning a car does not", () => {
  ...
  const cars = { ...createProfileScreen(start), stage: STAGE_CARS };
  const pinned = confirmProfileScreen(cars, start);
  assertEqual(pinned.screen.stage, STAGE_CARS, "the roster stays open");
  assert(pinned.profile.favourites.includes(selectedCar(cars).id));
  // …and pressing it again takes the pin off.
  assertEqual(confirmProfileScreen(pinned.screen, pinned.profile).profile.favourites.length, 0);
});
...
test("a cursor never leaves a grid, whatever the row length", () => {
  ...
  let cars = { ...createProfileScreen(start), stage: STAGE_CARS };
  for (let i = 0; i < 40; i += 1) cars = moveProfileScreen(cars, "right");
  for (let i = 0; i < 40; i += 1) cars = moveProfileScreen(cars, "down");
  assert(selectedCar(cars), "the car cursor walked off the grid");
});
...
test("the roster view says when the list is full, and where each pin sits", () => {
  ...
  const view = viewOf({ ...createProfileScreen(full), stage: STAGE_CARS }, full);
  ...
  assertEqual(pinned.length, MAX_FAVOURITES);
  assertEqual(pinned[0].pin, 1);
});
...
test("every grid cell fits on screen and nothing collides with the scroll arrows", () => {
  ...
  for (const row of carRows()) {
    row.models.forEach((model, column) => {
      cells.push({ what: `car ${model.id}`, ...carCellRect(carRows().indexOf(row), column) });
    });
  }
  const arrows = [-1, 1].map((step) => ({ what: `scroll ${step}`, ...avatarScrollRect(step) }));

  for (const rect of [...cells, ...arrows]) {
    assert(rect.x >= 0 && rect.y >= 0, `${rect.what} starts off screen`);
    assert(rect.x + rect.width <= WORLD.width, `${rect.what} runs off the right edge`);
    assert(rect.y + rect.height <= WORLD.height, `${rect.what} runs off the bottom`);
  }
...
test("every drawn target is clickable, and reports what the cursor would take", () => {
  ...
  const cars = viewOf({ ...createProfileScreen(profile()), stage: STAGE_CARS });
