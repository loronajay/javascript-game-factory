(function attachHospitalPlan(root, factory) {
  ...
  function createHospitalPlan({ config, floorDefs = FLOOR_DEFS, layout, floorY, keyIdForFloor, keyLabelForFloor }) {
    ...
    for(const room of ROOMS){
      ...
      roomCenters.push({roomNumber:room.id,floor,x:clean(cx),z:clean(cz),side:leaf.side});
      fixtures.push({
    ...
    const navigation={
      ...
      spawnNodes:[
        {floor:1,x:-25,z:-27},{floor:1,x:0,z:27},{floor:1,x:25,z:-27},{floor:1,x:25,z:18},
        {floor:2,x:-25,z:27},{floor:2,x:0,z:-27},{floor:2,x:25,z:27},{floor:2,x:-25,z:-18}
      ],
      minSpawnSeparation:24
    };

    const spawns={
      // Free-roam review starts in the real main lobby rather than inside the elevator.
      seeker:{floor:1,x:0,z:-27,y:0},
      hiders:[
