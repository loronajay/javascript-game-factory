...
export function createRound({ camera, world, player, elevator, hiders, monster, monsters, flashlightDrops, logic, config, document, window }) {
  ...
  window.addEventListener('hotel:caught', (event) => {
    ...
    const local = logic.participant(state, SEEKER_ID);
    if (local?.alive) {
      const pose = seekerPose(); const flashlight = player.getState();
      flashlightDrops?.drop({ playerId: SEEKER_ID, ...pose, charge: flashlight.flashlightCharge });
    }
