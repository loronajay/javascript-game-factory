...
function fixture() {
    ...
    const game = createLobbyGame({ broadcast: (code, data) => messages.push(data), update: () => {}, now: () => clock,
        ...
        makeAuthority: ({ matchId, seats }) => { starts++; return { tick() {}, expire() {}, dispose() {}, input: () => false,
            snapshot: () => ({ matchId, seats, phase: 'faceoff', tick: 0 }), disconnect() {}, reconnect: () => true }; },
    });
    const lobby = { roomCode: 'ABCDE', members: new Set(['a', 'b']), settings: { protocolVersion: 2 }, status: 'open' };
...
test('both seats must opt into the supported protocol; guest can ready without host permission', () => {
    ...
    f.send('outsider', 'puck_ready', { protocolVersion: 2, ready: true }); assert.equal(f.starts(), 0);
    f.send('b', 'puck_ready', { protocolVersion: 2, ready: true }); assert.equal(f.starts(), 0);
    f.send('a', 'puck_ready', { protocolVersion: 2, ready: true }); assert.equal(f.starts(), 1);
    f.send('a', 'puck_ready', { protocolVersion: 2, ready: true }); assert.equal(f.starts(), 1);
...
test('unrecognized gameplay is never relayed, and unsupported settings cannot start', () => {
    ...
    for (const id of ['a', 'b']) f.send(id, 'puck_ready', { protocolVersion: 2, ready: true });
