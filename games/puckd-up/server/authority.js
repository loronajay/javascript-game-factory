...
import { clampTarget, EVENT_WINDOW_TICKS, INPUT_BURST, INPUT_HZ, PROTOCOL_VERSION, RECONNECT_MS } from '../scripts/online/protocol.js';
...
export function createAuthority({ CANNON, bodies, matchId, seats, colors, random = Math.random }) {
    ...
    const events = [], inputs = seats.map(() => ({ seq: 0, tick: -Infinity, target: null, credits: INPUT_BURST }));
    ...
    const emit = event => {
        ...
        if (['round-reset', 'serve', 'goal', 'match-end', 'puck-hit', 'wall-hit', 'on-fire'].includes(event.type)) {
            events.push({ ...event, id: ++eventId, tick });
    ...
    return {
        ...
        input(id, value) {
            ...
            if (disposed || seat < 0 || winner !== null || disconnected.some(Boolean) || match.state.phase !== 'live'
                ...
                || value.seq > previous.seq + 10000 || !Number.isFinite(value.x) || !Number.isFinite(value.z)) return false;
            // Token bucket: bunched commands are all applied (latest target wins
            // within a tick); a flood is what runs the budget dry.
            const credits = Math.min(INPUT_BURST, previous.credits + Math.max(0, tick - previous.tick) / TICKS_PER_INPUT);
            if (credits < 1) return false;
            Object.assign(previous, { seq: value.seq, tick, target: clampTarget(value), credits: credits - 1 });
            return true;
        },
        tick(dt = FIXED_DT) {
        ...
        snapshot() {
            ...
            return { protocolVersion: PROTOCOL_VERSION, matchId, tick, seats: [...seats], colors: [...colors],
                ...
                paddles: [simulation.bodies.player, simulation.bodies.cpu].map(m => pack(m.body)),
                events: events.filter(event => tick - event.tick <= EVENT_WINDOW_TICKS),
            };
        },
