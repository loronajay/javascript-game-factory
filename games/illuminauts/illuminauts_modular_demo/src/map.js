...
//
// Scope notes:
// - Beacon Core is a centered 5x5 room.
// - Beacon Core has exactly one entry point.
// - The only entry point is from the bottom and is blocked by a Laser Door.
// - The Access Chip is intentionally not near spawn; finishing requires route memory.
const RAW_MAP = [
  '#############################',
  '#S....#.........#...........#',
  '#####.#####.###.#########.#.#',
...
export function getPickupAt(map, x, y, type = null) {
  return map.pickups.find((pickup) => pickup.active && pickup.x === x && pickup.y === y && (!type || pickup.type === type));
}
...
function validateMap(map) {
  const chips = map.pickups.filter((pickup) => pickup.type === 'chip');
  if (chips.length < 1) {
    throw new Error('[Illuminauts map] At least one Access Chip is required.');
  }

  if (map.doors.length !== 1) {
    throw new Error(`[Illuminauts map] Expected exactly one final Laser Door, found ${map.doors.length}.`);
  }

  if (map.goals.length !== 25) {
    throw new Error(`[Illuminauts map] Beacon Core must be exactly 5x5. Found ${map.goals.length} tiles.`);
  }

  const door = map.doors[0];
  if (door.x !== 14 || door.y !== 14) {
    throw new Error(`[Illuminauts map] Final Laser Door must be centered below the Beacon Core at (14, 14). Found (${door.x}, ${door.y}).`);
  }

  const entryBelowDoor = { x: door.x, y: door.y + 1 };
  if (isWall(map, entryBelowDoor.x, entryBelowDoor.y)) {
    throw new Error('[Illuminauts map] Final Laser Door bottom approach is blocked.');
  }

  const doorGoalNeighbors = neighbors(door.x, door.y).filter((tile) => isGoalAt(map, tile.x, tile.y));
  ...
  for (const goal of map.goals) {
    for (const tile of neighbors(goal.x, goal.y)) {
      ...
      if (!isWall(map, tile.x, tile.y)) exposedGoalEdges.push(tile);
    }
  }

  if (exposedGoalEdges.length > 0) {
  ...
  const hasReachableChip = chips.some((chip) => reachableWithoutDoors.has(key(chip.x, chip.y)));
  if (!hasReachableChip) {
    throw new Error('[Illuminauts map] No Access Chip is reachable before the final Laser Door.');
  }

  if (!reachableWithoutDoors.has(key(entryBelowDoor.x, entryBelowDoor.y))) {
    throw new Error('[Illuminauts map] Player cannot reach the final Laser Door approach.');
  }
  ...
  if (!reachableWithDoor.has(key(centerGoal.x, centerGoal.y))) {
    throw new Error('[Illuminauts map] Beacon Core is not reachable after the final Laser Door opens.');
  }
}
...
function flood(map, starts, allowDoors) {
  const queue = starts.map((start) => ({ x: start.x, y: start.y }));
  const seen = new Set(queue.map((tile) => key(tile.x, tile.y)));
