  ...
  return `
    <div class="mobile-landscape-gate-card">
      ...
      <div class="mobile-landscape-gate-copy" data-mobile-gate-copy>
        Illuminauts needs a wide maze view for readable beacon runs. Rotate your device, then enter fullscreen so the touch controls and HUD stay clear.
      </div>
...
function initMobileLandscapeGate(options = {}) {
  ...
  function update() {
    ...
    if (status.needsRotation) {
      ...
      copy.textContent = 'Illuminauts needs a wide maze view for deliberate four-way movement. Rotate your device, then enter fullscreen for the cleanest run.';
      ...
      copy.textContent = 'Landscape is set. Fullscreen hides browser chrome so the maze, HUD, and touch controls stay in reach.';
