export const TILE = 32;
export const VIEW_TILES_X = 15;
export const VIEW_TILES_Y = 11;

export const HEARTS_MAX = 3;
export const STAMINA_MAX = 100;
export const WALK_STEP_MS = 155;            // ms per tile step (walk)
export const SPRINT_STEP_MS = 92;           // ms per tile step (sprint)
export const STAMINA_DRAIN_PER_STEP = 11;   // stamina cost per sprint step
export const STAMINA_RECOVER_PER_SECOND = 25;
export const INVULN_MS = 1050;
export const POWER_CELL_MS = 15000;
export const BASE_LIGHT_RADIUS = 3.1;
export const POWER_LIGHT_RADIUS = 5.2;

export const COLORS = {
  bg: '#02040a',
  floor: '#111a2f',
  floorAlt: '#0d1426',
  wall: '#273753',
  wallEdge: '#40577f',
  player: '#76f4ff',
  playerInvuln: '#ffffff',
  chip: '#ffd166',
  power: '#76f4ff',
  door: '#ff4fd8',
  doorOpen: '#263353',
  goal: '#7df29a',
  alien: '#b98cff',
  laserWarn: '#ffd166',
  laserActive: '#ff536b',
  turret: '#ff8a4a',
  fog: '#000000'
};
