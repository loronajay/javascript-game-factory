// onlineClient.js — WebSocket relay layer for Mini-Tactics 2-4 player online.
//
...
// — the same "host-nothing relay" pattern the 1v1 used, generalized to a lobby so
// 2-4 players can share one room.
//
// Authority model (see CLAUDE.md / onlineSession.js): deterministic lockstep. Every
// client builds the match from the relay-provided shared `seed` + the ordered
// `members` array `lobby_started` delivers (seat = index in members + 1, identical
// on all clients), and runs the same seeded core reducer — so dice match without
// ever being sent. The active player applies its own accepted command locally and
// broadcasts the command; every peer replays it. The lobby OWNER broadcasts its
...
// Owner-authored match framing. Every field is optional/clamped at build time
// (core normalizes board size, format, colors); here we only confirm the shape.
...
// (core/composition.js); here we only confirm the wire shape.
