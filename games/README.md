...
| `mini-hoops/` | On the grid (15) | Pull-back mini basketball: one drag sets power, aim and arc at once, solved backwards against the hoop's rest position. Four hoop motion modes, two round lengths, five rooms and three cosmetic balls, scored on local boards keyed by `mode:duration`. Strictly layered — a test enforces that the sim never touches the DOM, a store or a renderer. |
...
| `puckd-up/` | On the grid (17) | 3D first-to-seven air hockey against CPU, three difficulties and four venues. Account-backed Quick Search/private lobbies are in preview on Factory Network; online match play and durable result reporting remain gated. See its MULTIPLAYER.md. |
...
| `shark-hall/` | On the grid (18) | House 8-ball in a late-night room, vs CPU at three strengths or local hotseat. Power is stroke hold time and the contact point is a drag target on the cue ball, so follow, draw and English are real angular velocity rather than modifiers. `scripts/sim/` is pure — no THREE, no DOM, no clock, no random — and a test enforces the layering, so the physics and the rules are testable and the break is deterministic. Served from its own folder; it does not import `/js/platform/**`. |
...
| `yam-bowling/` | On the grid (14) | Character-driven arcade bowling with a 28-bowler roster, three-frame Quick Bowl and regulation Classic Ten modes, local hotseat play, two CPU difficulties, fixed-step ball and pin physics, result portraits, music, and Node regression tests. |
