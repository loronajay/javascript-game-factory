  ...
  return [...units]
    ...
    .map(
      (unit) =>
        ...
        // Mother Nature's global weather, no-repeat gate, and pending next-turn MOVE
        // charge change future costs/ranges/damage/restoration/legality.
        `${unit.weather ?? "-"},${unit.lastWeather ?? "-"},${unit.weatherMoveCharged ?? 0},` +
...
function canonicalActivation(activation) {
  ...
  return (
    ...
    `${activation.spellUsed ? 1 : 0},${activation.summonerId ?? "-"},${activation.summonerArtId ?? "-"},` +
