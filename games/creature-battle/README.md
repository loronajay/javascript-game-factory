...
**Fully playable 3v3 training battle mode.**
...
- **Status effects — all 6 live**:
  ...
  - `burn` — 3-round tick (same 6% HP), also drops DEF one stage on application
  - `stun` — creature is skipped automatically in the input phase for the duration
  ...
  - Status ticks fire end-of-round before duration advance; battle-end is checked after ticks
- **Stat modifier stages**: STR/DEF/INT/SPI/SPD/ACC/EVA can be raised or lowered up to ±5 stages; stage multiplier table applies to all affected calculations
- Utility moves resolve through the status/modifier system (soak_hide raises DEF, cold_feet/snow_blind apply debuffs, verdant_guard raises SPR)

### Move system
- 55 creature moves across 4 creatures + 1 shared basic attack (56 total)
...
- **Art command menu**: 3-column grid layout; tiered abilities (e.g. Sprout Tap 1/2/3) grouped into rows with null placeholder cells padding partial tiers
...
- Multi-hit animations stagger with 100ms between each target; `multi_hit` type (Scorch) staggers float texts 140ms apart
- Float text damage numbers per creature slot (`showBattleFloatText`): `damage`, `crit`, `heal`, `miss`, `status` kinds
- **All 4 creatures fully animated** — complete CSS + JS passes for every move:
  ...
  - Flor: 14 moves (sprout tap tier, petal mend tier, thorn bind, root snare, verdant guard, toxic spores, bloom surge, pollen veil, nature's ward, world tree)
...
- Status effect cure art — poison is permanent until cured; no cure move exists yet
- HUD status badge color differentiation — all 6 statuses share the same badge style
