registerMoveAnimations({
  ...
  // ── cold_feet ──────────────────────────────────────────────────────────────
  // Slow utility. Frost forms at actor's feet before the shot; after impact
  // frost spreads DOWN across the target's position — the freeze lingers.
  cold_feet: {
    timeline: [
      { at:0,   type:'particle_stream', origin:'actor',  color:'#88ccff', count:3, interval:85, direction:'down', size:3, duration:360 },
      ...
      { at:310, type:'particle_stream', origin:'target', color:'#55aaff', count:2, interval:90, direction:'down', size:3, duration:480 },
  ...
  // ── glacier_wall ───────────────────────────────────────────────────────────
  // DEF + SPI self-buff. Ice plates form as crystals drip down over the body;
  // dual status rings signal both stat raises.
  glacier_wall: {
    timeline: [
      { at:0,   type:'particle_stream', origin:'actor',  color:'#aaddff', count:3, interval:88, direction:'down', size:3, duration:480 },
  ...
  // ── snow_blind ─────────────────────────────────────────────────────────────
  // Blind utility. Snow flurry lingers on target after hit — target stays
  // visually obscured and status ring confirms the blind.
  snow_blind: {
    timeline: [
      { at:0,   type:'sound',           id:'beam-light' },
      ...
      { at:340, type:'particle_stream', origin:'target', color:'#eef8ff', count:4, interval:80, direction:'all', spread:55, duration:520 },
  ...
  // ── whiteout ───────────────────────────────────────────────────────────────
  // Stronger blind via beam. Denser snow flurry + deeper white-out tint.
  whiteout: {
    timeline: [
      { at:0,   type:'sound',           id:'beam-light' },
      ...
      { at:260, type:'particle_stream', origin:'target', color:'#ddf8ff', count:5, interval:70, direction:'all', spread:60, glow:true, duration:560 },
  ...
  // ── ice_lock ───────────────────────────────────────────────────────────────
  // Stun utility. Ice crystals radiate and linger on target; deep dark-blue
  // tint + stun ring make the freeze status unmistakable.
  ice_lock: {
    timeline: [
      { at:0,   type:'sound',           id:'charge-light' },
      ...
      { at:280, type:'particle_stream', origin:'target', color:'#55aaff', count:4, interval:80, direction:'all', spread:48, duration:500 },
  ...
  // radiates outward from actor at the shatter point.
  shatter_chill: {
    timeline: [
  ...
  shatter_chill_3: {
    timeline: [
      ...
      { at:590, type:'particle_stream', origin:'actor',  color:'#55aaff', count:3, interval:88, direction:'all', duration:320 },
  ...
  // ── blizzard ───────────────────────────────────────────────────────────────
  // Wide AoE sweep. Distinct from shatter_chill's detonation — a wave_sweep
  // travels across the field, reading as a blizzard washing over all enemies.
  blizzard: {
    timeline: [
      { at:0,   type:'particle_stream', origin:'actor',  color:'#aaddff', count:4, interval:78, direction:'up', spread:40, duration:380, glow:true },
  ...
  // ── absolute_zero ──────────────────────────────────────────────────────────
  // Ultimate lunge. Entire 520ms charge alive with ice energy; hit_stop 85ms;
  // double shockwave cascade + lingering cold wash + deep-freeze tint on target.
  absolute_zero: {
    timeline: [
      { at:0,   type:'particle_stream', origin:'actor',  color:'#44aaff', count:5, interval:60, direction:'up', spread:38, duration:520, glow:true },
      ...
      { at:845, type:'particle_stream', origin:'actor',  color:'#44aaff', count:3, interval:88, direction:'all', spread:50, duration:380 },
