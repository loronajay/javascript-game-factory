...
export function buildDefaultFriendCode(playerId: unknown, attempt: any = 0): string {
...
function sanitizeFactoryProfileName(value: unknown): string {
...
function normalizeLadderPlacement(entry: unknown): any {
...
function normalizeLadderPlacements(value: unknown): any[] {
  if (!Array.isArray(value)) return [];
  const seen = new Set<string>();
...
function normalizeFriendPreviewEntry(entry: unknown, options: any = {}): any {
  if (!isPlainObject(entry)) return null;
...
function normalizeFriendsPreview(value: unknown): any[] {
  if (!Array.isArray(value)) return [];
  const seen = new Set<string>();
  const normalized: any[] = [];
  value.forEach((entry) => {
...
function normalizeProfileLink(link: unknown, index = 0): any {
...
function normalizeProfileLinks(links: unknown): any[] {
...
function normalizeMusicTrack(entry: unknown): any {
  if (!isPlainObject(entry)) return null;
...
function normalizeProfileMusicPlaylist(value: unknown): any[] {
  if (!Array.isArray(value)) return [];
  const normalized: any[] = [];
  for (const entry of value) {
...
function normalizeProfileFields(profile: any = {}) {
...
function normalizeSimpleStringList(value: unknown): string[] {
  if (!Array.isArray(value)) return [];
  const seen = new Set<string>();
  const normalized: string[] = [];
  for (const entry of value) {
...
export function normalizeFactoryProfile(profile: any = {}, options: any = {}) {
  const source = isPlainObject(profile) ? profile : {};
