...
function rankedAvatarIndex(avatarId: any): number {
  const match = typeof avatarId === "string" ? RANKED_AVATAR_ID_PATTERN.exec(avatarId) : null;
  return match ? Number(match[1]) : 0;
}

export function isValidRankedAvatarId(avatarId: any): boolean {
  const index = rankedAvatarIndex(avatarId);
  return index >= 1 && index <= RANKED_AVATAR_COUNT;
}

export function isFreeRankedAvatarId(avatarId: any): boolean {
  const index = rankedAvatarIndex(avatarId);
  return index >= 1 && index <= RANKED_AVATAR_FREE_COUNT;
}

export function rankedAvatarEntitlementId(avatarId: string): string {
