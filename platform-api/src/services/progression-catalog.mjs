...
export function listProgressionSlugs() {
...
export function levelFromXp(curve, xp) {
    const total = Math.max(0, safeInt(xp, 0));
    let level = 1;
    while (level < curve.maxLevel && total >= xpForLevel(curve, level + 1))
        level += 1;
...
export function computeOnlineGrant(gameSlug, { modeId, outcome = "loss", performance = 0, forfeitRole = null, } = {}) {
    ...
    if (forfeitRole === "leaver")
        return refused("left-early");
    const breakdown = { completion: 0, win: 0, performance: 0, forfeit: 0 };
    if (forfeitRole === "remaining") {
        ...
        breakdown.performance = Math.min(definition.maxPerformanceXp, Math.max(0, safeInt(performance, 0)) * definition.performanceXpPerUnit);
    }
