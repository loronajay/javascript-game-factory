...
const DECOR_PRICES = Object.freeze({
...
function surfacePrice(id) {
    if (/neon-|neon-grid|circuit/.test(id))
        return 500;
    if (/marble|starfield|galaxy-carpet/.test(id))
        return 350;
    if (/herringbone|terrazzo|hex|stripes|chevron|memphis|diamonds|tartan|wainscot/.test(id))
        return 250;
    if (/planks|brick|cinderblock|concrete|panels|diamond-plate|hazard|steel-grid/.test(id))
        return 200;
    if (/checker|carpet|tile|subway|acoustic/.test(id))
        return 150;
    return 100;
}
const items = new Map();
for (const [id, price] of Object.entries(DECOR_PRICES))
    items.set(id, Object.freeze({ id, price }));
for (const slug of [
...
export const ARCADE_ROOM_STARTER_IDS = new Set([
...
for (const id of SURFACE_IDS) {
    if (!ARCADE_ROOM_STARTER_IDS.has(id))
        items.set(id, Object.freeze({ id, price: surfacePrice(id) }));
}
export const ARCADE_ROOM_TICKET_ITEMS = Object.freeze([...items.values()]);
export const ARCADE_ROOM_CATALOG_IDS = new Set([
    ...ARCADE_ROOM_STARTER_IDS,
    ...ARCADE_ROOM_TICKET_ITEMS.map((entry) => entry.id),
]);
export function findArcadeRoomTicketItem(itemId) {
