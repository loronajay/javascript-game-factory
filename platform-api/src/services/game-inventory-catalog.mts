...
type InventoryPolicy = {
  /** Resolve an untrusted item id to its canonical id, or "" if the game cannot grant it. */
  resolveItemId: (itemId: string) => string;
  maxQuantity: number;
};

const POLICIES: Record<string, InventoryPolicy> = Object.freeze({
  "tactical-arena": Object.freeze({
    resolveItemId: (itemId: string) => getConsumableOffer(itemId)?.id || "",
  ...
  "yam-bowling": Object.freeze({
    ...
    resolveItemId: (itemId: string) => (["skin-voucher", "swimsuit-voucher", "emote-voucher"].includes(itemId) ? itemId : ""),
    maxQuantity: MAX_INVENTORY_GRANT_QUANTITY,
  }),
});
...
function policyFor(gameSlug: any): InventoryPolicy | null {
  return POLICIES[cleanText(gameSlug, 60).toLowerCase()] || null;
}

export function isGrantableInventoryItem(gameSlug: any, itemId: any): boolean {
 ...
 */
export function normalizeInventoryGrants(gameSlug: any, rows: any): { itemId: string; quantity: number }[] {
  const policy = policyFor(gameSlug);
  if (!policy || !Array.isArray(rows)) return [];
