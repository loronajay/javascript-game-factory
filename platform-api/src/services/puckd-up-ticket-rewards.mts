...
const MODES = ["cpu", "circuit", "online"] as const;
...
export interface PuckdUpResult {
  resultId: string;
...
export function normalizePuckdUpResult(raw: unknown): NormalizedResult<PuckdUpResult> {
  ...
  if (mode !== "online") {
    ...
    if (!rivalId) return refuse("invalid_rival");
  }
  ...
  if (myGoals === null || opponentGoals === null || durationMs === null) return refuse("invalid_counts");
  if (outcome === "win" ? myGoals !== target || opponentGoals >= target : opponentGoals !== target || myGoals >= target) {
    return refuse("invalid_counts");
  }
  if (durationMs < (myGoals + opponentGoals) * MIN_MS_PER_GOAL) return refuse("implausible_duration");

  return { result: { resultId, mode, rivalId, outcome, myGoals, opponentGoals, durationMs } };
}

export function calculatePuckdUpTicketReward({ result }: { result: PuckdUpResult }): TicketRewardBreakdown {
