...
const OUTCOMES = ["win", "loss", "forfeit_win"];
...
const COMPLETION = { cpu: 20, online: 25 };
const CPU_WIN_BONUS = { easy: 5, medium: 15, hard: 25 };
const ONLINE_WIN_BONUS = 20;
const ACCURACY_BONUS = 5;
const ACCURACY_LINE = 0.5;
function minCellsForSunk(count) {
    return FLEET_LENGTHS.slice(0, count).reduce((sum, length) => sum + length, 0);
}
export function normalizeBattleshitsResult(raw) {
    ...
    if (!outcome || (outcome === "forfeit_win" && mode !== "online"))
    ...
    // its cells hit.
    if ((hits === FLEET_CELLS) !== (shipsSunk === FLEET_LENGTHS.length))
        return refuse("invalid_counts");
    if (hits < minCellsForSunk(shipsSunk))
        return refuse("invalid_counts");
    const allSunk = shipsSunk === FLEET_LENGTHS.length;
    const allLost = shipsLost === FLEET_LENGTHS.length;
    if (allSunk && allLost)
        return refuse("invalid_outcome");
    if (outcome === "win" && !allSunk)
        return refuse("invalid_outcome");
    if (outcome === "loss" && !allLost)
        return refuse("invalid_outcome");
    if (outcome === "forfeit_win" && (allSunk || allLost))
        return refuse("invalid_outcome");
    if (durationMs < shots * MIN_MS_PER_SHOT)
...
export function calculateBattleshitsTicketReward({ result }) {
