...
const BY_GAME = new Map(GAMES.map((game) => [game.gameSlug, game]));
...
export function getGameBoards(gameSlug) {
 ...
 */
export function getBoard(gameSlug, boardId) {
    const game = BY_GAME.get(cleanSlug(gameSlug));
    if (!game)
        return null;
...
export function isBoardSlug(gameSlug) {
 ...
 */
export function isBetterValue(board, candidate, current) {
    if (!board)
        return false;
    if (!Number.isFinite(candidate) || !Number.isFinite(current))
        return false;
...
export function boardOrderSql(board) {
 ...
 */
export function normalizeBoardValue(board, value) {
    if (!board)
        return null;
    const number = Math.round(Number(value));
    if (!Number.isFinite(number))
        return null;
    if (number < board.min || number > board.max)
        return null;
    return number;
}
