...
export function resolveYamBowlingVoucherOffer(input: any = {}): any {
  const id = cleanText(input?.id, 120);
  const offer = YAM_BOWLING_VOUCHER_OFFERS.find((entry: any) => entry.id === id);
  if (!offer || cleanText(input?.kind, 40) !== "inventory") return stripeError(400, "offer_not_found");
  const requestedSku = cleanText(input?.sku, 200);
  if (requestedSku && requestedSku !== offer.sku) return stripeError(400, "offer_mismatch");
  return {
    ok: true,
    offer: {
      ...offer,
      inventoryItems: offer.inventoryItems.map((item: any) => ({ ...item })),
      entitlementIds: [],
...
const SKIN_PACK_PRICES: Record<string, number> = Object.freeze({
  "summer-vibes": 1499,
  halloween: 2499,
  arcane: 1999,
  "blood-moon": 4999,
...
function skuPart(value: any): string {
...
function skinSku(type: string, slug: string): string {
  return `ta.skin.${skuPart(type)}.${skuPart(slug)}`;
}

function unitSku(type: string): string {
...
function ownedEntitlements(progress: any): Set<string> {
...
function findSkinOffer(type: string, slug: string): any {
...
function skinPackName(packId: string): string {
  if (packId === "fuck-cancer") return "Fight Cancer Pack";
  return `${titleCase(packId)} Pack`;
}

function findSkinPackOffer(packId: string): any {
  const skins = SKIN_CATALOG.filter((skin) => skin.packId === packId);
  const amountCents = moneyCents(SKIN_PACK_PRICES[packId]);
  if (!skins.length || !amountCents) return null;
  return {
    kind: "skin-pack",
    packId,
...
function findUnitOffer(type: string): any {
  return UNIT_CATALOG.find((unit) => unit.type === type) || null;
}

export async function resolveTacticalArenaPremiumOffer(offerInput: any, progress: any = {}): Promise<any> {
  ...
  if (kind === "skin-pack") {
    ...
    const unownedSkins = skinPack.skins.filter((skin: any) => !owned.has(skin.entitlementId));
    if (!unownedSkins.length) {
      return stripeError(409, "offer_already_owned");
    }
    const individualPriceCents = skinPack.skins.reduce((sum: number, skin: any) => sum + moneyCents(skin.amountCents), 0);
    const unownedPriceCents = unownedSkins.reduce((sum: number, skin: any) => sum + moneyCents(skin.amountCents), 0);
    ...
    return {
      ...
      offer: {
        ...
        entitlementIds: [
          ...unownedSkins.map((skin: any) => skin.entitlementId),
...
export async function createTacticalArenaCheckoutSession(params: any = {}): Promise<any> {
  ...
  if (gameSlug === YAM_BOWLING_GAME_SLUG) {
    ...
    if (!successUrl || !cancelUrl) return stripeError(400, "invalid_return_url");
...
function parseStripeSignature(signature: string): any {
...
export function verifyStripeSignature(params: any = {}): boolean {
  ...
  return (parsed.v1 || []).some((candidate: string) => {
    const candidateBuffer = Buffer.from(candidate, "hex");
    return candidateBuffer.length === expectedBuffer.length && timingSafeEqual(candidateBuffer, expectedBuffer);
  });
}

export async function fulfillTacticalArenaCheckoutSession(params: any = {}): Promise<any> {
  ...
  if (!recordGameProgressClaim) return stripeError(503, "fulfillment_not_configured");
  ...
  if (!resolved.ok && resolved.error !== "offer_already_owned") return resolved;
...
function premiumClaimKind(offerKind: any): string {
  if (offerKind === "unit") return "premium-unit-purchase";
  if (offerKind === "consumable") return "premium-consumable-purchase";
  if (offerKind === "inventory") return "premium-inventory-purchase";
...
async function recoverSessionIdByPaymentIntent(params: any = {}): Promise<string> {
  const stripeApiKey = cleanText(params.stripeApiKey, 500);
  const paymentIntentId = cleanText(params.paymentIntentId, 200);
  if (!stripeApiKey || !paymentIntentId.startsWith("pi_")) return "";
  const fetchImpl = typeof params.fetchImpl === "function" ? params.fetchImpl : globalThis.fetch;
...
async function resolveStripeGrantForEvent(params: any = {}): Promise<any> {
  ...
  if (!findStripeGrant) return null;
  ...
  if (!grant && paymentIntentId) {
    ...
    if (sessionId) grant = await findStripeGrant({ sessionId });
  }
...
async function revokeForStripeEvent(params: any = {}): Promise<any> {
  const revokeGameEntitlements = typeof params.revokeGameEntitlements === "function" ? params.revokeGameEntitlements : null;
  if (!revokeGameEntitlements) return stripeError(503, "revocation_not_configured");
  const grant = await resolveStripeGrantForEvent(params);
  if (!grant) return { ok: true, ignored: true, reason: "grant_not_found" };
...
async function regrantForStripeEvent(params: any = {}): Promise<any> {
  const regrantStripeEntitlements = typeof params.regrantStripeEntitlements === "function" ? params.regrantStripeEntitlements : null;
  if (!regrantStripeEntitlements) return stripeError(503, "revocation_not_configured");
  const grant = await resolveStripeGrantForEvent(params);
  if (!grant) return { ok: true, ignored: true, reason: "grant_not_found" };
...
export async function fulfillStripeWebhook(params: any = {}): Promise<any> {
  ...
  if (!verified) return stripeError(400, "invalid_stripe_signature");
