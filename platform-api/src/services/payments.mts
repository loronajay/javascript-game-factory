...
function skinPackName(packId: string): string {
  if (packId === "fuck-cancer") return "Fuck Cancer Charity Pack";
  ...
  if (kind === "skin-pack") {
    ...
    return {
      ...
      offer: {
        ...
        entitlementIds: unownedSkins.map((skin: any) => skin.entitlementId),
        metadata: {
          offerKind: "skin-pack",
...
export async function createTacticalArenaCheckoutSession(params: any = {}): Promise<any> {
  ...
  if (!stripePublishableKey) return stripeError(503, "stripe_publishable_key_not_configured");
