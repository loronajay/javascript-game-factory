...
let marketplaceModulePromise: Promise<any> | null = null;
...
async function tacticalArenaMarketplace(): Promise<any> {
  if (!marketplaceModulePromise) {
    const importModule = Function("specifier", "return import(specifier)") as (specifier: string) => Promise<any>;
...
export async function resolveTacticalArenaPremiumOffer(offerInput: any, progress: any = {}): Promise<any> {
  ...
  if (kind === "skin-pack") {
    ...
    const individualPriceCents = skinPack.skins.reduce((sum: number, skin: any) => sum + moneyCents(skin.price?.cents), 0);
    const unownedPriceCents = unownedSkins.reduce((sum: number, skin: any) => sum + moneyCents(skin.price?.cents), 0);
