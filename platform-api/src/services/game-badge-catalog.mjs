...
function packCollectionRule(packId) {
    return {
        ...
        entitlementCount: Object.freeze({
            ...
            min: PACK_SIZE_AT_BADGE_LAUNCH[packId],
        }),
    };
}
...
function ladderBadge(id, label, art, tierId, description) {
    const min = LADDER_BADGE_TIER_MINIMUMS[tierId];
    return Object.freeze({
        ...
        tierId,
        autoAward: (facts) => Number(facts?.peakRating || 0) >= min,
        awardableUntil: null,
    });
}
...
const GAME_BADGE_CATALOG = Object.freeze({
    "tactical-arena": Object.freeze([
        Object.freeze({
            ...
            autoAward: (facts) => Boolean(facts?.playedRanked || facts?.campaignComplete),
        ...
        ladderBadge("ladder-climber", "Ladder Climber", "ladder-climber", "gold", "Climbed to Gold on the ranked ladder."),
        ladderBadge("platinum-ascent", "Platinum Ascent", "platinum-ascent", "platinum", "Climbed to Platinum on the ranked ladder."),
        ladderBadge("diamond-ascent", "Diamond Ascent", "diamond-ascent", "diamond", "Climbed to Diamond on the ranked ladder."),
        ladderBadge("master-ascent", "Master Ascent", "master-ascent", "master", "Climbed to Master on the ranked ladder."),
        ladderBadge("ladder-conqueror", "Ladder Conqueror", "ladder-conqueror", "grandmaster", "Reached Grandmaster — the top of the ranked ladder."),
...
export function listGameBadgeCatalog(gameSlug) {
    return GAME_BADGE_CATALOG[gameSlug] || [];
}
export function findGameBadge(gameSlug, badgeId) {
    return listGameBadgeCatalog(gameSlug).find((badge) => badge.id === badgeId) || null;
}
...
export function isAwardableGameBadge(gameSlug, badgeId) {
...
export function serializeBadge(badge, { earnedAt = null, source = "" } = {}) {
    if (!badge)
        return null;
    return {
        badgeId: badge.id,
...
function matchesEntitlements(badge, ids) {
    if ((badge.entitlementPatterns || []).some((pattern) => ids.some((id) => pattern.test(id)))) {
        return true;
    }
...
export function derivedBadgesForEntitlements(gameSlug, entitlementIds) {
    const ids = (Array.isArray(entitlementIds) ? entitlementIds : []).map((id) => String(id || ""));
    if (!ids.length)
        return [];
    return listGameBadgeCatalog(gameSlug)
        .filter((badge) => badge.earn === BADGE_EARN_DERIVED && matchesEntitlements(badge, ids))
        .map((badge) => serializeBadge(badge, { source: BADGE_EARN_DERIVED }));
}
...
export function autoAwardableGameBadges(gameSlug, facts, now = new Date()) {
    return listGameBadgeCatalog(gameSlug).filter((badge) => {
        if (badge.earn !== BADGE_EARN_AWARDED || typeof badge.autoAward !== "function")
            return false;
        if (badge.awardableUntil && new Date(badge.awardableUntil) < now)
            return false;
