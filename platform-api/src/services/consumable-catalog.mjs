...
const SKIN_RARITY_BY_CENTS = Object.freeze({
    99: "common",
    199: "rare",
    299: "epic",
    399: "legendary",
...
export function selectRandomUnownedSkins(skinCatalog, { rarity, count = 1, ownedEntitlementIds = new Set(), randomIndex = randomInt } = {}) {
    ...
    if (!wantedRarity || !wanted)
        return [];
    const pool = (Array.isArray(skinCatalog) ? skinCatalog : []).filter((skin) => skinRarity(skin?.amountCents) === wantedRarity && !ownedEntitlementIds.has(skin?.entitlementId));
    const picks = [];
