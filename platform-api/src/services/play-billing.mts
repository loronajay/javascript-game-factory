...
export function toPlayProductId(sku: any): string {
...
function playError(statusCode: number, error: string): any {
  return { ok: false, statusCode, error };
}

// --- product index -----------------------------------------------------------
//
// Every premium offer the store sells, keyed by the Play product id it was published
// under. The values are offer INPUTS, not resolved offers: resolution (pricing, ownership,
// pack proration) stays with resolveTacticalArenaPremiumOffer so the Play and Stripe paths
// cannot drift apart on what a purchase is worth.

interface PlayProductIndex {
  byProductId: Map<string, any>;
  ambiguous: Set<string>;
}

function skinPackIds(): string[] {
  return [...new Set(SKIN_CATALOG.map((skin: any) => cleanText(skin.packId, 120)).filter(Boolean))];
}

function catalogOfferInputs(): any[] {
  return [
    ...UNIT_CATALOG.map((unit: any) => ({ kind: "unit", sku: unit.sku, type: unit.type })),
    ...SKIN_CATALOG.map((skin: any) => ({ kind: "skin", sku: skin.sku, type: skin.type, slug: skin.slug })),
    ...skinPackIds().map((packId) => ({ kind: "skin-pack", sku: `ta.skinpack.${packId}`, packId })),
    ...getConsumableOffers().map((item: any) => ({ kind: "consumable", sku: item.sku, id: item.id })),
  ];
}

function buildPlayProductIndex(): PlayProductIndex {
...
let productIndex: PlayProductIndex | null = null;

function index(): PlayProductIndex {
  if (!productIndex) productIndex = buildPlayProductIndex();
  return productIndex;
}
...
export function listPlayProductIds(): string[] {
  return [...index().byProductId.keys()].sort();
}

export function listAmbiguousPlayProductIds(): string[] {
...
export function resolvePlayProductOffer(productId: any): any {
  const id = cleanText(productId, 200).toLowerCase();
  if (!id || index().ambiguous.has(id)) return null;
  return index().byProductId.get(id) || null;
}

// --- service-account auth ----------------------------------------------------

function parseServiceAccountKey(value: any): any {
...
const base64url = (value: any) => Buffer.from(value).toString("base64url");
...
export function createPlayAccessTokenProvider(options: any = {}): () => Promise<string> {
  ...
  return async function getPlayAccessToken(): Promise<string> {
    if (cachedToken && now() < cachedUntilMs) return cachedToken;
    if (!inFlight) {
...
export async function fetchPlayPurchase(params: any = {}): Promise<any> {
  ...
  if (!productId || !purchaseToken) return playError(400, "invalid_purchase_request");
  if (!accessToken) return playError(503, "play_verification_not_configured");
  if (typeof fetchImpl !== "function") return playError(503, "fetch_not_configured");
...
function purchaseTokenHash(purchaseToken: string): string {
...
function playClaimId(orderId: string, tokenHash: string): string {
 ...
 */
export async function fulfillPlayPurchase(params: any = {}): Promise<any> {
  ...
  if (!offerInput) return playError(400, "offer_not_found");

  const recordGameProgressClaim = typeof params.recordGameProgressClaim === "function" ? params.recordGameProgressClaim : null;
  if (!recordGameProgressClaim) return playError(503, "fulfillment_not_configured");
  ...
  if (!recorded?.ok) return playError(502, "fulfillment_failed");
