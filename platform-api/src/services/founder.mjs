...
export async function resolveFounderPlayerId({ founderEmail, findAccountByEmail } = {}) {
    const email = typeof founderEmail === "string" ? founderEmail.trim() : "";
    if (!email || typeof findAccountByEmail !== "function")
        return "";
    try {
...
export async function linkNewAccountToFounder({ playerId, founderEmail, findAccountByEmail, createFriendship, } = {}) {
    const newPlayerId = typeof playerId === "string" ? playerId.trim() : "";
    if (!newPlayerId)
        return { linked: false, reason: "invalid_player" };
    if (!founderEmail)
        return { linked: false, reason: "not_configured" };
    if (typeof createFriendship !== "function")
        return { linked: false, reason: "not_configured" };
    const founderPlayerId = await resolveFounderPlayerId({ founderEmail, findAccountByEmail });
    if (!founderPlayerId)
        return { linked: false, reason: "founder_not_found" };
    // The founder registering their own account would otherwise befriend themselves.
    if (founderPlayerId === newPlayerId)
        return { linked: false, reason: "self" };
    try {
        await createFriendship(newPlayerId, founderPlayerId);
        return { linked: true, founderPlayerId };
    }
    catch (err) {
        process.stderr.write(`[founder] linkNewAccountToFounder error: ${err?.message || err}\n`);
