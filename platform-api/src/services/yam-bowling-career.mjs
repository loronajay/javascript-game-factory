...
export function yamBowlingVenueMask(slug) {
    const index = YAM_BOWLING_VENUE_SLUGS.indexOf(String(slug));
    return index < 0 ? 0 : 1 << index;
}
export function mergeYamBowlingCareerStats(stored = {}, incoming = {}, standard = {}) {
...
export function earnedYamBowlingCareerBadges(tracks = {}) {
    ...
    const earned = [];
    if (bestSpareRun >= 20)
        earned.push("badge:precision-bowler");
    if ((venueWinMask & YAM_BOWLING_ALL_VENUES_MASK) === YAM_BOWLING_ALL_VENUES_MASK)
        earned.push("badge:lane-legend");
    if ((venueMask & YAM_BOWLING_ALL_VENUES_MASK) === YAM_BOWLING_ALL_VENUES_MASK)
        earned.push("badge:road-tested");
    if (YAM_BOWLING_BOWLER_SLUGS.every((slug) => count(tracks?.[slug]?.careerWins) > 0))
        earned.push("badge:deep-bench");
    return earned;
}
