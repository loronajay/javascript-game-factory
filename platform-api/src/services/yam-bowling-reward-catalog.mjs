...
import { YAM_BOWLING_BOWLER_SLUGS, yamBowlingVenueMask, } from "./yam-bowling-career.mjs";
...
function careerMatchClaim(params) {
...
export function validateYamBowlingPublicClaim(params = {}) {
    ...
    if (kind === YAM_BOWLING_CAREER_MATCH_CLAIM_KIND)
        return careerMatchClaim(params);
...
export function validateYamBowlingEmoteVoucherTarget(gameSlug, entitlementId) {
    if (gameSlug !== YAM_BOWLING_GAME_SLUG || typeof entitlementId !== "string")
        return null;
    const match = /^emote:([a-z0-9-]+)$/.exec(entitlementId);
    if (!match || !EMOTE_VOUCHER_SLUGS.has(match[1]))
        return null;
