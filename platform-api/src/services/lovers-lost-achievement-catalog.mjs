...
export const LOVERS_LOST_DEFINITIONS = Object.freeze([
    // Root
...
export const LOVERS_NEVER_DIE_REQUIRED_IDS = Object.freeze([
    "ll_found_again",
    "ll_his_side",
    "ll_her_side",
    "ll_both_sides",
...
function normalizeLane(value, active, elapsedFrames) {
    if (!active)
        return inactiveLane();
    ...
    if ([obstaclesFaced, perfects, goods, misses, score, warmupFaced, warmupMisses].some((n) => n === null))
        return null;
    if (!successfulByType || !perfectByType || !missesByType)
        return null;
    // The tallies are one partition of the obstacles faced, twice over.
    if (perfects + goods + misses !== obstaclesFaced)
        return null;
    if (sumByType(successfulByType) !== perfects + goods)
        return null;
    if (sumByType(perfectByType) !== perfects)
        return null;
    if (sumByType(missesByType) !== misses)
        return null;
    if (OBSTACLE_TYPES.some((key) => perfectByType[key] > successfulByType[key]))
        return null;
    if (warmupMisses > warmupFaced)
        return null;
    if (warmupFaced > obstaclesFaced)
        return null;
    if (score > obstaclesFaced * MAX_SCORE_PER_OBSTACLE)
        return null;
    const finished = src.finished === true;
    let finishFrame = null;
    if (finished) {
        const frame = count(src.finishFrame, LOVERS_LOST_HARD_CUTOFF_FRAMES);
        if (frame === null || frame < MIN_FINISH_FRAME || frame > elapsedFrames)
            return null;
        finishFrame = frame;
    }
...
export function normalizeLoversLostRun(payload) {
    const src = payload && typeof payload === "object" && !Array.isArray(payload) ? payload : null;
    if (!src)
        return { ok: false, error: "invalid_run" };
    if (src.gameSlug !== LOVERS_LOST_GAME_SLUG)
        return { ok: false, error: "invalid_run" };
    const runId = typeof src.runId === "string" && RUN_ID_PATTERN.test(src.runId) ? src.runId : "";
    if (!runId)
        return { ok: false, error: "invalid_run_id" };
    const mode = src.mode === "single" || src.mode === "local" || src.mode === "online" ? src.mode : null;
    if (!mode)
        return { ok: false, error: "invalid_run" };
    const soloSide = mode === "single"
        ? (src.soloSide === "boy" || src.soloSide === "girl" ? src.soloSide : null)
        : null;
    if (mode === "single" && !soloSide)
        return { ok: false, error: "invalid_run" };
    ...
    if (mode === "single")
        ...
        if (claimed.length !== 1 || (claimed[0] !== "boy" && claimed[0] !== "girl"))
            return { ok: false, error: "invalid_run" };
    ...
    const outcome = finishedCount === activeLanes.length ? "reunion"
        : finishedCount > 0 ? "partial" : "game_over";
    if (typeof src.outcome === "string" && src.outcome !== outcome)
...
export function detectLoversLostAchievements({ run, ownedIds }) {
    const earned = [];
    const has = (id) => ownedIds.has(id) || earned.includes(id);
    ...
    if (reunion)
        earned.push("ll_found_again");
    if (run.mode === "single" && run.soloSide === "boy" && run.lanes.boy.finished)
        earned.push("ll_his_side");
    if (run.mode === "single" && run.soloSide === "girl" && run.lanes.girl.finished)
        earned.push("ll_her_side");
    if (has("ll_his_side") && has("ll_her_side"))
        earned.push("ll_both_sides");
    if (coop && run.mode === "local")
        earned.push("ll_side_by_side");
    if (coop && run.mode === "online")
        earned.push("ll_long_distance");
    if (coop && gap !== null && gap <= LOVERS_LOST_SYNC_FRAMES.IN_SYNC)
        earned.push("ll_in_sync");
    if (ownedLaneList(run).some((lane) => lane.warmupFaced === LOVERS_LOST_WARMUP_COUNT && lane.warmupMisses === 0))
        earned.push("ll_clean_start");
    if (completed.some((lane) => OBSTACLE_TYPES.every((key) => lane.successfulByType[key] >= 1)))
        earned.push("ll_four_moves");
    if (completed.some((lane) => lane.perfects >= LOVERS_LOST_PERFECT_COUNTS.SHARP_TIMING))
        earned.push("ll_sharp_timing");
    if (completed.some((lane) => lane.perfects >= LOVERS_LOST_PERFECT_COUNTS.CLOCKWORK))
        earned.push("ll_clockwork");
    if (completed.some((lane) => lane.misses === 0))
        earned.push("ll_untouchable");
    if (completed.some((lane) => lane.misses === 0 && lane.goods === 0 && lane.perfects === lane.obstaclesFaced))
        earned.push("ll_perfect_run");
    if (coop && [run.lanes.boy, run.lanes.girl].every((lane) => lane.finished && lane.obstaclesFaced > 0 && lane.misses === 0))
        earned.push("ll_perfect_pair");
    const bestScore = completed.reduce((best, lane) => Math.max(best, lane.score), 0);
    if (bestScore >= LOVERS_LOST_SCORE_THRESHOLDS.SPARKS_FLY)
        earned.push("ll_sparks_fly");
    if (bestScore >= LOVERS_LOST_SCORE_THRESHOLDS.BURNING_BRIGHT)
        earned.push("ll_burning_bright");
    if (bestScore >= LOVERS_LOST_SCORE_THRESHOLDS.WRITTEN_IN_STARS)
        earned.push("ll_written_stars");
    if (reunion && run.elapsedFrames <= LOVERS_LOST_SPEED_THRESHOLDS_FRAMES.DONT_KEEP_WAITING)
        earned.push("ll_dont_keep_waiting");
    if (reunion && run.elapsedFrames <= LOVERS_LOST_SPEED_THRESHOLDS_FRAMES.HEART_RACING)
        earned.push("ll_heart_racing");
    if (reunion && run.elapsedFrames <= LOVERS_LOST_SPEED_THRESHOLDS_FRAMES.NO_TIME_TO_LOSE)
        earned.push("ll_no_time_to_lose");
    if (reunion && run.elapsedFrames >= LOVERS_LOST_SECONDS_TO_SPARE_MIN_FRAMES && run.elapsedFrames < LOVERS_LOST_HARD_CUTOFF_FRAMES)
        earned.push("ll_seconds_to_spare");
    if (coop && gap !== null && gap <= LOVERS_LOST_SYNC_FRAMES.MIRROR_IMAGE)
        earned.push("ll_mirror_image");
    if (LOVERS_NEVER_DIE_REQUIRED_IDS.every((id) => has(id)))
        earned.push("ll_lovers_never_die");
    return earned;
}
export const LOVERS_LOST_ACHIEVEMENTS = Object.freeze({
