...
const UNIT_VALOR_STAR_BY_TYPE: Record<string, number> = Object.freeze({
  juggernaut: 1,
  "big-brother": 1,
  "witch-doctor": 1,
  monk: 1,
...
const SKIN_PACK_VALOR: Record<string, number> = Object.freeze({
  "summer-vibes": 12500,
  halloween: 19500,
  arcane: 16000,
  "blood-moon": 40000,
...
function roundValorAmount(amount: any): number {
...
function prorateValor(baseAmount: number, totalAmount: number, partAmount: number): number {
...
export function skinValorCost(amountCents: any): number {
...
export function unitValorCost(type: any): number {
  const star = UNIT_VALOR_STAR_BY_TYPE[cleanText(type, 80)];
  return UNIT_VALOR_COST_BY_STAR[star] ?? 0;
}

function findSkin(type: string, slug: string): any {
  return SKIN_CATALOG.find((skin: any) => skin.type === type && skin.slug === slug) || null;
}

function findUnit(type: string): any {
  return UNIT_CATALOG.find((unit: any) => unit.type === type) || null;
}

function packSkins(packId: string): any[] {
  return SKIN_CATALOG.filter((skin: any) => skin.packId === packId);
}
...
export function getValorOffer(offerInput: any): any {
  ...
  if (kind === "skin-pack") {
    ...
    return {
      ...
      packId,
      packBaseValor,
      entitlements: skins.map((skin: any) => ({
...
export function priceValorOffer(resolvedOffer: any, ownedEntitlementIds: Set<string>): any {
  const entitlements = Array.isArray(resolvedOffer?.entitlements) ? resolvedOffer.entitlements : [];
  const unowned = entitlements.filter((entry: any) => !ownedEntitlementIds.has(entry.entitlementId));
  if (!unowned.length) {
    return { alreadyOwned: true, valorCost: 0, grants: [] };
  }

  if (resolvedOffer.kind === "skin-pack") {
    const totalIndividual = entitlements.reduce((sum: number, entry: any) => sum + (entry.valorCost || 0), 0);
    const unownedIndividual = unowned.reduce((sum: number, entry: any) => sum + (entry.valorCost || 0), 0);
  ...
  const valorCost = unowned.reduce((sum: number, entry: any) => sum + (entry.valorCost || 0), 0);
  return { alreadyOwned: false, valorCost, grants: unowned };
}
