...
const SURFACE_KINDS = ["floor", "wall", "ceiling", "trim"] as const;
...
function surfaceIdPattern(kind: string): RegExp {
...
function mayUseCatalogId(id: string, context: any): boolean {
  if (!ARCADE_ROOM_CATALOG_IDS.has(id)) return false;
  const state = ownership(context);
  return !state.enforce || ARCADE_ROOM_STARTER_IDS.has(id) || state.owned.has(id);
}

function normalizeSurfaces(value: any, context: any): Record<string, string> {
  const source = value && typeof value === "object" ? value : {};
...
function normalizeSurfaceColors(value: any): Record<string, string[]> {
  const source = value && typeof value === "object" ? value : {};
  const colors: Record<string, string[]> = {};
  for (const kind of SURFACE_KINDS) {
    const list = Array.isArray(source[kind]) ? source[kind].slice(0, SURFACE_COLOR_SLOTS) : [];
    const slots = list.map((raw: any) => (typeof raw === "string" && HEX_COLOR.test(raw.toLowerCase()) ? raw.toLowerCase() : ""));
 ...
 */
export function normalizeArcadeRoomGarage(value: any, context: any = {}): any {
  ...
  if (input.surfaceColors && typeof input.surfaceColors === "object") garage.surfaceColors = normalizeSurfaceColors(input.surfaceColors);
...
export function arcadeRoomLoadoutFromGarage(garage: any, context: any = {}): any {
