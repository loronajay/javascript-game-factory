...
// are not a mode here.
import { namesClientAmount, readChoice, readDurationMs, readResultId, readResultSource, refuse, } from "./game-result-shared.mjs";
const MODES = ["sprint", "sweep", "online"];
const ONLINE_OUTCOMES = ["win", "loss"];
...
export const ILLUMINAUTS_PAR_MS = Object.freeze({
...
const COMPLETION_CAP = { sprint: 15, sweep: 35, online: 20 };
const PAR_BONUS = { sprint: 5, sweep: 10 };
const ONLINE_WIN_BONUS = 10;
export function normalizeIlluminautsResult(raw) {
    ...
    if (durationMs < floorPar / PAR_FLOOR_DIVISOR)
        return refuse("implausible_duration");
    return { result: { resultId, mode, mapId, outcome, durationMs } };
}
export function calculateIlluminautsTicketReward({ result }) {
