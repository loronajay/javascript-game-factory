...
export function normalizeSumoraiResult(raw) {
    ...
    if (myWins === null || opponentWins === null || rounds === null || durationMs === null)
        return refuse("invalid_counts");
    if (myWins === roundTarget && opponentWins === roundTarget)
        return refuse("invalid_counts");
    if (outcome === "win" && myWins !== roundTarget)
        return refuse("invalid_counts");
    if (outcome === "loss" && opponentWins !== roundTarget)
        return refuse("invalid_counts");
    if (rounds < myWins + opponentWins)
        return refuse("invalid_counts");
    if (durationMs < rounds * MIN_MS_PER_ROUND)
...
export function calculateSumoraiTicketReward({ result }) {
