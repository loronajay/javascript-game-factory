import type { LoversLostLane, LoversLostRun } from "./lovers-lost-achievement-catalog.mjs";
...
function completedOwnedLane(run: LoversLostRun): LoversLostLane | null {
  if (run.mode === "local" || run.disconnected) return null;
...
export function calculateLoversLostTicketReward({ run, unlockedIds = [] }: {
  ...
  const achievements = (run.disconnected ? [] : [...new Set(unlockedIds)])
    .map((id) => ({ id, amount: Number((LOVERS_LOST_ACHIEVEMENT_TICKET_REWARDS as Record<string, number>)[id]) || 0 }))
