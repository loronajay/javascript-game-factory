...
const CAMPAIGN_REWARDS: Readonly<Record<string, any>> = Object.freeze({
...
const CAMPAIGN_UNIT_PACKS: Readonly<Record<string, any>> = Object.freeze({
  brothers: Object.freeze({ missionId: "mechs-on-the-farm", choices: Object.freeze(["big-brother", "little-brother"]) }),
});

const CAMPAIGN_SKIN_PACKS: Readonly<Record<string, any>> = Object.freeze({
...
function cleanInt(value: any, { min = 0, max = Number.MAX_SAFE_INTEGER } = {}): number {
  const number = Math.floor(Number(value));
  if (!Number.isFinite(number)) return min;
  return Math.max(min, Math.min(max, number));
}

function accepted(payload: any, sourceId: string, prerequisite: any = null, valorBase = 0): any {
...
export function validateTacticalArenaPublicClaim(params: any = {}): any {
  if (cleanText(params.gameSlug, 60) !== TACTICAL_ARENA_GAME_SLUG) return rejected();
  ...
  if (kind === "tutorial-complete") {
    ...
    if (!TACTICAL_ARENA_TUTORIAL_IDS.includes(tutorialId) || claimId !== `tutorial-complete:${tutorialId}`) return rejected();
    return accepted({ tutorialId }, tutorialId);
  }

  if (kind === "tutorial-valor") {
    if (claimId !== "tutorial-valor:all-tutorials") return rejected();
    return accepted(
      { amount: TUTORIAL_VALOR_REWARD, completedTutorials: [...TACTICAL_ARENA_TUTORIAL_IDS] },
      "all-tutorials",
      { kind: "all-tutorials" },
  ...
  if (kind === "tutorial-unit-reward") {
    if (claimId !== "tutorial-unit-reward:all-tutorials:juggernaut") return rejected();
    return accepted(
      { type: "juggernaut", entitlementId: "unit:juggernaut" },
      "all-tutorials",
      { kind: "all-tutorials" },
    );
  }
  ...
  if (kind === "tutorial-skin-choice") {
    ...
    if (!TUTORIAL_SKINS.has(`${type}:${slug}`) || claimId !== `tutorial-skin-choice:${type}:${slug}`) return rejected();
    return accepted(
      { type, slug, entitlementId: `skin:${type}:${slug}` },
      "all-tutorials",
      { kind: "all-tutorials" },
    );
  }
  ...
  if (kind === "campaign-progress") {
    ...
    if (claimId !== `campaign-progress:${epochSegment}${missionId}:${stars}`) return rejected();
    return accepted({ missionId, stars, campaignEpoch }, missionId);
  }

  if (kind === "campaign-valor") {
    if (claimId !== `campaign-valor:${missionId}`) return rejected();
  ...
  if (kind === "campaign-unit-choice") {
    ...
    if (pack) {
      if (pack.missionId !== missionId || !pack.choices.includes(type)
        || claimId !== `campaign-unit-choice:${packId}:${type}`) return rejected();
    } else if (!mission.units?.includes(type) || claimId !== `campaign-unit-reward:${missionId}:${type}`) {
      return rejected();
    }
    return accepted(
      { ...(pack ? { packId } : {}), missionId, type, entitlementId: `unit:${type}`, stars },
      missionId,
      { kind: "campaign-mission", missionId },
    );
  }
  ...
  if (kind === "campaign-skin-choice") {
    ...
    if (pack) {
      if (pack.missionId !== missionId || !pack.choices.includes(key)
        || claimId !== `campaign-skin-choice:${packId}:${type}:${slug}`) return rejected();
    } else if (!mission.skins?.includes(key) || claimId !== `campaign-skin-reward:${missionId}:${type}:${slug}`) {
      return rejected();
    }
    return accepted(
      { ...(pack ? { packId } : {}), missionId, type, slug, entitlementId: `skin:${type}:${slug}`, stars },
      missionId,
      { kind: "campaign-mission", missionId },
    );
  }
...
export function getTacticalArenaCampaignRewardCatalog(): Readonly<Record<string, any>> {
