...
const GEOMETRY_BOUNDS: Record<string, Bounds> = {
...
const MATERIAL_BOUNDS: Record<string, Bounds> = {
...
const HARDWARE_BOUNDS: Record<string, Bounds> = {
...
const DECAL_BOUNDS: Record<string, Bounds> = {
  x: range(-0.75, 0.75),
  y: range(-0.75, 0.75),
...
const SURFACE_BOUNDS: Record<string, Bounds> = {
...
const RAIL_BOUNDS: Record<string, Bounds> = {
...
// ---------------------------------------------------------------------------
// CATALOG — the ids that may be stored, and the parameters each one seeds.
// ---------------------------------------------------------------------------

type NumberMap = Record<string, number>;

type ShapePreset = Record<string, number | boolean> & { crownEnabled: boolean };

const SHAPE_PRESETS: Record<string, ShapePreset> = {
...
const MATERIAL_PRESETS: Record<string, NumberMap> = {
...
type SurfacePreset = { baseColor: string; secondaryColor: string; patternId: string; patternColor: string; patternScale: number; patternOpacity: number; roughness: number; metalness: number };

const SURFACE_PRESETS: Record<string, SurfacePreset> = {
...
type RailPreset = { bodyColor: string; topColor: string; roughness: number; metalness: number };
...
type GoalPreset = { color: string; emissiveColor: string; glowIntensity: number };
...
const DEFAULT_HARDWARE: NumberMap = { radius: 0.49, thickness: 0.045, height: 0.18, glowIntensity: 0.34 };
...
function clampNumber(value: any, bounds: Bounds, fallback: number): number {
...
function normalizeColor(value: any, fallback: string): string {
  ...
  if (SHORT_HEX.test(text)) return `#${text.slice(1).split("").map((c) => c + c).join("")}`;
  return fallback;
}

/** One id, or the default. An unknown id falls back rather than failing the save. */
function pickId(value: any, allowed: Set<string> | Record<string, unknown>, fallback: string): string {
  const text = typeof value === "string" ? value.trim() : "";
  const known = allowed instanceof Set ? allowed.has(text) : Object.hasOwn(allowed, text);
  return known ? text : fallback;
}

function clampMap(raw: any, bounds: Record<string, Bounds>, defaults: NumberMap): NumberMap {
  const input = asObject(raw);
  const out: NumberMap = {};
  for (const [key, bound] of Object.entries(bounds)) out[key] = clampNumber(input[key], bound, defaults[key]);
  return out;
}

function normalizeGeometry(raw: any, defaults: any): any {
  const out: any = clampMap(raw, GEOMETRY_BOUNDS, defaults);
  const input = asObject(raw);
  out.crownEnabled = typeof input.crownEnabled === "boolean" ? input.crownEnabled : Boolean(defaults.crownEnabled);
  return out;
}

function normalizeDecal(raw: any): any {
  const input = asObject(raw);
  const placement = clampMap(input, DECAL_BOUNDS, DEFAULT_DECAL as any);
...
function normalizeMallet(raw: any): any {
  const input = asObject(raw);
...
function normalizeTableHalf(raw: any): any {
  const input = asObject(raw);
  ...
  return {
    surface: {
      ...
      ...clampMap(surfaceInput, SURFACE_BOUNDS, surfaceDefaults as unknown as NumberMap),
    },
    ...
    rails: {
      ...
      ...clampMap(railInput, RAIL_BOUNDS, railDefaults as unknown as NumberMap),
    },
    goal: {
      preset: goalPreset,
 ...
 */
export function defaultPuckdUpGarage(): any {
