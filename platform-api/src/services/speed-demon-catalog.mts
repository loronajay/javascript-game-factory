...
export const SPEED_DEMON_FADE_AXES: readonly string[] = Object.freeze([
  "nose-tail", "tail-nose", "left-right", "right-left",
]);
const FADE_AXIS_SET = new Set(SPEED_DEMON_FADE_AXES);

export const SPEED_DEMON_LAYER_KINDS: readonly string[] = Object.freeze(["band", "stripe"]);
 ...
 */
function normalizePaint(value: any): any {
 ...
 */
function normalizeLayer(value: any, index: number): any {
...
export function normalizeLivery(value: any): any {
  ...
  return {
    ...
    layers: layers.map((layer: any, index: number) => normalizeLayer(layer, index)),
...
export function normalizeDriverProfile(value: any): any {
  ...
  for (const raw of (Array.isArray(source.favourites) ? source.favourites : []).slice(0, 64)) {
    ...
    const entry = typeof raw === "string" ? { modelId: raw }
      ...
      : null;
    if (!entry || !isValidSpeedDemonModelId(entry.modelId)) continue;
