...
export function normalizeFarmPetGrowthShape(value) {
    ...
    const count = (raw) => Math.floor(clamp(finite(raw) ?? 0, 0, 99));
    return {
 ...
 */
export function pinFarmPetGrowth(species, submitted, stored) {
    const life = species ? FARM_PET_LIFESPANS[species.id] : undefined;
    const incoming = normalizeFarmPetGrowthShape(submitted?.growth);
    if (!species || !life || !incoming)
        return null;
    ...
    const growth = { ...incoming, base: {}, rates: {}, gained: {} };
    ...
    if (pinned)
        growth.grade = pinned.grade;
    const stat = (id) => round(Math.min(STAT_CEILING, (growth.base[id] + growth.gained[id]) * (1 + bonus)), 1);
