...
const FORMATS = ["ffa", "teams"] as const;
// Mirrors BOARD_SIZES in games/mini-tactics/src/config.js.
const BOARD_SIZES = [10, 13] as const;
...
export interface MiniTacticsResult {
  resultId: string;
...
const CPU_COMPLETION_CAP: Readonly<Record<number, number>> = { 10: 60, 13: 90 };
...
const CPU_WIN_BONUS = { easy: 5, normal: 15, hard: 25 } as const;
const ONLINE_WIN_BONUS = 30;

export function normalizeMiniTacticsResult(raw: unknown): NormalizedResult<MiniTacticsResult> {
  ...
  if (playerCount === null || !format) return refuse("invalid_table");
  // The CPU plays the classic duel; teams is always two against two.
  if (mode === "cpu" && (playerCount !== 2 || format !== "ffa")) return refuse("invalid_table");
  if (format === "teams" && playerCount !== 4) return refuse("invalid_table");

  const boardSize = readInt(source.boardSize, 10, 13);
  if (boardSize === null || !(BOARD_SIZES as readonly number[]).includes(boardSize)) return refuse("invalid_board");

  const turns = readInt(source.turns, 1, MAX_TURNS);
  const durationMs = readDurationMs(source.durationMs);
  if (turns === null || durationMs === null) return refuse("invalid_counts");
  if (durationMs < turns * MIN_MS_PER_TURN) return refuse("implausible_duration");

  return { result: { resultId, mode, difficulty, outcome, playerCount, format, boardSize, turns, durationMs } };
}

export function calculateMiniTacticsTicketReward({ result }: { result: MiniTacticsResult }): TicketRewardBreakdown {
