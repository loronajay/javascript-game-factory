...
const ONLINE_OUTCOMES = ["win", "loss", "tie"];
...
const ONLINE_WIN_BONUS = { duel: 5, table: 8 };
export function normalizeBirdDutyResult(raw) {
    ...
    if (mode === "solo") {
        ...
        if (score === null)
            return refuse("invalid_counts");
        if (durationMs < SOLO_SHOTS * MIN_MS_PER_SHOT)
    ...
    if (outcome !== expected)
        return refuse("invalid_outcome");
    if (durationMs < playerCount * ONLINE_SHOTS_PER_PLAYER * MIN_MS_PER_SHOT)
        return refuse("implausible_duration");
    return { result: { resultId, mode, playerCount, outcome, myScore, bestOpponentScore, durationMs } };
}
export function calculateBirdDutyTicketReward({ result }) {
    if (result.mode === "solo") {
