import { CALENDAR_GAME_SLUG, CALENDAR_OFFER, CALENDAR_PREORDER_CLAIM_KIND, CALENDAR_PRODUCT_ID, calendarBonusClaimId, calendarBonusInventoryItems, priceCalendarPreorder, } from "./calendar-catalog.mjs";
import { appendMetadata, cleanText, logStripeCheckoutError, moneyCents, retrieveStripeCheckoutSession, stripeCheckoutError, stripeError, stripeFetchHeaders, STRIPE_CHECKOUT_SESSIONS_URL, } from "./payments.mjs";
...
function checkoutUrl(appBaseUrl, status) {
 ...
 * server catalog.
 */
export async function createCalendarCheckoutSession(params = {}) {
    ...
    if (!playerId)
        return stripeError(401, "authentication_required");
    ...
    if (!sessionId || !url)
        return stripeError(502, "stripe_checkout_failed");
...
function quantityFromSession(session, metadata) {
    const fromLineItem = Number(session?.line_items?.data?.[0]?.quantity);
    if (Number.isFinite(fromLineItem) && fromLineItem > 0)
        return Math.floor(fromLineItem);
 ...
 */
export async function fulfillCalendarCheckoutSession(params = {}) {
    ...
    if (!settled?.ok)
        return settled || stripeError(500, "order_not_settled");
 ...
 */
export async function fulfillCalendarCheckoutFromReturn(params = {}) {
    const playerId = cleanText(params.playerId, 120);
    const body = params.body && typeof params.body === "object" ? params.body : {};
    ...
    if (session?.payment_status !== "paid")
        return stripeError(409, "checkout_not_paid");
 ...
 */
export async function closeCalendarOrderForStripeEvent(params = {}) {
...
export function isCalendarCheckoutSession(session) {
    const metadata = session?.metadata && typeof session.metadata === "object" ? session.metadata : {};
