...
export interface AchievementDefinition {
  /** Stable forever. Stored on every ownership row. */
  id: string;
  name: string;
  description: string;
  category: AchievementCategory;
  /** Hidden until unlocked: locked reads show a masked placeholder. */
  secret?: boolean;
  /** Visual tree parent. Presentation only — see the note above. */
  parentId?: string | null;
  /** Display tier within a branch (1 = first rung). Presentation only. */
  tier?: number;
  points?: number;
  /** A game-relative icon path, or null for the platform's generated glyph. */
  icon?: string | null;
}

export interface AchievementDetectInput<Run> {
  run: Run;
  /** Ids the player already owns for this game, plus any earned earlier in this evaluation. */
  ownedIds: ReadonlySet<string>;
}

export interface AchievementGame<Run = unknown> {
  gameSlug: string;
  title: string;
  definitions: readonly AchievementDefinition[];
  /**
   * Turns the raw request body into a validated run, or returns
   * `{ ok: false, error }` for anything that cannot have happened.
   */
  normalizeRun: (payload: unknown) => { ok: true; run: Run } | { ok: false; error: string };
  /**
   * Pure. Returns every id this run earns, in catalog order. Must be safe to
   * call with ids the player already owns (the caller filters those out) and
   * must not depend on anything but its arguments.
   */
  detect: (input: AchievementDetectInput<Run>) => string[];
}
...
function cleanText(value: unknown, maxLength = 60): string {
  return typeof value === "string" ? value.trim().slice(0, maxLength) : "";
}

export function listAchievementGames(): Array<{ gameSlug: string; title: string; total: number }> {
  return GAMES.map((game) => ({ gameSlug: game.gameSlug, title: game.title, total: game.definitions.length }));
}

export function isAchievementGameSlug(value: unknown): boolean {
  return BY_SLUG.has(cleanText(value).toLowerCase());
}

export function getAchievementGame(value: unknown): AchievementGame<any> | null {
  return BY_SLUG.get(cleanText(value).toLowerCase()) ?? null;
}

export function getAchievementDefinition(gameSlug: unknown, achievementId: unknown): AchievementDefinition | null {
 ...
 */
export function presentAchievement(definition: AchievementDefinition, unlockedAt: string | null): any {
 ...
 */
export function evaluateAchievementRun<Run>(game: AchievementGame<Run>, run: Run, ownedIds: Iterable<string>): string[] {
  const known = new Set(game.definitions.map((definition) => definition.id));
  const owned = new Set<string>();
  for (const id of ownedIds) if (known.has(id)) owned.add(id);
  const earned: string[] = [];
