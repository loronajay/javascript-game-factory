...
async function announceContent(kind, { record, actorPlayerId = "", claimAnnouncement, broadcast, } = {}) {
    if (!record?.id)
        return { announced: false, reason: "invalid_record" };
    if (!kind.isPublic(record))
        return { announced: false, reason: "not_public" };
    ...
    try {
        ...
        process.stderr.write(`[announce] ${kind.notificationType} error: ${err?.message || err}\n`);
        return { announced: false, reason: "server_error" };
    }
}
export async function announceBulletinService({ bulletin, ...rest } = {}) {
    return announceContent(BULLETIN_KIND, { record: bulletin, ...rest });
}
export async function announceEventService({ event, ...rest } = {}) {
