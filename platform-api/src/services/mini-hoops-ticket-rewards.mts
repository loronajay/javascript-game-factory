...
const ONLINE_OUTCOMES = ["win", "loss", "draw"] as const;
...
export interface MiniHoopsResult {
  resultId: string;
...
const ONLINE_WIN_BONUS: Readonly<Record<number, number>> = { 30: 2, 60: 4 };

export function normalizeMiniHoopsResult(raw: unknown): NormalizedResult<MiniHoopsResult> {
  ...
  if (!hoopMode) return refuse("invalid_hoop_mode");
  const roundSeconds = readInt(source.roundSeconds, 1, 600);
  if (roundSeconds === null || !MINI_HOOPS_ROUND_SECONDS.includes(roundSeconds)) return refuse("invalid_round");

  let outcome: MiniHoopsResult["outcome"] = null;
  if (mode === "online") {
  ...
  if (shots === null || made === null || durationMs === null || made > shots) return refuse("invalid_counts");
  // The clock only runs forward and the buzzer ends the round, so a result
  // shorter than its own round is not one.
  if (durationMs < roundSeconds * 1000) return refuse("implausible_duration");

  return { result: { resultId, mode, hoopMode, roundSeconds, shots, made, outcome, durationMs } };
}

export function calculateMiniHoopsTicketReward({ result }: { result: MiniHoopsResult }): TicketRewardBreakdown {
