 ...
 */
const TABLE_SLOTS = Object.freeze({
    cloth: "table.cloth.",
    rail: "table.rail.",
    apron: "table.apron.",
...
const HALL_SLOTS = Object.freeze({
    walls: "hall.wall.",
    floor: "hall.floor.",
...
const DEFAULT_HALL = Object.freeze({
 ...
 */
function slotId(raw, prefix, fallback) {
    if (raw === null || raw === undefined)
        return fallback === null ? null : fallback;
    const id = cleanText(raw, 80);
    if (!id || !ID_PATTERN.test(id) || !id.startsWith(prefix))
        return fallback;
    return id;
}
function normalizeDomain(raw, slots, defaults) {
...
export function defaultSharkHallGarage() {
    return {
 ...
 */
export function normalizeSharkHallGarage(value) {
 ...
 */
export function sharkHallLoadoutFromGarage(garage) {
