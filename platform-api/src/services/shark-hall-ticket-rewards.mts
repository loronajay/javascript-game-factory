...
export const SHARK_HALL_DIFFICULTIES = Object.freeze(["casual", "club", "sharp"] as const);
...
export interface SharkHallResult {
  resultId: string;
  mode: (typeof MODES)[number];
  difficulty: (typeof SHARK_HALL_DIFFICULTIES)[number] | null;
  raceTo: number;
  outcome: (typeof OUTCOMES)[number];
  myRacks: number;
  opponentRacks: number;
  strokes: number;
  durationMs: number;
}
...
const ONLINE_COMPLETION_CAP: Readonly<Record<number, number>> = { 1: 25, 3: 75, 5: 125 };
const CPU_WIN_BONUS: Readonly<Record<(typeof SHARK_HALL_DIFFICULTIES)[number], number>> = { casual: 5, club: 10, sharp: 15 };
const ONLINE_WIN_BONUS: Readonly<Record<number, number>> = { 1: 10, 3: 25, 5: 40 };
...
export function normalizeSharkHallResult(raw: unknown): NormalizedResult<SharkHallResult> {
  ...
  if (raceTo === null || !SHARK_HALL_RACE_LENGTHS.includes(raceTo)) return refuse("invalid_race");
  // The CPU plays a single rack.
  if (mode === "cpu" && raceTo !== 1) return refuse("invalid_race");
  ...
  if (myRacks === null || opponentRacks === null || strokes === null || durationMs === null) return refuse("invalid_counts");
  if (myRacks === raceTo && opponentRacks === raceTo) return refuse("invalid_counts");
  if (outcome === "win" && myRacks !== raceTo) return refuse("invalid_counts");
  if (outcome === "loss" && opponentRacks !== raceTo) return refuse("invalid_counts");
  if (strokes < (myRacks + opponentRacks) * MIN_STROKES_PER_RACK) return refuse("invalid_counts");
  if (durationMs < strokes * MIN_MS_PER_STROKE) return refuse("implausible_duration");

  return { result: { resultId, mode, difficulty, raceTo, outcome, myRacks, opponentRacks, strokes, durationMs } };
}

export function calculateSharkHallTicketReward({ result }: { result: SharkHallResult }): TicketRewardBreakdown {
  ...
  if (result.mode === "cpu") {
    ...
    if (won) win = CPU_WIN_BONUS[result.difficulty ?? "casual"];
  } else {
    completion = Math.min(ONLINE_COMPLETION_CAP[result.raceTo] ?? 0, earnedByTime);
    if (won) win = ONLINE_WIN_BONUS[result.raceTo] ?? 0;
  }
