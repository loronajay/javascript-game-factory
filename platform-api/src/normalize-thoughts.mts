...
const THOUGHT_REACTION_ID_SET = new Set<string>(THOUGHT_REACTION_IDS);
...
function sanitizeThoughtReactionId(value: unknown): string {
  const normalized = sanitizeSingleLine(value, 24).toLowerCase();
  return THOUGHT_REACTION_ID_SET.has(normalized) ? normalized : "";
}

function sanitizeThoughtShareId(value: unknown): string {
  return sanitizeSingleLine(value, 80);
}

function sanitizeThoughtCommentId(value: unknown): string {
  return sanitizeSingleLine(value, 80);
}

function createFallbackThoughtId(): string {
  return `thought-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
}

function createFallbackCommentId(): string {
  return `comment-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
}

export function normalizeThoughtPost(post: any = {}, index = 0) {
...
export function normalizeThoughtComment(comment: any = {}, index = 0) {
  return {
