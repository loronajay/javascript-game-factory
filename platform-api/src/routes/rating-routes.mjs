...
function normalizeProgression(raw) {
    ...
    if (value.stats && typeof value.stats === "object") {
        for (const [key, entry] of Object.entries(value.stats)) {
            if (Object.keys(stats).length >= MAX_TRACK_STATS)
                break;
            if (!/^[a-zA-Z][a-zA-Z0-9]{0,30}$/.test(key))
                continue;
            const parsed = Math.floor(Number(entry));
            if (Number.isFinite(parsed))
                stats[key] = Math.max(0, Math.min(parsed, 1_000_000));
        }
    }
    return {
        trackId,
        ...
        forfeitRole: VALID_FORFEIT_ROLES.has(value.forfeitRole) ? value.forfeitRole : null,
        stats,
    };
}
...
export async function handleRatingRoute(context) {
