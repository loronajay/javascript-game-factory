...
export async function handleThoughtRoute(context) {
    ...
    const { listThoughts, listThoughtComments, saveThought, shareThought, commentOnThought, reactToThought, deleteThought, deleteThoughtComment, createNotification, deleteNotificationsByPayloadRef, } = services;
    ...
    if (method === "POST" && thoughtCommentMatch) {
        ...
        const commentRecord = await commentOnThought(decodeURIComponent(thoughtCommentMatch[1]), actorPlayerId, actorDisplayName, body.value?.text);
    ...
    if (method === "DELETE" && thoughtCommentDeleteMatch) {
        ...
        const result = await deleteThoughtComment(decodeURIComponent(thoughtCommentDeleteMatch[1]), decodeURIComponent(thoughtCommentDeleteMatch[2]), authClaims.playerId);
        if (!result?.ok) {
    ...
    if (method === "POST" && thoughtShareMatch) {
        ...
        const actorPlayerId = String(authClaims.playerId);
        const actorDisplayName = String(body.value?.viewerAuthorDisplayName || "");
        const share = await shareThought(decodeURIComponent(thoughtShareMatch[1]), actorPlayerId, actorDisplayName, body.value);
    ...
    if (method === "POST" && thoughtReactionMatch) {
        ...
        const thought = await reactToThought(decodeURIComponent(thoughtReactionMatch[1]), actorPlayerId, body.value?.reactionId);
        writeJson(res, 200, { thought }, requestOrigin);
