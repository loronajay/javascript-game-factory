...
async function readBody(req: any, res: any, requestOrigin: any, timestamp: any): Promise<any | null> {
...
function statusForError(error: string): number {
  ...
  if (error === "invalid_slug" || error === "invalid_key" || error === "invalid_request") return 400;
  if (error === "database_unavailable") return 503;
  return 500;
}

export async function handleAdminContentRoute(context: any): Promise<boolean> {
  const { req, res, method, pathname, adminPlayerId, requestOrigin, timestamp, services } = context;
  const {
    listAllBulletins, createBulletin, updateBulletin, deleteBulletin,
    listAllEvents, createEvent, updateEvent, deleteEvent,
    listCabinetOverrides, saveCabinetOverride, deleteCabinetOverride,
    listSiteSettings, saveSiteSetting,
    listReports, listSuspendedAccounts, listAdmins,
    writeAuditLog, announceBulletin, announceEvent,
  } = services || {};
  ...
  const announce = async (announcer: any, record: any) => {
    if (typeof announcer !== "function") return { announced: false };
    try {
  ...
  const announceDetails = (result: any) => ({
  ...
  if (method === "GET" && pathname === "/admin/overview") {
    ...
    writeJson(res, 200, {
      overview: {
        ...
        publishedBulletinCount: (bulletins || []).filter((entry: any) => entry.status === "published").length,
        eventCount: (events || []).length,
        upcomingEventCount: (events || []).filter((entry: any) => entry.status === "scheduled" || entry.status === "live").length,
