...
export async function handleGameProfileRoute(context) {
    ...
    if (!mineMatch && !playerMatch && !batchMatch)
        return false;
    ...
    if (mineMatch && (method === "GET" || method === "PUT")) {
        ...
        if (!result?.ok) {
            writeJson(res, result?.statusCode || 400, { status: "error", error: result?.error || "save_failed", timestamp }, requestOrigin);
            return true;
        }
