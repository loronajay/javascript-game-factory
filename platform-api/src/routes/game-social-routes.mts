...
const ROUTES: ReadonlyArray<[string, RegExp]> = Object.freeze([
...
const ERROR_STATUS: Record<string, number> = Object.freeze({
...
function matchRoute(pathname: string): { name: string; params: string[] } | null {
  for (const [name, pattern] of ROUTES) {
    const match = pathname.match(pattern);
    if (match) return { name, params: match.slice(1).map((value) => decodeURIComponent(value)) };
  }
  return null;
}

export async function handleGameSocialRoute(context: any): Promise<boolean> {
  ...
  const fail = (error: string, fallbackStatus = 500) => {
  ...
  const send = (result: any, unavailableError: string, payloadKey: string) => {
    if (!result) return fail(unavailableError);
  ...
  if (name === "requests" && method === "POST") {
    ...
    if (!body.ok) return fail(body.error || "invalid_body", 400);
    const recipientPlayerId = body.value?.recipientPlayerId;
    if (typeof recipientPlayerId !== "string" || !recipientPlayerId.trim()) return fail("missing_player", 400);
    return send(
      await services.sendGameFriendRequest(gameSlug, { requesterPlayerId: playerId, recipientPlayerId }),
      "request_failed",
      "",
    );
  }
  ...
  if ((name === "requestAccept" || name === "requestDecline") && method === "POST") {
    ...
    return send(
      await services.respondToGameFriendRequest(gameSlug, { requestId: params[1], playerId, action }),
      "respond_failed",
      "",
    );
  }
  ...
  if (name === "request" && method === "DELETE") {
    return send(
      await services.cancelGameFriendRequest(gameSlug, { requestId: params[1], playerId }),
      "cancel_failed",
      "",
    );
  }
  ...
  if (name === "relationship" && method === "GET") {
    return send(
      await services.getGamePlayerRelationship(gameSlug, { viewerPlayerId: playerId, playerId: params[1] }),
