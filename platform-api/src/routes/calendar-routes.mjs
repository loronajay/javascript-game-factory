...
function unauthorized(res, timestamp, requestOrigin) {
    writeJson(res, 401, { status: "error", error: "unauthorized", timestamp }, requestOrigin);
    return true;
}
export async function handleCalendarRoute(context) {
    const { req, res, method, pathname, authClaims, requestOrigin, timestamp, services } = context;
    const { createCalendarCheckoutSession, fulfillCalendarCheckoutFromReturn, listCalendarOrdersForPlayer, } = services;
    ...
    if (method === "POST" && pathname === "/calendar/checkout-sessions") {
        ...
        if (!authClaims?.playerId)
            return unauthorized(res, timestamp, requestOrigin);
