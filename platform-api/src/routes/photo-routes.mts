...
function buildPhotoThoughtRecord({ playerId, authorDisplayName, subject, text, visibility, imageUrl, caption }: any): any {
...
function buildPhotoNotificationPayload(type: any, { actorPlayerId, actorDisplayName, commentRecord, photo }: any): any {
...
export async function handlePhotoRoute(context: any): Promise<boolean> {
  const {
    req,
    res,
    method,
  ...
  if (method === "DELETE" && photoCommentDeleteMatch) {
    ...
    const result = await deletePhotoComment(
      decodeURIComponent(photoCommentDeleteMatch[1]),
      decodeURIComponent(photoCommentDeleteMatch[2]),
      authClaims.playerId,
    );
