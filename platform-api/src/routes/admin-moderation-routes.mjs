...
function statusForError(error) {
    if (error === "not_found" || error === "no_account")
        return 404;
    if (error === "last_admin" || error === "not_suspendable")
        return 409;
    if (error === "invalid_request" || error === "unsupported_target")
        return 400;
    if (error === "database_unavailable")
        return 503;
    return 500;
}
export async function handleAdminModerationRoute(context) {
    const { req, res, method, pathname, requestUrl, adminPlayerId, requestOrigin, timestamp, services } = context;
    const { listReports, resolveReport, removeContentAsAdmin, listSuspendedAccounts, suspendAccount, liftSuspension, listAdmins, setAdminFlag, listAuditLog, writeAuditLog, } = services || {};
