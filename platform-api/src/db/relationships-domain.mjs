import { DIRECT_INTERACTION_POINTS, FRIENDSHIP_CREATION_POINTS, SHARED_EVENT_POINTS, SHARED_SESSION_POINTS, buildDefaultProfileRelationshipsRecord, normalizeProfileRelationshipsRecord, } from "../normalize.mjs";
...
function compareRelationshipPriority(record, leftPlayerId, rightPlayerId) {
    ...
    const leftRecent = Math.max(parseTimestamp(record.lastSharedSessionAtByPlayerId?.[leftPlayerId]), parseTimestamp(record.lastSharedEventAtByPlayerId?.[leftPlayerId]), parseTimestamp(record.lastInteractionAtByPlayerId?.[leftPlayerId]));
    const rightRecent = Math.max(parseTimestamp(record.lastSharedSessionAtByPlayerId?.[rightPlayerId]), parseTimestamp(record.lastSharedEventAtByPlayerId?.[rightPlayerId]), parseTimestamp(record.lastInteractionAtByPlayerId?.[rightPlayerId]));
...
function reorderFriendPlayerIds(record) {
    ...
    record.friendPlayerIds = [...current].sort((leftPlayerId, rightPlayerId) => (compareRelationshipPriority(record, leftPlayerId, rightPlayerId)));
}
...
export function createEmptyRelationshipResult(leftPlayerId = "", rightPlayerId = "") {
    return buildFriendshipResult(buildDefaultProfileRelationshipsRecord(leftPlayerId), buildDefaultProfileRelationshipsRecord(rightPlayerId), false, 0);
}
export function applyFriendshipState(leftRecord, rightRecord, options = {}) {
    ...
    return buildFriendshipResult(normalizedLeftRecord, normalizedRightRecord, !alreadyAwarded, alreadyAwarded ? 0 : FRIENDSHIP_CREATION_POINTS);
}
export function applySharedSessionState(leftRecord, rightRecord, options = {}) {
    ...
    setRecordMapTimestamp(normalizedLeftRecord, "lastSharedSessionAtByPlayerId", rightPlayerId, occurredAt);
    setRecordMapTimestamp(normalizedRightRecord, "lastSharedSessionAtByPlayerId", leftPlayerId, occurredAt);
    if (!alreadyAwarded) {
    ...
    return buildFriendshipResult(normalizedLeftRecord, normalizedRightRecord, !alreadyAwarded, alreadyAwarded ? 0 : SHARED_SESSION_POINTS);
}
export function applySharedEventState(leftRecord, rightRecord, options = {}) {
    ...
    const occurredAt = createTimestamp(options.occurredAt);
    const alreadyAwarded = !!options.alreadyAwarded;
    setRecordMapTimestamp(normalizedLeftRecord, "lastSharedEventAtByPlayerId", rightPlayerId, occurredAt);
    setRecordMapTimestamp(normalizedRightRecord, "lastSharedEventAtByPlayerId", leftPlayerId, occurredAt);
    if (!alreadyAwarded) {
    ...
    reorderFriendPlayerIds(normalizedLeftRecord);
    reorderFriendPlayerIds(normalizedRightRecord);
    return buildFriendshipResult(normalizedLeftRecord, normalizedRightRecord, !alreadyAwarded, alreadyAwarded ? 0 : SHARED_EVENT_POINTS);
}
...
export function applyDirectInteractionState(leftRecord, rightRecord, options = {}) {
    ...
    setRecordMapTimestamp(normalizedLeftRecord, "lastInteractionAtByPlayerId", rightPlayerId, occurredAt);
    setRecordMapTimestamp(normalizedRightRecord, "lastInteractionAtByPlayerId", leftPlayerId, occurredAt);
    if (canAward) {
    ...
    reorderFriendPlayerIds(normalizedLeftRecord);
    reorderFriendPlayerIds(normalizedRightRecord);
    return buildFriendshipResult(normalizedLeftRecord, normalizedRightRecord, canAward, canAward ? DIRECT_INTERACTION_POINTS : 0);
}
