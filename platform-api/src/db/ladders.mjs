...
function toRunPlacement(row, ladder) {
...
async function runRecordPlacement(pool, ladder, playerId) {
    ...
    try {
       ...
       select * from standings where player_id = $3`, [ladder.gameSlug, board.id, playerId]);
...
export async function getPlayerLadderPlacements(pool, params = {}) {
    ...
    const runPlacements = (await Promise.all(runLadders.map((ladder) => runRecordPlacement(pool, ladder, playerId)))).filter(Boolean);
    if (!ratingLadders.length) {
        return {
            playerId,
            placements: runPlacements.sort((a, b) => a.rank - b.rank).slice(0, limit),
        };
    }
    try {
       ...
       select * from standings
        ...
        const ratingPlacements = (res.rows || []).map((row) => toPlacement(row, getLadder(row.game_slug)));
        ...
        return {
            ...
            placements: [...ratingPlacements, ...runPlacements]
                .sort((a, b) => a.rank - b.rank || String(a.gameSlug).localeCompare(String(b.gameSlug)))
                .slice(0, limit),
        };
    }
    catch (err) {
...
async function runLadderStandings(pool, ladder, limit) {
    const board = getBoard(ladder.gameSlug, ladder.boardId);
    if (!board)
        return null;
    try {
        const res = await pool.query(`select r.player_id, r.value, r.model_id, r.verified, r.recorded_at,
        ...
        limit $3`, [ladder.gameSlug, board.id, limit]);
...
export async function getLadderStandings(pool, params = {}) {
    ...
    if (ladder.source === "run-records")
        return runLadderStandings(pool, ladder, limit);
    try {
