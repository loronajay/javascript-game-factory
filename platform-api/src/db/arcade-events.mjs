...
export async function listPublicEvents(pool, options = {}) {
    if (!pool)
        return [];
    const limit = Math.min(Math.max(Number.parseInt(String(options?.limit || "50"), 10) || 50, 1), 200);
    try {
        const result = await pool.query(`select ${SELECT_COLUMNS} from arcade_events
...
export async function listAllEvents(pool) {
    if (!pool)
        return [];
    try {
        const result = await pool.query(`select ${SELECT_COLUMNS} from arcade_events order by starts_at desc nulls last, title asc`);
        return (result?.rows || []).map(mapRow);
    }
    catch {
        return [];
...
export async function getPublicEventBySlug(pool, slug) {
    if (!pool || !slug)
        return null;
    try {
        const result = await pool.query(`select ${SELECT_COLUMNS} from arcade_events where slug = $1 and status <> 'cancelled'`, [String(slug)]);
...
export async function createEvent(pool, input, createdBy) {
    if (!pool)
    ...
    try {
        const result = await pool.query(`insert into arcade_events
       ...
       returning ${SELECT_COLUMNS}`, [
        ...
        process.stderr.write(`[arcade-events] createEvent error: ${err?.message || err}\n`);
        return { ok: false, error: "server_error" };
    }
}
export async function updateEvent(pool, id, input) {
    if (!pool || !id)
    ...
    if (!values.slug)
        return { ok: false, error: "invalid_slug" };
    try {
        const result = await pool.query(`update arcade_events
        ...
        if (err?.code === "23505")
            return { ok: false, error: "slug_taken" };
        process.stderr.write(`[arcade-events] updateEvent error: ${err?.message || err}\n`);
        return { ok: false, error: "server_error" };
    }
}
