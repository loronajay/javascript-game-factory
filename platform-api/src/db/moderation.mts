...
const TARGET_TYPES = ["thought", "thought_comment", "photo", "photo_comment", "player"] as const;
const REPORT_REASONS = ["spam", "harassment", "hate", "sexual", "violence", "impersonation", "other"] as const;
...
function mapReportRow(row: any): ContentReport {
  return {
    id: String(row.id),
...
export async function fileReport(pool: any, input: any = {}): Promise<{ ok: boolean; error?: string; duplicate?: boolean }> {
  ...
  if (!targetId || !reporterPlayerId) return { ok: false, error: "invalid_request" };
  ...
  try {
    ...
    process.stderr.write(`[moderation] fileReport error: ${(err as any)?.message || err}\n`);
    return { ok: false, error: "server_error" };
  }
}

export async function listReports(pool: any, options: any = {}): Promise<ContentReport[]> {
  if (!pool) return [];
  const status = enumValue(options?.status, [...REPORT_STATUSES, "all" as const], "open");
  const limit = Math.min(Math.max(Number.parseInt(String(options?.limit || "100"), 10) || 100, 1), 250);

  try {
    const result = await pool.query(
      `select r.id, r.target_type, r.target_id, r.target_owner_player_id, r.reporter_player_id,
...
export async function resolveReport(pool: any, id: any, status: any, adminPlayerId: any): Promise<{ ok: boolean; error?: string }> {
  if (!pool || !id) return { ok: false, error: "invalid_request" };
  const nextStatus = enumValue(status, ["resolved", "dismissed"] as const, "resolved");
  try {
    const result = await pool.query(
      `update content_reports
          set status = $2, resolved_at = now(), resolved_by = $3
        where id = $1 and status = 'open'`,
      [String(id), nextStatus, String(adminPlayerId || "")],
    );
...
const REMOVAL_TARGETS: Record<string, { table: string; label: string }> = {
...
export async function removeContentAsAdmin(pool: any, targetType: any, targetId: any): Promise<{ ok: boolean; error?: string }> {
  if (!pool) return { ok: false, error: "database_unavailable" };

  const target = REMOVAL_TARGETS[enumValue(targetType, TARGET_TYPES, "thought")];
  const id = singleLine(targetId, 120);
  if (!target) return { ok: false, error: "unsupported_target" };
  ...
  try {
    ...
    process.stderr.write(`[moderation] removeContentAsAdmin error: ${(err as any)?.message || err}\n`);
    return { ok: false, error: "server_error" };
  }
}
...
export async function suspendAccount(pool: any, playerId: any, options: any = {}): Promise<{ ok: boolean; error?: string; until?: string }> {
  if (!pool || !playerId) return { ok: false, error: "invalid_request" };
  ...
  try {
    const result = await pool.query(
      `update accounts set suspended_until = $2, suspended_reason = $3, updated_at = now()
        where player_id = $1 and is_admin = false
        returning player_id`,
      [String(playerId), until, reason],
    );
    ...
    process.stderr.write(`[moderation] suspendAccount error: ${(err as any)?.message || err}\n`);
    return { ok: false, error: "server_error" };
  }
}

export async function liftSuspension(pool: any, playerId: any): Promise<{ ok: boolean; error?: string }> {
  if (!pool || !playerId) return { ok: false, error: "invalid_request" };
  try {
    const result = await pool.query(
      `update accounts set suspended_until = null, suspended_reason = null, updated_at = now()
        where player_id = $1`,
      [String(playerId)],
    );
...
export async function listSuspendedAccounts(pool: any): Promise<any[]> {
  if (!pool) return [];
  try {
    const result = await pool.query(
