...
function computeNewRatings(ratingA, ratingB, outcomeA) {
...
export async function getGameRating(pool, playerId, gameSlug) {
    if (!pool || !playerId || !gameSlug)
        return null;
    try {
        const result = await pool.query(`select player_id, game_slug, rating, wins, losses, draws, last_match_at
       from game_ratings where player_id = $1 and game_slug = $2`, [playerId, gameSlug]);
        if (!result.rows.length)
            return { playerId, gameSlug, rating: DEFAULT_RATING, wins: 0, losses: 0, draws: 0, lastMatchAt: null };
...
export async function recordMatchRating(pool, { reporterPlayerId, opponentPlayerId, gameSlug, outcome, sessionId, occurredAt }) {
    ...
    `;
        ...
        process.stderr.write(`[ratings] recordMatchRating error: ${err?.message || err}\n`);
        return null;
    }
    finally {
        client.release();
