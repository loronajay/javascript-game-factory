...
import { CALENDAR_GAME_SLUG, CALENDAR_PRODUCT_ID, } from "../services/calendar-catalog.mjs";
...
const PAYMENT_STATES = ["pending", "paid", "refunded", "cancelled"];
...
export function isFulfillmentState(value) {
    return FULFILLMENT_STATES.includes(cleanText(value, 40));
}
/**
 ...
 */
export function newCalendarOrderId(randomImpl = () => randomBytes(8)) {
...
export function isShippableAddress(address) {
...
export function toCustomerOrder(order) {
    if (!order)
        return null;
    const { checkoutSessionId, paymentIntentId, adminNote, playerId, voucherBonusState, ...rest } = order;
...
export async function createCalendarOrder(pool, params = {}) {
    ...
    const res = await pool.query(`insert into calendar_orders (
    ...
    if (res.rowCount)
        return { ok: true, order: rowToOrder(res.rows[0]), created: true };
...
export async function getCalendarOrderBySession(pool, checkoutSessionId) {
...
export async function getCalendarOrder(pool, orderId) {
...
export async function findCalendarOrderByPaymentIntent(pool, paymentIntentId) {
 ...
 */
export async function markCalendarOrderPaid(pool, params = {}) {
    const checkoutSessionId = cleanText(params.checkoutSessionId, 200);
    if (!pool || !checkoutSessionId)
        return { ok: false, statusCode: 400, error: "invalid_order" };
    const address = normalizeShippingAddress(params.shippingAddress);
    const res = await pool.query(`update calendar_orders set
       payment_state = 'paid',
    ...
    if (res.rowCount)
        return { ok: true, order: rowToOrder(res.rows[0]), alreadyPaid: false };
    const existing = await getCalendarOrderBySession(pool, checkoutSessionId);
    return existing
...
export async function recordCalendarBonus(pool, params = {}) {
    const orderId = cleanText(params.orderId, 40);
    const state = cleanText(params.state, 30);
    if (!pool || !orderId || !state)
        return null;
    const res = await pool.query(`update calendar_orders
     set voucher_bonus_state = $2,
         voucher_bonus_claim_id = coalesce(nullif($3, ''), voucher_bonus_claim_id),
         voucher_bonus_quantity = $4,
         updated_at = now()
     where order_id = $1
     returning *`, [orderId, state, cleanText(params.claimId, 200), clampInt(params.quantity, 0)]);
    return rowToOrder(res.rows[0]);
}
/**
 ...
 */
export async function markCalendarOrderClosed(pool, params = {}) {
    const paymentState = cleanText(params.paymentState, 30);
    if (!pool || !PAYMENT_STATES.includes(paymentState)) {
    ...
    const res = await pool.query(`update calendar_orders
     ...
     returning *`, [sessionId, orderId, paymentState]);
    return res.rowCount
        ? { ok: true, order: rowToOrder(res.rows[0]) }
        : { ok: false, statusCode: 404, error: "order_not_found" };
}
export async function listCalendarOrdersForPlayer(pool, playerId) {
    const id = cleanText(playerId, 120);
    if (!pool || !id)
        return [];
    const res = await pool.query(`${SELECT} where player_id = $1 order by created_at desc limit 50`, [id]);
    return res.rows.map(rowToOrder);
}
export async function listCalendarOrders(pool, options = {}) {
    if (!pool)
        return [];
    const filters = [];
    const values = [];
    const paymentState = cleanText(options.paymentState, 30);
    if (PAYMENT_STATES.includes(paymentState)) {
    ...
    const res = await pool.query(`${SELECT} ${where} order by created_at desc limit $${values.length}`, values);
    return res.rows.map(rowToOrder);
}
export async function updateCalendarFulfillment(pool, params = {}) {
    const orderId = cleanText(params.orderId, 40);
    if (!pool || !orderId)
        return { ok: false, statusCode: 400, error: "invalid_order" };
    ...
    const res = await pool.query(`update calendar_orders set
       ...
       updated_at = now()
     where order_id = $1
     returning *`, [orderId, fulfillmentState, trackingNumber, carrier, note]);
    return res.rowCount
        ? { ok: true, order: rowToOrder(res.rows[0]) }
        : { ok: false, statusCode: 404, error: "order_not_found" };
}
 ...
 */
export async function getCalendarPreorderMetrics(pool) {
    if (!pool)
        return null;
