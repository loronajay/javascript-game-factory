...
export async function getGamePlayerBadges(pool: any, { gameSlug, playerId }: any): Promise<any> {
  ...
  try {
    ...
    const derived = derivedBadgesForEntitlements(
      gameSlug,
      (entitlements.rows || []).map((row: any) => row.entitlement_id),
    );
    ...
    const explicit = (awarded.rows || [])
      .map((row: any) => {
        ...
        if (!badge || seen.has(badge.id)) return null;
        seen.add(badge.id);
...
export async function awardGameBadge(pool: any, { gameSlug, playerId, badgeId, source = "award" }: any): Promise<any> {
  const target = cleanPlayerId(playerId);
  ...
  if (!target || !id) return { error: "missing_badge" };
  if (!isAwardableGameBadge(gameSlug, id)) return { error: "badge_not_awardable" };

  try {
    await pool.query(
      `insert into game_player_badges (player_id, game_slug, badge_id, source)
       values ($1, $2, $3, $4) on conflict do nothing`,
      [target, gameSlug, id, String(source || "award").slice(0, 60)],
    );
