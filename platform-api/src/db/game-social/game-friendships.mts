...
export async function listGameFriends(pool: any, { gameSlug, playerId }: any): Promise<any> {
...
export async function removeGameFriend(pool: any, { gameSlug, playerId, otherPlayerId }: any): Promise<any> {
  if (!pool || !isValidGameSocialSlug(gameSlug)) return { error: "invalid_game_slug" };
  const pair = canonicalPair(playerId, otherPlayerId);
  if (!pair) return { error: "missing_player" };

  try {
    const res = await pool.query(
      `delete from game_friendships
        where game_slug = $1 and player_id_a = $2 and player_id_b = $3
        returning game_slug`,
      [gameSlug, pair[0], pair[1]],
    );
    if (!res.rows?.length) return { error: "not_friends" };
...
export async function areGameFriends(pool: any, { gameSlug, playerId, otherPlayerId }: any): Promise<boolean> {
  if (!pool || !isValidGameSocialSlug(gameSlug)) return false;
  const pair = canonicalPair(playerId, otherPlayerId);
