...
import { cleanLimit, cleanPlayerId, isValidGameSocialSlug, logGameSocialError, serializePlayerSummary, } from "./game-social-shared.mjs";
...
export async function searchGamePlayers(pool, { gameSlug, viewerPlayerId, query, limit }) {
    const viewer = cleanPlayerId(viewerPlayerId);
    const search = cleanQuery(query);
    if (!pool || !isValidGameSocialSlug(gameSlug) || !viewer)
        return null;
    if (!search)
        return { gameSlug, query: "", results: [] };
    const cap = cleanLimit(limit, { fallback: 25, max: 50 });
    try {
        const res = await pool.query(`select gr.player_id as other_player_id,
              pp.profile_name,
        ...
        limit $5`, [gameSlug, viewer, likePattern(search), search, cap]);
        return {
            gameSlug,
            query: search,
...
function relationshipFor(row) {
    if (row.is_friend)
        return "friend";
    if (row.outgoing_request_id)
        return "request-sent";
    if (row.incoming_request_id)
...
export async function getGamePlayerRelationship(pool, { gameSlug, viewerPlayerId, playerId }) {
    const viewer = cleanPlayerId(viewerPlayerId);
    const target = cleanPlayerId(playerId);
    if (!pool || !isValidGameSocialSlug(gameSlug) || !viewer || !target)
        return null;
    if (viewer === target)
        return { gameSlug, playerId: target, relationship: "self", requestId: null, blocked: false };
    try {
        const res = await pool.query(`select
         ...
         ) as they_blocked_me`, [gameSlug, viewer, target]);
