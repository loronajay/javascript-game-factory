...
const CAMPAIGN_FINALE_MISSION_ID: Record<string, string> = Object.freeze({
...
export async function loadPlayerBadgeFacts(pool: any, { gameSlug, playerId }: any): Promise<any> {
  ...
  const [ranked, campaign] = await Promise.all([
    ...
    pool.query(
      `select rating, peak_rating from game_ratings where player_id = $1 and game_slug = $2 limit 1`,
      [playerId, gameSlug],
    ),
    finaleMissionId
      ? pool.query(
        `select 1 from game_campaign_progress
          ...
          limit 1`,
        [playerId, gameSlug, finaleMissionId],
      )
 ...
 */
export async function syncAutoAwardedBadges(pool: any, { gameSlug, playerId, heldBadgeIds }: any): Promise<string[]> {
  try {
    ...
    const pending = autoAwardableGameBadges(gameSlug, facts).filter((badge: any) => !held.has(badge.id));
    ...
    for (const badge of pending) {
      ...
      const res = await pool.query(
        `insert into game_player_badges (player_id, game_slug, badge_id, source)
         values ($1, $2, $3, $4) on conflict do nothing
         returning badge_id`,
        [playerId, gameSlug, badge.id, source],
      );
      if ((res.rows || []).length) awarded.push(badge.id);
    }
    return awarded;
  } catch (err: any) {
