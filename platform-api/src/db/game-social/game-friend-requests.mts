...
async function pairIsBlocked(client: any, gameSlug: string, a: string, b: string): Promise<boolean> {
  const res = await client.query(
    `select 1 from game_friend_blocks
      where game_slug = $1
...
async function alreadyFriends(client: any, gameSlug: string, a: string, b: string): Promise<boolean> {
  const pair = canonicalPair(a, b);
  if (!pair) return false;
  const res = await client.query(
    `select 1 from game_friendships where game_slug = $1 and player_id_a = $2 and player_id_b = $3 limit 1`,
...
export async function sendGameFriendRequest(pool: any, { gameSlug, requesterPlayerId, recipientPlayerId }: any): Promise<any> {
  ...
  if (!requester || !recipient) return { error: "missing_player" };
  if (requester === recipient) return { error: "cannot_friend_self" };
  ...
  try {
    ...
    const inbound = await client.query(
      `select id from game_friend_requests
        where game_slug = $1 and requester_player_id = $2 and recipient_player_id = $3 and status = 'pending'
        for update`,
      [gameSlug, recipient, requester],
    );
    if (inbound.rows?.length) {
      await client.query(
        `update game_friend_requests set status = 'accepted', responded_at = now() where id = $1`,
        [inbound.rows[0].id],
      );
      const pair = canonicalPair(requester, recipient)!;
      await client.query(
        `insert into game_friendships (game_slug, player_id_a, player_id_b)
    ...
    const inserted = await client.query(
      `insert into game_friend_requests (game_slug, requester_player_id, recipient_player_id, status)
...
export async function respondToGameFriendRequest(pool: any, { gameSlug, requestId, playerId, action }: any): Promise<any> {
  const responder = cleanPlayerId(playerId);
  const id = cleanRequestId(requestId);
  if (!pool || !isValidGameSocialSlug(gameSlug)) return { error: "invalid_game_slug" };
  if (!responder || !id) return { error: "missing_request" };
  if (action !== "accept" && action !== "decline") return { error: "invalid_action" };

  const client = await pool.connect();
  try {
    await client.query("begin");
    const res = await client.query(
      `select * from game_friend_requests where id = $1 and game_slug = $2 for update`,
      [id, gameSlug],
    );
    ...
    if (action === "decline") {
      await client.query(
        `update game_friend_requests set status = 'declined', responded_at = now() where id = $1`,
        [id],
      );
      await client.query("commit");
...
export async function cancelGameFriendRequest(pool: any, { gameSlug, requestId, playerId }: any): Promise<any> {
  const requester = cleanPlayerId(playerId);
  const id = cleanRequestId(requestId);
  if (!pool || !isValidGameSocialSlug(gameSlug)) return { error: "invalid_game_slug" };
  if (!requester || !id) return { error: "missing_request" };

  try {
    const res = await pool.query(
      `update game_friend_requests
          set status = 'canceled', responded_at = now()
        where id = $1 and game_slug = $2 and requester_player_id = $3 and status = 'pending'
        returning id`,
      [id, gameSlug, requester],
    );
    if (!res.rows?.length) return { error: "request_not_found" };
...
export async function listGameFriendRequests(pool: any, { gameSlug, playerId }: any): Promise<any> {
  const me = cleanPlayerId(playerId);
  if (!pool || !isValidGameSocialSlug(gameSlug) || !me) return null;

  try {
    const incoming = await pool.query(
      `select r.*, ${playerSummarySelect("r", "requester_player_id")}
    ...
    const outgoing = await pool.query(
      `select r.*, ${playerSummarySelect("r", "recipient_player_id")}
         from game_friend_requests r
