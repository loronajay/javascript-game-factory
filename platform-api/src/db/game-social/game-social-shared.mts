...
export const GAME_SOCIAL_SLUGS: ReadonlySet<string> = new Set(["tactical-arena"]);

export function isValidGameSocialSlug(slug: unknown): boolean {
...
export function cleanRequestId(value: unknown): number | null {
  const id = Math.floor(Number(value));
  return Number.isFinite(id) && id > 0 ? id : null;
}

export function cleanLimit(value: unknown, { fallback = 25, max = 100 } = {}): number {
...
export function canonicalPair(playerIdA: unknown, playerIdB: unknown): [string, string] | null {
...
export function serializeFriendRequest(row: any): any {
  if (!row) return null;
  return {
...
export function serializePlayerSummary(row: any): any {
  if (!row) return null;
  const raw = row.rating == null ? null : Number(row.rating);
  const rating = Number.isFinite(raw as number) ? (raw as number) : null;
...
export function playerSummarySelect(alias: string, idColumn: string): string {
...
export function playerSummaryJoins(alias: string, idColumn: string, slugParam: string): string {
...
export function logGameSocialError(operation: string, err: any): void {
