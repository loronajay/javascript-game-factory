...
export async function blockGamePlayer(pool, { gameSlug, playerId, otherPlayerId }) {
    ...
    try {
        ...
        await client.query(`insert into game_friend_blocks (game_slug, blocker_player_id, blocked_player_id)
       values ($1, $2, $3) on conflict do nothing`, [gameSlug, blocker, blocked]);
        // Any pending request either way is resolved, not left dangling.
        await client.query(`update game_friend_requests set status = 'declined', responded_at = now()
        where game_slug = $1 and status = 'pending'
          and ((requester_player_id = $2 and recipient_player_id = $3)
            or (requester_player_id = $3 and recipient_player_id = $2))`, [gameSlug, blocker, blocked]);
        const pair = canonicalPair(blocker, blocked);
        if (pair) {
            await client.query(`delete from game_friendships where game_slug = $1 and player_id_a = $2 and player_id_b = $3`, [gameSlug, pair[0], pair[1]]);
        }
...
export async function unblockGamePlayer(pool, { gameSlug, playerId, otherPlayerId }) {
    const blocker = cleanPlayerId(playerId);
    const blocked = cleanPlayerId(otherPlayerId);
    if (!pool || !isValidGameSocialSlug(gameSlug))
        return { error: "invalid_game_slug" };
    if (!blocker || !blocked)
        return { error: "missing_player" };
    try {
        const res = await pool.query(`delete from game_friend_blocks
        where game_slug = $1 and blocker_player_id = $2 and blocked_player_id = $3
        returning game_slug`, [gameSlug, blocker, blocked]);
        if (!res.rows?.length)
            return { error: "not_blocked" };
...
export async function listGameBlocks(pool, { gameSlug, playerId }) {
    const me = cleanPlayerId(playerId);
    if (!pool || !isValidGameSocialSlug(gameSlug) || !me)
        return null;
    try {
        const res = await pool.query(`select b.blocked_player_id as other_player_id, b.created_at,
              pp.profile_name,
        ...
        limit 200`, [gameSlug, me]);
        return {
            gameSlug,
            playerId: me,
            blocked: (res.rows || []).map((row) => ({
