...
export async function isAdminPlayer(pool: any, playerId: any): Promise<boolean> {
  if (!pool || !playerId) return false;
  try {
    const result = await pool.query(
      "select is_admin from accounts where player_id = $1",
      [String(playerId)],
    );
...
export async function getAccountSuspension(pool: any, playerId: any): Promise<{ until: string; reason: string } | null> {
  if (!pool || !playerId) return null;
  try {
    const result = await pool.query(
      `select suspended_until, suspended_reason
         from accounts
        where player_id = $1 and suspended_until is not null and suspended_until > now()`,
      [String(playerId)],
    );
...
export async function seedAdminsFromEmails(pool: any, rawEmails: unknown): Promise<string[]> {
  if (!pool) return [];
  ...
  try {
    const result = await pool.query(
      `update accounts set is_admin = true, updated_at = now()
        where lower(email) = any($1::text[]) and is_admin = false
        returning email`,
      [emails],
    );
    return (result?.rows || []).map((row: any) => String(row.email || ""));
  } catch (err) {
    process.stderr.write(`[admin] seedAdminsFromEmails error: ${(err as any)?.message || err}\n`);
    return [];
  }
}

export async function listAdmins(pool: any): Promise<AdminAccount[]> {
  if (!pool) return [];
  try {
    const result = await pool.query(
...
export async function setAdminFlag(pool: any, playerId: any, isAdmin: boolean): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    ...
    if (!isAdmin) {
      const remaining = await client.query(
        "select count(*)::int as count from accounts where is_admin and player_id <> $1",
        [String(playerId)],
      );
    ...
    const updated = await client.query(
      "update accounts set is_admin = $2, updated_at = now() where player_id = $1 returning player_id",
      [String(playerId), isAdmin === true],
    );
    if (!updated?.rowCount) {
      await client.query("rollback");
    ...
    process.stderr.write(`[admin] setAdminFlag error: ${(err as any)?.message || err}\n`);
...
export async function writeAuditLog(pool: any, entry: any = {}): Promise<void> {
  if (!pool || !entry?.adminPlayerId || !entry?.action) return;
  try {
    await pool.query(
      `insert into admin_audit_log (admin_player_id, action, target_type, target_id, details)
    ...
    process.stderr.write(`[admin] writeAuditLog error: ${(err as any)?.message || err}\n`);
  }
}

export async function listAuditLog(pool: any, options: any = {}): Promise<AuditEntry[]> {
  if (!pool) return [];
