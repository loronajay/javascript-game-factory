...
function normalizeSquad(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw.map((entry) => text(entry, 40)).filter(Boolean).slice(0, MAX_SQUAD);
}
...
function normalizeParticipant(raw, { viewerPlayerId, units, tallies }) {
    ...
    const fromBoard = units
        ? units.filter((unit) => unit.seat === seat).map((unit) => unit.unitType)
        : null;
    return {
        playerId,
        seat,
 ...
 */
export function buildMatchHistoryEntry(record, { viewerPlayerId = null, detail = false } = {}) {
    ...
    const participants = orderParticipants((Array.isArray(record.participants) ? record.participants : [])
        .slice(0, MAX_PARTICIPANTS)
        .map((raw) => normalizeParticipant(raw, { viewerPlayerId: viewerId, units, tallies })));
    const startedAt = isoOrNull(record.startedAt);
    const endedAt = isoOrNull(record.endedAt);
    const me = participants.find((p) => p.isViewer) || null;
    const entry = {
    ...
    if (detail) {
        entry.units = (units || []).map((unit) => {
            const owner = participants.find((p) => p.seat === unit.seat) || null;
            return {
                id: unit.id,
