import { DIRECT_INTERACTION_WINDOW_LIMIT, DIRECT_INTERACTION_WINDOW_MS, FRIENDSHIP_CREATION_POINTS, SHARED_EVENT_POINTS, SHARED_SESSION_POINTS, buildDefaultProfileRelationshipsRecord, normalizeProfileRelationshipsRecord, } from "../normalize.mjs";
import { loadPlayerMetrics, savePlayerMetrics } from "./metrics.mjs";
import { applyDirectInteractionState, applyFriendRemovalState, applyFriendshipState, applySharedEventState, applySharedSessionState, buildFriendshipResult, buildRelationshipPairKey, buildRelationshipsParams, createEmptyRelationshipResult, createTimestamp, mapRowToRelationshipsRecord, } from "./relationships-domain.mjs";
...
export async function createFriendshipBetweenPlayers(db, leftPlayerId, rightPlayerId, options = {}) {
    ...
    try {
        ...
        const savedLeftRecord = await savePlayerRelationships(client, normalizedLeftPlayerId, stateResult.leftRecord);
        const savedRightRecord = await savePlayerRelationships(client, normalizedRightPlayerId, stateResult.rightRecord);
    ...
    `, [normalizedRightPlayerId, JSON.stringify(rightFriendIds)]);
        ...
        return buildFriendshipResult(savedLeftRecord, savedRightRecord, stateResult.awarded, stateResult.awardedPoints);
    }
    catch (error) {
