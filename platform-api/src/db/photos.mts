...
const PHOTO_REACTION_ID_SET = new Set<string>(THOUGHT_REACTION_IDS);
...
function createPhotoCommentId(photoId: string, viewerPlayerId: string): string {
  return `pcomment-${viewerPlayerId.slice(0, 24)}-${photoId.slice(0, 24)}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}

function sanitizePhotoId(value: unknown): string {
  return typeof value === "string" ? value.trim().slice(0, 80) : "";
}

function sanitizeViewerPlayerId(value: unknown): string {
  return typeof value === "string" ? value.trim().slice(0, 80) : "";
}

function sanitizePhotoReactionId(value: unknown): string {
...
function sanitizeViewerAuthorDisplayName(value: unknown): string {
  return typeof value === "string" ? value.trim().slice(0, 60) : "";
}

function sanitizeReactionTotals(value: unknown): Record<string, number> {
...
function mapRowToPhoto(row: any = {}): any {
...
function mapRowToPhotoComment(row: any = {}): any {
  if (!row?.id) return null;
  return {
    id: String(row.id),
...
export async function savePlayerPhoto(db: any, { playerId, assetId, imageUrl, caption = "", visibility = "public" }: any = {}): Promise<any> {
...
export async function listPlayerPhotos(db: any, playerId: any, { limit = 40, visibility = null }: any = {}): Promise<any> {
...
export async function getPlayerPhoto(db: any, photoId: any, { viewerPlayerId = "" }: any = {}): Promise<any> {
...
export async function deletePlayerPhoto(db: any, photoId: any, playerId: any): Promise<boolean> {
...
export async function reactToPhoto(db: any, photoId: any, viewerPlayerId: any, reactionId: any): Promise<any> {
  const safePhotoId = sanitizePhotoId(photoId);
  const safeViewerPlayerId = sanitizeViewerPlayerId(viewerPlayerId);
...
export async function commentOnPhoto(db: any, photoId: any, viewerPlayerId: any, authorDisplayName: any, text: any): Promise<any> {
  const safePhotoId = sanitizePhotoId(photoId);
  const safeViewerPlayerId = sanitizeViewerPlayerId(viewerPlayerId);
...
export async function deletePhotoComment(
  ...
  photoId: any,
  commentId: any,
  requesterPlayerId: any,
): Promise<{ ok: boolean; reason?: string; photo?: any; commentId?: string; commentCount?: number }> {
  const safePhotoId = sanitizePhotoId(photoId);
...
export async function listPhotoComments(db: any, photoId: any, { limit = 100 }: any = {}): Promise<any> {
