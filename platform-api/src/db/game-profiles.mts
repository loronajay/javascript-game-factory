 ...
 */
const CATALOGS: Record<string, { normalizeProfile: (value: any) => any }> = {
...
export function isValidGameProfileSlug(value: any): boolean {
 ...
 * client is drawing.
 */
export async function getGameProfile(pool: any, { playerId, gameSlug }: any = {}): Promise<any> {
  ...
  try {
    const res = await pool.query(
      `select profile, updated_at from game_driver_profiles where player_id = $1 and game_slug = $2`,
    ...
    process.stderr.write(`[game-profiles] getGameProfile error: ${(err as any)?.message || err}\n`);
    return null;
  }
}
 ...
 */
export async function saveGameProfile(pool: any, { playerId, gameSlug, profile }: any = {}): Promise<any> {
  const normalizedPlayerId = cleanText(playerId);
  const slug = normalizeGameSlug(gameSlug);
  const catalog = CATALOGS[slug];
  ...
  try {
    await pool.query(
      `insert into game_driver_profiles (player_id, game_slug, profile, updated_at)
    ...
    process.stderr.write(`[game-profiles] saveGameProfile error: ${(err as any)?.message || err}\n`);
 ...
 */
export async function getGameProfiles(pool: any, { playerIds, gameSlug }: any = {}): Promise<any[]> {
  const slug = normalizeGameSlug(gameSlug);
  const catalog = CATALOGS[slug];
  ...
  try {
    const res = await pool.query(
      `select player_id, profile from game_driver_profiles where game_slug = $1 and player_id = any($2::text[])`,
      [slug, ids],
    );
    const byPlayer = new Map(res.rows.map((row: any) => [row.player_id, row.profile]));
    ...
    process.stderr.write(`[game-profiles] getGameProfiles error: ${(err as any)?.message || err}\n`);
    return [];
  }
}
