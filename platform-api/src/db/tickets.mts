...
function presentWallet(row: any) {
  return {
...
async function ensureTicketWallet(client: any, playerId: string): Promise<void> {
  const inserted = await client.query(
    `insert into ticket_wallets (player_id, balance)
     values ($1, $2)
     on conflict (player_id) do nothing
     returning player_id`,
    [playerId, WELCOME_TICKETS],
  );
...
export async function getTicketWalletInTransaction(client: any, rawPlayerId: unknown) {
...
async function inTransaction<T>(pool: any, work: (client: any) => Promise<T>): Promise<T> {
...
export async function getTicketWallet(pool: any, rawPlayerId: unknown) {
  const playerId = requiredText(rawPlayerId, "playerId");
  return inTransaction(pool, (client) => getTicketWalletInTransaction(client, playerId));
}

export async function awardTicketsInTransaction(client: any, input: any) {
  ...
  const inserted = await client.query(
      `insert into ticket_transactions
       ...
       on conflict (player_id, transaction_key) do nothing
       returning amount`,
      [playerId, transactionKey, amount, reason, JSON.stringify(metadata)],
    );
...
export async function awardTickets(pool: any, input: any) {
...
export async function spendTicketsInTransaction(client: any, input: any) {
  const playerId = requiredText(input?.playerId, "playerId");
  const transactionKey = requiredText(input?.transactionKey, "transactionKey");
  const reason = requiredText(input?.reason, "reason");
  const amount = Number(input?.amount);
  ...
  const existing = await client.query(
    `select amount from ticket_transactions where player_id = $1 and transaction_key = $2`,
    [playerId, transactionKey],
  );
  if (existing.rows?.length) return { ok: true, duplicate: true, spent: 0, balance };

  const debited = await client.query(
    `update ticket_wallets set balance = balance - $2, updated_at = now()
     where player_id = $1 and balance >= $2 returning balance, updated_at`,
    [playerId, amount],
  );
  if (!debited.rows?.length) return { ok: false, error: "insufficient_tickets", duplicate: false, spent: 0, balance };
  await client.query(
    `insert into ticket_transactions (player_id, transaction_key, amount, reason, metadata)
     values ($1, $2, $3, $4, $5::jsonb)`,
    [playerId, transactionKey, -amount, reason, JSON.stringify(metadata)],
  );
...
export async function getTicketShop(pool: any, input: any) {
  const playerId = requiredText(input?.playerId, "playerId");
  const shop = findTicketShop(input?.shopSlug);
  if (!shop) throw new TypeError("unknown ticket shop");
  return inTransaction(pool, async (client) => {
    ...
    const owned = await client.query(
      ...
      [playerId, shop.entitlementGameSlug],
    );
    return {
      shopSlug: shop.slug,
      balance: wallet?.balance ?? 0,
      ownedIds: owned.rows.map((row: any) => String(row.entitlement_id)),
      items: shop.items,
    };
  });
}
 ...
 */
export async function purchaseTicketShopItem(pool: any, input: any) {
  const playerId = requiredText(input?.playerId, "playerId");
  const shop = findTicketShop(input?.shopSlug);
  if (!shop) throw new TypeError("unknown ticket shop");
  const item = shop.findItem(input?.itemId);
  if (!item) throw new TypeError("unknown ticket shop item");
  ...
  return inTransaction(pool, async (client) => {
    ...
    const existing = await client.query(
      `select entitlement_id from game_entitlements
       where player_id = $1 and game_slug = $2 and entitlement_id = $3`,
      [playerId, shop.entitlementGameSlug, item.id],
    );
    ...
    await client.query(
      ...
      [playerId, shop.entitlementGameSlug, item.id, shop.slug],
    );
    return {
      ok: true,
      itemId: item.id,
