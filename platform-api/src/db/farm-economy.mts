...
async function transaction<T>(pool: any, work: (client: any) => Promise<T>): Promise<T> {
...
async function lockedFarm(client: any, playerId: string): Promise<{ layout: any; owned: Set<string> } | null> {
  const result = await client.query(
    `select garage from game_loadouts where player_id = $1 and game_slug = 'farm' for update`,
    [playerId],
  );
  if (!result.rows?.[0]) return null;
  const entitlements = await client.query(
    `select entitlement_id from game_entitlements where player_id = $1 and game_slug = 'farm'`,
    [playerId],
  );
  const owned = new Set<string>(entitlements.rows.map((row: any) => String(row.entitlement_id)));
  return { layout: normalizeFarmGarage(result.rows[0].garage, { ownedEntitlementIds: owned }), owned };
}

async function duplicatePurchase(client: any, playerId: string, transactionKey: string): Promise<boolean> {
  const result = await client.query(
    `select 1 from ticket_transactions where player_id = $1 and transaction_key = $2`,
    [playerId, transactionKey],
  );
  return Boolean(result.rows?.length);
}

async function saveFarm(client: any, playerId: string, layout: any): Promise<void> {
  await client.query(
    `update game_loadouts set garage = $3::jsonb, updated_at = now()
     where player_id = $1 and game_slug = $2`,
    [playerId, "farm", JSON.stringify(layout)],
  );
}
...
export async function adoptFarmPet(pool: any, input: any, random: () => number = Math.random) {
  ...
  return transaction(pool, async (client) => {
    ...
    if ((farm.layout.pets?.length ?? 0) >= MAX_PETS) return { ok: false, error: "farm_full" };
    if (species.habitat === "water" && !(farm.layout.decor ?? []).some((row: any) => String(row.itemId).startsWith("decor.water."))) {
    ...
    if (!spend.ok) return { ok: false, error: spend.error, balance: spend.balance, price: FARM_ADOPTION_PRICE };
...
export async function purchaseFarmSupply(pool: any, input: any) {
  const playerId = required(input?.playerId, "playerId");
  const purchaseId = required(input?.purchaseId, "purchaseId");
  if (!PURCHASE_ID.test(purchaseId)) throw new TypeError("invalid purchaseId");
  const supply = findFarmSupply(input?.itemId);
  if (!supply) return { ok: false, error: "unknown_supply" };
