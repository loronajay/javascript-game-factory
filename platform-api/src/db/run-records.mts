...
} from "../services/leaderboard-catalog.mjs";
 ...
 */
function normalizeInputLog(value: unknown): { ok: boolean; log: unknown[] | null } {
  if (value === null || value === undefined) return { ok: true, log: null };
  if (!Array.isArray(value)) return { ok: false, log: null };
  if (value.length > MAX_INPUT_LOG_ENTRIES) return { ok: false, log: null };
  return { ok: true, log: value };
}

function toRecord(row: any, board: BoardDefinition | null): any {
 ...
 */
export async function recordRun(pool: any, params: any = {}): Promise<any> {
  ...
  if (!pool || !playerId || !board) return null;

  const value = normalizeBoardValue(board, params.value);
  if (value === null) return { error: "implausible_value" };

  const { ok, log } = normalizeInputLog(params.inputLog);
  if (!ok) return { error: "invalid_input_log" };
  ...
  try {
    ...
    const existing = await pool.query(
      `select * from game_run_records where player_id = $1 and game_slug = $2 and board_id = $3`,
      [playerId, gameSlug, board.id],
    );
    return {
      improved: false,
 ...
 */
export async function getPlayerRunRecords(pool: any, params: any = {}): Promise<any> {
  const gameSlug = cleanText(params.gameSlug, 60).toLowerCase();
  const playerId = cleanText(params.playerId, 120);
  ...
  try {
    const res = await pool.query(
      `select player_id, board_id, value, model_id, track_id, verified, recorded_at
         from game_run_records
        where player_id = $1 and game_slug = $2`,
      [playerId, gameSlug],
    );
    const byBoard = new Map<string, any>((res.rows || []).map((row: any) => [row.board_id, row]));
 ...
 */
export async function getBoardStandings(pool: any, params: any = {}): Promise<any> {
  const gameSlug = cleanText(params.gameSlug, 60).toLowerCase();
  const board = getBoard(gameSlug, params.boardId);
  const limit = clampLimit(params.limit, 25, 100);
  if (!pool || !board) return null;

  try {
    const res = await pool.query(
      `select r.player_id, r.value, r.model_id, r.track_id, r.verified, r.recorded_at,
 ...
 */
export async function getBoardPlacement(pool: any, params: any = {}): Promise<any> {
  const gameSlug = cleanText(params.gameSlug, 60).toLowerCase();
  const playerId = cleanText(params.playerId, 120);
  const board = getBoard(gameSlug, params.boardId);
  if (!pool || !playerId || !board) return null;
