...
export function mapRowToRelationshipsRecord(row: any = {}, fallbackPlayerId = ""): any {
...
export function buildRelationshipsParams(playerId: any, relationships: any): any[] {
  return [
    playerId,
...
export function createTimestamp(value: unknown): string {
  return sanitizeTimestamp(value) || new Date().toISOString();
}

export function buildRelationshipPairKey(leftPlayerId: any, rightPlayerId: any): string {
...
function incrementRecordMapCount(record: any, field: any, playerId: any, amount: any): void {
...
function setRecordMapTimestamp(record: any, field: any, playerId: any, timestamp: any): void {
  const key = sanitizePlayerId(playerId);
...
function pushRecentlyPlayedWith(record: any, playerId: any): void {
  const key = sanitizePlayerId(playerId);
  if (!key) return;
  ...
  record.recentlyPlayedWithPlayerIds = [
    key,
    ...current.filter((entry: any) => entry !== key),
  ].slice(0, 8);
}

function compareRelationshipPriority(record: any, leftPlayerId: any, rightPlayerId: any): number {
...
function reorderFriendPlayerIds(record: any): void {
  const current = Array.isArray(record.friendPlayerIds) ? record.friendPlayerIds : [];
  record.friendPlayerIds = [...current].sort((leftPlayerId: any, rightPlayerId: any) => (
    compareRelationshipPriority(record, leftPlayerId, rightPlayerId)
  ));
}

function deriveMostPlayedWithPlayerId(record: any): string {
...
function ensureFriendPlayer(record: any, otherPlayerId: any): void {
...
function awardFriendPoints(record: any, otherPlayerId: any, points: any): void {
  incrementRecordMapCount(record, "friendPointsByPlayerId", otherPlayerId, points);
}

export function buildFriendshipResult(leftRecord: any, rightRecord: any, awarded: any, awardedPoints: any): any {
  return {
    awarded,
    awardedPoints,
...
export function createEmptyRelationshipResult(leftPlayerId: any = "", rightPlayerId: any = ""): any {
...
export function applyFriendshipState(leftRecord: any, rightRecord: any, options: any = {}): any {
...
export function applySharedSessionState(leftRecord: any, rightRecord: any, options: any = {}): any {
  const normalizedLeftRecord = normalizeProfileRelationshipsRecord(leftRecord);
  const normalizedRightRecord = normalizeProfileRelationshipsRecord(rightRecord);
  const leftPlayerId = sanitizePlayerId(options.leftPlayerId || normalizedLeftRecord.playerId);
  const rightPlayerId = sanitizePlayerId(options.rightPlayerId || normalizedRightRecord.playerId);
...
export function applySharedEventState(leftRecord: any, rightRecord: any, options: any = {}): any {
  const normalizedLeftRecord = normalizeProfileRelationshipsRecord(leftRecord);
  const normalizedRightRecord = normalizeProfileRelationshipsRecord(rightRecord);
  const leftPlayerId = sanitizePlayerId(options.leftPlayerId || normalizedLeftRecord.playerId);
  const rightPlayerId = sanitizePlayerId(options.rightPlayerId || normalizedRightRecord.playerId);
...
export function applyFriendRemovalState(leftRecord: any, rightRecord: any, options: any = {}): any {
  const normalizedLeftRecord = normalizeProfileRelationshipsRecord(leftRecord);
  const normalizedRightRecord = normalizeProfileRelationshipsRecord(rightRecord);
  const leftPlayerId = sanitizePlayerId(options.leftPlayerId || normalizedLeftRecord.playerId);
  const rightPlayerId = sanitizePlayerId(options.rightPlayerId || normalizedRightRecord.playerId);
  ...
  normalizedLeftRecord.friendPlayerIds = (normalizedLeftRecord.friendPlayerIds || [])
    .filter((playerId: any) => playerId !== rightPlayerId);
  normalizedRightRecord.friendPlayerIds = (normalizedRightRecord.friendPlayerIds || [])
    .filter((playerId: any) => playerId !== leftPlayerId);
...
export function applyDirectInteractionState(leftRecord: any, rightRecord: any, options: any = {}): any {
  const normalizedLeftRecord = normalizeProfileRelationshipsRecord(leftRecord);
  const normalizedRightRecord = normalizeProfileRelationshipsRecord(rightRecord);
  const leftPlayerId = sanitizePlayerId(options.leftPlayerId || normalizedLeftRecord.playerId);
  const rightPlayerId = sanitizePlayerId(options.rightPlayerId || normalizedRightRecord.playerId);
