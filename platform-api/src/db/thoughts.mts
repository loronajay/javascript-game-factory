...
export async function listThoughts(db: any, options: any = {}): Promise<any> {
  ...
  const thoughtIds = rows.map((row: any) => sanitizeThoughtId(row.id)).filter(Boolean);
  ...
  let viewerReactionRows: any[] = [];
  let viewerShareRows: any[] = [];
  if (viewerPlayerId) {
  ...
  const reactionTotalsByThoughtId = mapReactionTotalsByThoughtId(
    ...
    rows.map((row: any) => ({ thought_id: row.id, reaction_totals: row.reaction_totals })),
  );
  ...
  return rows.map((row: any) => mapRowToThought(row, {
...
export async function listThoughtComments(db: any, thoughtId: any, options: any = {}): Promise<any> {
  ...
  return (result?.rows || []).map((row: any) => mapRowToComment(row));
}

export async function saveThought(db: any, thought: any = {}): Promise<any> {
...
export async function deleteThoughtComment(
  db: any,
  thoughtId: any,
  commentId: any,
  requesterPlayerId: any,
): Promise<{ ok: boolean; reason?: string; thought?: any; commentId?: string; commentCount?: number }> {
  const normalizedThoughtId = sanitizeThoughtId(thoughtId);
...
export async function reactToThought(db: any, thoughtId: any, viewerPlayerId: any, reactionId: any): Promise<any> {
  const normalizedThoughtId = sanitizeThoughtId(thoughtId);
...
export async function shareThought(db: any, thoughtId: any, viewerPlayerId: any, viewerAuthorDisplayName: any = "", options: any = {}): Promise<any> {
  const normalizedThoughtId = sanitizeThoughtId(thoughtId);
  const normalizedViewerPlayerId = sanitizeViewerPlayerId(viewerPlayerId);
  ...
  try {
    let insertedSharedThought: any = null;
    const thoughtResult = await db.query(`
      select
        id,
        author_player_id,
...
export async function commentOnThought(db: any, thoughtId: any, viewerPlayerId: any, viewerAuthorDisplayName: any = "", text: any = ""): Promise<any> {
  const normalizedThoughtId = sanitizeThoughtId(thoughtId);
  const normalizedViewerPlayerId = sanitizeViewerPlayerId(viewerPlayerId);
  const normalizedViewerDisplayName = sanitizeViewerAuthorDisplayName(viewerAuthorDisplayName);
