function createFriendRequestId(): string {
  return `fr-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
}

function mapRowToFriendRequest(row: any = {}): any {
...
export async function createFriendRequest(db: any, {
  id,
  fromPlayerId,
  toPlayerId,
  fromDisplayName = "",
...
export async function findPendingFriendRequest(db: any, fromPlayerId: any, toPlayerId: any): Promise<any> {
  if (!db || !fromPlayerId || !toPlayerId) return null;
...
export async function getFriendRequest(db: any, id: any): Promise<any> {
...
export async function acceptFriendRequest(db: any, id: any): Promise<any> {
  if (!db || !id) return null;
...
export async function rejectFriendRequest(db: any, id: any): Promise<any> {
  if (!db || !id) return null;
