import { normalizeThoughtComment, THOUGHT_REACTION_IDS, normalizeThoughtPost, } from "../normalize.mjs";
...
export function mapReactionTotalsByThoughtId(reactionRows = [], fallbackRows = []) {
    ...
    for (const fallbackRow of fallbackRows) {
        if (!fallbackRow?.thought_id)
            continue;
        totalsByThoughtId.set(fallbackRow.thought_id, {
...
export function mapViewerShareByOriginalThoughtId(rows = []) {
    ...
    for (const row of rows) {
        ...
        if (!originalThoughtId || !sharedThoughtId)
            continue;
...
export function createSharedThoughtId(originalThoughtId, viewerPlayerId, options = {}) {
    return sanitizeThoughtId(`thought-share-${sanitizeThoughtId(viewerPlayerId).slice(0, 24)}-${sanitizeThoughtId(originalThoughtId).slice(0, 24)}-${createSuffix(options)}`);
}
export function createCommentId(thoughtId, viewerPlayerId, options = {}) {
    return sanitizeThoughtId(`comment-${sanitizeThoughtId(viewerPlayerId).slice(0, 24)}-${sanitizeThoughtId(thoughtId).slice(0, 24)}-${createSuffix(options)}`);
}
