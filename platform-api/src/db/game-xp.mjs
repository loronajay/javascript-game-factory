...
function mergeTrackStats(definition, stored, incoming) {
    const merged = {};
...
export async function recordYamBowlingCareerMatch(client, params = {}) {
    ...
    if (!definition)
        return { entitlementIds: [] };
    const existing = await client.query(`select stats from game_xp_tracks
     where player_id = $1 and game_slug = $2 and track_id = $3 for update`, [params.playerId, params.gameSlug, params.trackId]);
    const stats = mergeTrackStats(definition, existing.rows[0]?.stats || {}, params);
    await client.query(`insert into game_xp_tracks (player_id, game_slug, track_id, stats)
     values ($1, $2, $3, $4::jsonb)
     on conflict (player_id, game_slug, track_id) do update
       set stats = excluded.stats, updated_at = now()`, [params.playerId, params.gameSlug, params.trackId, JSON.stringify(stats)]);
    const allTracks = await client.query(`select track_id, stats from game_xp_tracks where player_id = $1 and game_slug = $2`, [params.playerId, params.gameSlug]);
    const tracks = Object.fromEntries((allTracks.rows || []).map((row) => [String(row.track_id), row.stats || {}]));
    const entitlementIds = [];
    for (const entitlementId of earnedYamBowlingCareerBadges(tracks)) {
        const granted = await client.query(`insert into game_entitlements (player_id, game_slug, entitlement_id, kind, source, source_id)
       values ($1, $2, $3, 'badge', 'career-achievement', $4)
       on conflict (player_id, game_slug, entitlement_id) do nothing`, [params.playerId, params.gameSlug, entitlementId, params.sourceId || "career"]);
...
function resolveDisputedMode(gameSlug, params) {
    ...
    if (!attested || attested === params.modeId || !claimed.eligible)
        return claimed;
...
async function persistXpAward(client, definition, params, verdict) {
    const ledger = await client.query(`insert into game_xp_grants (player_id, game_slug, grant_id, track_id, xp, source)
     values ($1, $2, $3, $4, $5, $6)
     on conflict (player_id, game_slug, grant_id) do nothing`, [params.playerId, params.gameSlug, params.grantId, params.trackId, verdict.xp, params.source]);
    if (ledger.rowCount === 0)
        return { awarded: false, reason: "already-granted" };
    const profile = await client.query(`insert into game_xp_profiles (player_id, game_slug, xp, matches, updated_at)
     values ($1, $2, $3, 1, now())
     on conflict (player_id, game_slug) do update
       set xp = game_xp_profiles.xp + excluded.xp,
           matches = game_xp_profiles.matches + 1,
           updated_at = now()
     returning xp`, [params.playerId, params.gameSlug, verdict.xp]);
...
export async function awardMatchXp(client, params) {
    const { playerId, gameSlug, grantId, trackId, modeId, outcome } = params || {};
    ...
    if (!definition)
        return { awarded: false, reason: "game-not-registered" };
    const verdict = resolveDisputedMode(gameSlug, params);
    if (!verdict.eligible)
        return { awarded: false, reason: verdict.reason };
...
export async function awardCampaignXp(client, params) {
    const { playerId, gameSlug, grantId, trackId, kind } = params || {};
    if (!client || !playerId || !gameSlug || !grantId || !trackId) {
        return { awarded: false, reason: "incomplete-grant" };
    }
    const definition = getProgression(gameSlug);
...
export async function getGameXpProgress(pool, playerId, gameSlug) {
    ...
    try {
        ...
        return {
            ...
            grants: (grants.rows || []).map((row) => String(row.grant_id)).reverse(),
            syncedAt: profile.rows[0]?.updated_at || null,
        };
    }
    catch (err) {
        process.stderr.write(`[game-xp] getGameXpProgress error: ${err?.message || err}\n`);
        return null;
    }
}
