...
function presentTickets(awarded, balance, reward) {
    return {
        awarded,
        balance,
...
export function fitsTimeBudget(elapsedMs, claimedMs) {
    return elapsedMs >= claimedMs * FENCE_RATIO - FENCE_SLACK_MS;
}
export async function submitGameResult(pool, params = {}) {
    ...
    try {
        ...
        const prior = await client.query(`select ticket_awarded, ticket_breakdown
       from game_ticket_results
       where player_id = $1 and game_slug = $2 and result_id = $3`, [playerId, gameSlug, resultId]);
        ...
        const last = await client.query(`select extract(epoch from (now() - submitted_at)) * 1000 as elapsed_ms
       from game_ticket_results
       where player_id = $1
       ...
       limit 1`, [playerId]);
        ...
        await client.query(`insert into game_ticket_results
       ...
       on conflict (player_id, game_slug, result_id) do nothing`, [playerId, gameSlug, resultId, JSON.stringify(result), durationMs, awarded, JSON.stringify(reward)]);
        const wallet = await getTicketWalletInTransaction(client, playerId);
        await client.query("commit");
