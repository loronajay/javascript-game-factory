...
export const VALID_OUTCOMES: readonly string[] = Object.freeze(["win", "loss", "draw"]);

export function eloExpected(ratingA: number, ratingB: number): number {
  return 1 / (1 + Math.pow(10, (ratingB - ratingA) / 400));
}

export function provisionalK(gamesPlayed: number): number {
...
export function sameOpponentGainFactor(priorMeetings: number): number {
...
export function computeRankedRatings({
  ratingA,
  ratingB,
  outcomeA,
  kA = K_ESTABLISHED,
...
export function reportsAgree(reportA: string, reportB: string): boolean {
  if (!reportA || !reportB) return false;
  if (reportA === "draw" || reportB === "draw") return reportA === "draw" && reportB === "draw";
  return reportA !== reportB;
}

// Convert a single side's own-perspective outcome into A's perspective.
export function toOutcomeA(side: string, outcome: string): string {
  if (side === "a") return outcome;
  if (outcome === "win") return "loss";
  if (outcome === "loss") return "win";
  return "draw";
}

export function outcomeScoreA(outcomeA: string): number {
  return outcomeA === "win" ? 1 : outcomeA === "draw" ? 0.5 : 0;
}

export const RANK_TIERS: readonly any[] = Object.freeze([
...
export function rankTier(rating: number): any {
...
export function banFirstIndex(seed: string): number {
  let h = 0;
  const s = String(seed);
...
export function decideReport({
  ...
  existingReportB,
  matchAgeSeconds,
  minMatchSeconds = MIN_MATCH_SECONDS,
  graceSeconds = FORFEIT_GRACE_SECONDS,
  now,
}: any): any {
...
export function decideLivenessForfeit({
  status,
  reportA,
  reportB,
  heartbeatA,
  ...
  if (Number.isNaN(aMs) || Number.isNaN(bMs) || Number.isNaN(nowMs)) return { action: "none" };
  ...
  if (aSilent && bPresent) return { action: "resolve", outcomeA: "loss", reason: "abandoned" };
  if (bSilent && aPresent) return { action: "resolve", outcomeA: "win", reason: "abandoned" };
  return { action: "none" };
}
...
export function decideForfeitFinalize({ reportA, reportB, forfeitDeadline, now }: any): any {
  const claim = reportA || reportB;
  if (!claim) return { action: "none" };
  const base = now ? new Date(now).getTime() : Date.now();
