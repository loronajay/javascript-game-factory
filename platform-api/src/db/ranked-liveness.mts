...
import {
  RANKED_HEARTBEAT_STALE_SECONDS,
  decideLivenessForfeit,
} from "./ranked-elo.mjs";
...
function heartbeatColumn(side: string): string {
...
// double-applying ELO.
export async function resolveAbandonedRankedMatches(client: any, gameSlug: any, now: any = null): Promise<number> {
...
export async function sweepRankedMaintenance(pool: any, gameSlug: any, now: any = null): Promise<boolean> {
  if (!pool || !gameSlug) return false;
...
export async function recordRankedHeartbeat(pool: any, { matchId, gameSlug, playerId, now = null }: any): Promise<any> {
  ...
  try {
    ...
    await client.query(
      `update ranked_matches set ${heartbeatColumn(side)}=$1 where match_id=$2 and game_slug=$3`,
      [now ? new Date(now) : new Date(), matchId, gameSlug],
    );
    await client.query("commit");
  } catch (err: any) {
    await client.query("rollback").catch(() => {});
  ...
  try {
    const after = await pool.query(
      `select * from ranked_matches where match_id=$1 and game_slug=$2`,
      [matchId, gameSlug],
    );
    const row = after.rows[0];
    if (!row) return { ok: true, finished: true, status: "voided", match: null };
    return {
      ok: true,
