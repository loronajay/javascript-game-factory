...
function outcomeForSeat(outcomeA, seat) {
    if (!outcomeA)
        return null;
    if (outcomeA === "draw")
        return "draw";
...
export function rankedMatchNotes(row, { verified, rated }) {
...
export function rankedMatchRecord(row, gameSlug) {
...
export async function getRankedMatches(pool, { playerId, gameSlug, limit }) {
    ...
    try {
        const res = await pool.query(`select ${MATCH_HISTORY_COLUMNS} ${MATCH_HISTORY_JOINS}
        where m.game_slug=$1 and m.status='resolved' and (m.player_a=$2 or m.player_b=$2)
        order by m.resolved_at desc limit $3`, [gameSlug, playerId, cap]);
        const matches = (res.rows || [])
            .map((row) => buildMatchHistoryEntry(rankedMatchRecord(row, gameSlug), { viewerPlayerId: playerId }))
            .filter(Boolean);
...
export async function getRankedMatchDetail(pool, { matchId, gameSlug, viewerPlayerId }) {
    if (!pool || !matchId || !gameSlug)
        return null;
    try {
        const res = await pool.query(`select ${MATCH_HISTORY_COLUMNS} ${MATCH_HISTORY_JOINS}
        where m.game_slug=$1 and m.match_id=$2 and m.status = any($3::text[])
        limit 1`, [gameSlug, matchId, FINISHED_STATUSES]);
        const row = res.rows?.[0];
        if (!row)
            return { error: "match_not_found" };
        const isMember = viewerPlayerId === row.player_a || viewerPlayerId === row.player_b;
        if (viewerPlayerId && !isMember)
