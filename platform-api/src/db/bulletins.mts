...
export interface BulletinRecord {
  ...
  publishedAt: string;
  createdBy: string;
  // The author's display name, resolved from the profile that `createdBy` points at.
  // Empty when the id has no profile (or on an insert/update, which cannot join), and the
  // browser falls back to `createdBy` then. Stored ids stay the durable reference; this is
  // presentation only, so renaming a profile renames the byline everywhere at once.
...
function mapRow(row: any): BulletinRecord {
  return {
    id: String(row.id),
...
export async function listPublicBulletins(pool: any, options: any = {}): Promise<BulletinRecord[]> {
  if (!pool) return [];
  const limit = Math.min(Math.max(Number.parseInt(String(options?.limit || "50"), 10) || 50, 1), 200);
  try {
    const result = await pool.query(
      `select ${SELECT_WITH_AUTHOR} from bulletins b ${AUTHOR_JOIN}
...
export async function listAllBulletins(pool: any): Promise<BulletinRecord[]> {
  if (!pool) return [];
  try {
    const result = await pool.query(
      `select ${SELECT_WITH_AUTHOR} from bulletins b ${AUTHOR_JOIN}
...
export async function getPublicBulletinBySlug(pool: any, slug: any): Promise<BulletinRecord | null> {
  if (!pool || !slug) return null;
  try {
    const result = await pool.query(
      `select ${SELECT_WITH_AUTHOR} from bulletins b ${AUTHOR_JOIN}
        where b.slug = $1 and b.status = 'published' and b.audience = 'public'`,
      [String(slug)],
    );
...
export async function createBulletin(pool: any, input: any, createdBy: any): Promise<{ ok: boolean; error?: string; bulletin?: BulletinRecord }> {
  ...
  try {
    ...
    process.stderr.write(`[bulletins] createBulletin error: ${(err as any)?.message || err}\n`);
    return { ok: false, error: "server_error" };
  }
}

export async function updateBulletin(pool: any, id: any, input: any): Promise<{ ok: boolean; error?: string; bulletin?: BulletinRecord }> {
  ...
  try {
    ...
    if ((err as any)?.code === "23505") return { ok: false, error: "slug_taken" };
    process.stderr.write(`[bulletins] updateBulletin error: ${(err as any)?.message || err}\n`);
    return { ok: false, error: "server_error" };
  }
}
...
export async function claimBulletinAnnouncement(pool: any, id: any): Promise<boolean> {
  if (!pool || !id) return false;
  try {
    const result = await pool.query(
      `update bulletins set announced_at = now()
    ...
    process.stderr.write(`[bulletins] claimBulletinAnnouncement error: ${(err as any)?.message || err}\n`);
    return false;
  }
}

export async function deleteBulletin(pool: any, id: any): Promise<{ ok: boolean; error?: string }> {
  if (!pool || !id) return { ok: false, error: "invalid_request" };
  try {
