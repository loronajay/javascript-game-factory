...
function buildProfileOpenSourceBreakdown(value: any = {}): Record<string, number> {
  const source = isPlainObject(value) ? value : {};
  return PROFILE_OPEN_SOURCE_KEYS.reduce((normalized: Record<string, number>, key) => {
    normalized[key] = sanitizeCount(source[key]);
    return normalized;
  }, {} as Record<string, number>);
}

export function buildDefaultProfileMetricsRecord(playerId: any = "") {
  return {
...
export function normalizeProfileMetricsRecord(record: any = {}) {
